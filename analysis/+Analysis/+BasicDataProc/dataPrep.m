...
[varargin, DoBandFilterCanonical] = util.argkeyval('DoBandFilterCanonical', varargin, true);  %if you want to also run canonical basic band filters (different than the 2 hz filter method below) in classic frequencies (delta 1-4 theta 4-8 alpha 8-13 beta 13-30 gamma 30-50 high gamma 50 to 200)
[varargin, bandPassed] = util.argkeyval('Bandpassed', varargin, false); %alternate to spectrogram to run filters and build them on top of each other at 2 hz bands with band squared.
[varargin, hilbertPowerAngle] = util.argkeyval('hilbertPowerAngle', varargin, true); %alternate to spectrogram to run filters and build them on top of each other at 2 hz bands with hilbert (records the angle as well)
...
if bandPassed || DoBandFilterCanonical
...
if Spectrogram
 ...
 if bandPassed
     ...
     if isempty(bandfilter)
         ...
         [bandfilter]=Analysis.PAC.bandfiltersAP( fs, 'AmpFreq_BandWidth', Freq_BandWidth, 'MinAmpFreq', MinFreq, 'MaxAmpFreq', MaxFreq); %create the band filters
     end
 ...
 if hilbertPowerAngle
     ...
     %remove the mirrored end
     dataTempEnv=tempMDCenv(:,ramp:end-ramp,:);
     dataTempAngle=tempMDCang(:,ramp:end-ramp,:);
     filtData.Hilbert.Power = dataTempEnv;
     filtData.Hilbert.Angle = dataTempAngle;
     %dataTemp=permute(dataTemp, [2,1,3]);
     toc(tt)
     ...
     tplot = 0:1/fs:size(dataTempEnv,2)/fs;
     filtData.Hilbert.tplot=tplot;
     filtData.Hilbert.f=freqBand;
     filtData.Hilbert.dB = 'not 10*log10 passed';
 end


%%CanonicalBands        
        
if DoBandFilterCanonical
    if  IIR
...
if bandPassed || Spectrogram || DoBandFilterCanonical
    if dBPower
        ...
        elseif DoBandFilterCanonical && ~IIR
