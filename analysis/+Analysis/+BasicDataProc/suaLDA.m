function [acc,fracZeroVar, h] = suaLDA(spk1, spk2, varargin)
...
[varargin, win]=util.argkeyval('win', varargin, 50); %window over which to do the LDA
[varargin, decPCA]=util.argkeyval('decPCA', varargin, true); %do a pca and then decode (remember must do the pca only on the training data
[varargin, numPC]=util.argkeyval('numPC', varargin, 20); %number of PCs to do. can get fancier later about variance or to maximize.

[varargin, plt]=util.argkeyval('plt', varargin, true); %toggle plotting the decode on and off
[varargin, multPlt]=util.argkeyval('multPlt', varargin, false); %if you are trying to do multiple plots on the same figure
...
[varargin, lineColor]=util.argkeyval('lineColor', varargin, [.10, .22, .43]); %optional title
...
acc = nan(round(nTime/win),1);

%windows to decode over, default 50 ms
idx = 1;
for ii = 1:win:nTime-win-1
    ...
    for jj = 1:nUnits        
        X1(:,jj) = mean(spk1{jj,1}(:,ii:ii+win),2);
        X2(:,jj) = mean(spk2{jj,1}(:,ii:ii+win),2);
        if ~isempty(spk3) 
            X1(:,jj) = mean(spk1{jj,1}(:,ii:ii+win),2);
            X2(:,jj) = mean(spk2{jj,1}(:,ii:ii+win),2);
            X3(:,jj) = mean(spk3{jj,1}(:,ii:ii+win),2);        
        end
    end
    ...
    for fold = 1:k
        ...
        if decPCA %if going to do a pca and then decode (
            [coeff, scoreTrain] = pca(Xz(trainIdx,:));
            %  test data in same PC space
            scoreTest = Xz(testIdx,:) * coeff;

            % Keep top PCs
            Ztrain = scoreTrain(:,1:numPC);
            Ztest  = scoreTest(:,1:numPC);
        else
            Ztrain = Xz(trainIdx,:);
            Ztest = Xz(testIdx,:);
        end
        ...
        lda = fitcdiscr(Ztrain, labels(trainIdx), ...
                'DiscrimType','pseudoLinear');
         % test
        yhat = predict(lda, Ztest); %predicts which are in which label
    ...
    acc(idx) = mean(foldAcc);
    if ~isempty(spk3)
            confMat(:,:,idx) = foldCM ./ sum(foldCM,2);
    end
    idx = idx+1;
end
...
if plt 
    ...
    timeAdjIdx = 0-st:win:nTime-st-win;
    ttt = tt(1):win:tt(2)-1;   
    [~, ttidx(1)] = min(abs(timeAdjIdx - tt(1)));    
    [~, ttidx(2)] = min(abs(timeAdjIdx - tt(2)));    

    if multPlt == false
        myTitleTemp = erase(myTitle, newline);
        figure('Name', ['LDA decoding ', myTitleTemp])
    else
        hold on
        
    end
    set(0, 'DefaultAxesFontSize', 14)
    h = plot(ttt, acc(ttidx(1)+1:ttidx(2)),'LineWidth',1.2, 'Color', lineColor);
