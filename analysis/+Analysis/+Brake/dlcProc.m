function [spikeData, peakAmp] = dlcProc(spkData, posData, T, K, N, varargin)
%dlcProc is the workhorse of data tremor alignment of kinematics to
%neural  alignment. Will have a brake component and a tremor component that
%feed into this.
%   
%   Inputs
%       spkData - raw spike data filtered with typical low and high pass
%       filters from the AO //OR will be spike times already occurring from
%       a spike filtering/sorting process
%       posData - should be in x,y,z and position, not accel or velocity,
%       in ms
%   Outputs
%        spikeData - all of the spike outputs from the raw voltage
        
[varargin, spikeProcessed] = util.argkeyval('spikeProcessed',varargin, 1); %1 if spike data is processed to spike times (and sorted) 0 if not and needs that (built for threshold crossing, but not sorting)
[varargin, spkStartTime] = util.argkeyval('spkStartTime',varargin, 1); %what's the first known time stamp for this spike time (in ms). It will be the first spike recorded, which is always before the trials start and the time scale is from AO so it's the same as that of the trial markers)
[varargin, fsSpk] = util.argkeyval('fsSpk',varargin, 1250); %sampling rate of spike data
[varargin, fsPos] = util.argkeyval('fsPos',varargin, 120); %sampling rate of position data
[varargin, posDataTime] = util.argkeyval('posDataTime',varargin, []); %if the time relative to the AO clock is entered or not
[varargin, spuriousPoint] = util.argkeyval('spuriousPoint',varargin, []); %if the time relative to the AO clock is entered or not
...
[varargin, plotSmPosData] = util.argkeyval('plotSmPosData',varargin, 0); %plot smoothed pos data to make sure it looks ok 
...
if ~spikeProcessed
%% make spike raster
[spikeRaster, spikeTimeStamp, spkMsRaster]=Analysis.BasicDataProc.spikeTime(spkData, 'rmsMethod', 1, 'threshMultiplier', -4.5, 'fs', fsSpk);
else
    %THIS CURRENTLY CONVERTS TO 1MS WINDOW FOR A SPIKE AND THE INPUT IS FROM .8 MS
    %PER SPIKE.
    [spikeTimeStamp, spkMsRaster, tmSpk] = Analysis.BasicDataProc.spikeTime2msTimes(spkData, fsSpk, T.Time.StartOfSession, T.Time.TrialTerminated);
end
%% smooth spikes
[spk, spkRate, spkRateSmooth, tmSpkSmooth] = Analysis.BasicDataProc.spikeRate(spkMsRaster, 'win', win/1000, 'timeSt', spkStartTime);
...
if abs(tmSpkSmooth(end)-tmPosTotalMs)>=10
    warning('more than 10ms difference between position data time and spike data time. Note in AO recordings, spike recordings are usually longer so can ignore');
end

%% load spike data for output
spikeData=struct;
spikeData.spikeMsRaster=spkMsRaster;
spikeData.spikeMsTm=tmSpk;
spikeData.spk=spk;
spikeData.spkRate=spkRate;
spikeData.spkRateSmooth=spkRateSmooth;
spikeData.timeSpkSmooth=tmSpkSmooth;

%% time convert
%make the position time match the AO clock of everything else, if not
%already done
if isempty(posDataTime)
    tmPos=linspace(0, length(posData)/(fsPos/1000), length(posData));
else
    if length(posDataTime)==size(posData,1)
        tmPos=posDataTime;
    else
        warning('position time does not match position data')
        tmPos=linspace(0, length(posData)/(fsPos/1000), length(posData));
    end
end

if abs(tmPos(1)-tmSpk(1))>100
    tmPos=tmPos*1000;
    warning('time for position data likely not in ms, multiplied by 1000 to change to ms');
end

%% smooth positional data
posDataSm=smoothdata(posData, 'gaussian', fsPos/10); %smooth the jitter in the positional data
...
perTrialData=struct;
%% break up trials
for ii=1:length(T.Target_idx)
    if ii<length(T.Target_idx)
        trialSt=round(T.Time.CenterIsShown(ii)*1000); %get the trial start time in ms
        trialEnd=round(T.Time.CenterIsShown(ii+1)*1000); %get the trial end time in ms
    else
        trialSt=round(T.Time.CenterIsShown(ii)*1000);
        trialEnd=round(T.Time.EndOfTarget(ii)*1000);
    end
    trialStspk=find(tmSpk>=trialSt);
    trialEndspk=find(tmSpk<=trialEnd);
    perTrialData.spkMsRaster{ii}=spkMsRaster(trialStspk(1):trialEndspk(end));
    trialStspkSmooth=find(tmSpkSmooth>=trialSt);
    trialEndspkSmooth=find(tmSpkSmooth<=trialEnd);
    perTrialData.spkRateSmooth{ii}=spkRateSmooth(trialStspkSmooth(1):trialEndspkSmooth(end));
    trialStPos=find(tmPos>=trialSt);
    trialEndPos=find(tmPos<=trialEnd);
    perTrialData.posData{ii}=posDataSm(trialStPos(1):trialEndPos(end),:);
...
%which direction of the 3 is being focused on. SHOULDN'T NEED DIRECTION,
%
% switch direction
%     case 1 %x
...
for ii=1:length(timeOrderMs)-1
    ...
    for jj=0:binMs:peak2peak1-binMs
        spikeCount(idx,ii)=sum(spkMsRaster(timeOrderMs(ii)+jj:timeOrderMs(ii)+jj+binMs));
        idx=idx+1;
    end
...
for ii=epochSt(1):epochEnd(end)
    tmTemp=find(tmSpkSmooth>=timeOrderMs(ii,peakOne(1))); %identify the spike time of the peak of interest
...
idx=1;
for ii=epochSt(1):epochEnd(end)
    tmTemp=find(tmSpkSmooth>=timeOrderMs(ii,peakOne(2)));
