...
[varargin, clr12]=util.argkeyval('clr12', varargin, {'#58126A', '#F6B2E1', '#07A3B2', '#D9ECC7'}); %%dark to light purple, and dark to light sea green, color choices when doing just 2 conditions.
[varargin, clr3]=util.argkeyval('clr3', varargin,  {'#395886', '#B1C9EF', '#38000A', '#FFA896', '#253D2C', '#CFFFDC'}); %it's a blue, red, green transition, color choices when doing 3 conditions.
...
switch dataCenter
    case 1 %stim on
        ...
        if isempty(tt) %adjust the time if you only want to do smaller sections of the total time
            ...
            tt(1) = 1;
            tt(2) = length(spk.congMean{1});
        else          
            timeMask(1) = tt(1)-preStim; %find the location of these time points in the data
    ...
    case 2
        ...
        eventIdx = -tt(1); %find the location of the centered event on the truncated data, the event tt is + (and therefore after the event) it will label the first time point (incorrectly, but then just turn the label off)
        eventLbl = 'Response';
    ...
    case 3
        ...
        eventIdx = 1; %find the location of the centered event on the truncated data, for ITI, it's just the start. in the event tt is + (and therefore after the event) it will label the first time point (incorrectly, but then just turn the label off)
        eventLbl = 'ITI';
end
...
switch pcaRun
    case 1
        ...
        if plt3d
            ...
            colorOut1 = plt.PCAcolorplt(pc1, pc2, 'data3', pc3, 'colorChoice', clr12(1:2), 'eventIdx', eventIdx, 'eventLbl', eventLbl, 'titlePCA', centerDataOn{2});

            pc1 = PCdata.PCxTimeI(:,1);
            pc2 = PCdata.PCxTimeI(:,2);
            pc3 = PCdata.PCxTimeI(:,3);
            colorOut2 = plt.PCAcolorplt(pc1, pc2, 'data3', pc3,'colorChoice', clr12(3:4), 'newFig', false, 'eventIdx', eventIdx, 'eventLbl', eventLbl);
            
            hold on
            h1 = plot3(nan, nan, nan, 'LineWidth', 3, 'Color', colorOut1); % dummy
            h2 = plot3(nan, nan, nan, 'LineWidth', 3, 'Color', colorOut2); % dummy
    ...
    case 2 %=
        ...
        if plt3d
            ...
            titlePCAtemp = strcat(centerDataOn{2}, ' Congruent');
           
            pc1 = PCdata.PCxTimeC1(:,1);
            pc2 = PCdata.PCxTimeC1(:,2);
            pc3 = PCdata.PCxTimeC1(:,3);
            colorOut1 = plt.PCAcolorplt(pc1, pc2, 'data3', pc3, 'colorChoice',...
                clr3(1:2), 'eventIdx', eventIdx, 'eventLbl', eventLbl, 'dataLbl', 'Response 1', 'titlePCA', titlePCAtemp);

            pc1 = PCdata.PCxTimeC2(:,1);
            pc2 = PCdata.PCxTimeC2(:,2);
            pc3 = PCdata.PCxTimeC2(:,3);
            colorOut2 = plt.PCAcolorplt(pc1, pc2, 'data3', pc3,'colorChoice',...
                clr3(3:4), 'newFig', false, 'eventIdx', eventIdx, 'eventLbl', eventLbl, 'dataLbl', 'Response 2');
        
            pc1 = PCdata.PCxTimeC3(:,1);
            pc2 = PCdata.PCxTimeC3(:,2);
            pc3 = PCdata.PCxTimeC3(:,3);
            colorOut3 = plt.PCAcolorplt(pc1, pc2, 'data3', pc3,'colorChoice',...
               clr3(5:6), 'newFig', false, 'eventIdx', eventIdx, 'eventLbl', eventLbl, 'dataLbl', 'Response 3');
          
            
            hold on
            h1 = plot3(nan, nan, nan, 'LineWidth', 3, 'Color', colorOut1); % dummy
            h2 = plot3(nan, nan, nan, 'LineWidth', 3, 'Color', colorOut2); % dummy
            h3 = plot3(nan, nan, nan, 'LineWidth', 3, 'Color', colorOut3); % dummy

            legend([h1, h2, h3], {'Response 1', 'Response 2', 'Response 3'});
            clear titlePCAtemp
            titlePCAtemp = strcat(centerDataOn{2}, ' Incongruent');
            
            if length(clr3) == 12 %if you put in 6 colors
                cc = 6;
            else
                cc = 0;
            end
            ...
            colorOut1 = plt.PCAcolorplt(pc1, pc2, 'data3', pc3, 'colorChoice',...
                clr3(1+cc:2+cc), 'eventIdx', eventIdx, 'eventLbl', eventLbl, 'dataLbl', 'Response 1','titlePCA', titlePCAtemp);

            pc1 = PCdata.PCxTimeI2(:,1);
            pc2 = PCdata.PCxTimeI2(:,2);
            pc3 = PCdata.PCxTimeI2(:,3);
            colorOut2 = plt.PCAcolorplt(pc1, pc2, 'data3', pc3,'colorChoice',...
                clr3(3+cc:4+cc), 'newFig', false, 'eventIdx', eventIdx, 'eventLbl', eventLbl, 'dataLbl', 'Response 2');
        
            pc1 = PCdata.PCxTimeI3(:,1);
            pc2 = PCdata.PCxTimeI3(:,2);
            pc3 = PCdata.PCxTimeI3(:,3);
            colorOut3 = plt.PCAcolorplt(pc1, pc2, 'data3', pc3,'colorChoice',...
                clr3(5+cc:6+cc), 'newFig', false, 'eventIdx', eventIdx, 'eventLbl', eventLbl, 'dataLbl', 'Response 3');
        
           
            hold on
            h1 = plot3(nan, nan, nan, 'LineWidth', 3, 'Color', colorOut1); % dummy, uses the midpoint color to color the legend
            h2 = plot3(nan, nan, nan, 'LineWidth', 3, 'Color', colorOut2); % dummy
            h3 = plot3(nan, nan, nan, 'LineWidth', 3, 'Color', colorOut3); % dummy
