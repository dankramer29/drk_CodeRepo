...
%     if (MWallForLoading(1).RecordingLocation{1} == "L Amygdala") && (MWallForLoading.SigClusterSummStats(ii,:).AllImagesSignificant == 1) && ...
%     (MWallForLoading.SigClusterSummStats(ii,:).TrialType{1} == "identityTask") && (MWallForLoading.SigClusterSummStats(ii,:).TrialNumber{1} == 1) 
%         xx(idxX,1) = MWallForLoading.SigClusterSummStats(ii,:).TstatCluster;
%         idxX = idxX + 1;
%     elseif (MWallForLoading.SigClusterSummStats(ii,:).RecordingLocation{1} == "R Amygdala") && (MWallForLoading.SigClusterSummStats(ii,:).AllImagesSignificant == 1) && ...
%     (MWallForLoading.SigClusterSummStats(ii,:).TrialType{1} == "identityTask") && (MWallForLoading.SigClusterSummStats(ii,:).TrialNumber{1} == 1)
%         yy(ii,1) = MWallForLoading.SigClusterSummStats(ii,:).TstatCluster;
%         idxY = idxY + 1;
%     end
% end
