...
ResponseTimesDiff = (ResponseTimes-TTLTimes)*24*60*60*1e6;
ResponseTimesAdjEm = ttl_beh+ResponseTimesDiff; %moves the time into neural time scale
...
if preSpectrogramData
   [emotionTaskLFP] = Analysis.emuEmot.nwbLFPchProc(itiDataFiltEmotion, PresentedEmotionIdx,...
       PresentedIdentityIdx, 'timeStampsImage', behavioralIndexImageOn, 'timeStampsResponse',...
       behavioralIndexResponse , 'fs', fs, 'chNum', chInterest,...
    ...
    [emotionTaskLFP] = Analysis.emuEmot.nwbLFPchProc(dataFemotion, PresentedEmotionIdx,...
        PresentedIdentityIdx, 'timeStamps', behavioralIndex, 'fs', fs, 'chNum', chInterest,...
        'preTime', preTime, 'postTime', postTime, 'multiTaperWindow', multiTaperWindow);
end
%% create and iti
...
for ii= 1:length(channelName)
    ...
    [itiDataStitch.EmotionTask.(channelName{ii})] = stats.shuffleDerivedBaseline(S1, 'fs',...
        size(identityTaskLFP.byemotion.ch97.image.specD{2}, 2)/trialLength, ...
...
ResponseTimesDiff = (ResponseTimes-TTLTimes)*24*60*60*1e6;
ResponseTimesAdjId = ttl_beh+ResponseTimesDiff; %moves the time into neural time scale
