...
randomTimeIti = true; %to make your iti a bunch of randomly selected time points so the average is essentially low.
randomTimeItiTotal = 2.5; %seconds of the iti. since it's random times, it's evenly split between before and after. IF IT'S LESS THAN THE ACTUAL EPOCH, IT CREATES AN ARTIFACT IN THE BAND PASSED.
...
if randomTimeIti
    [itiDataReal.EmotionTask.RandomTimeIti, percKeptbyChEm] = Analysis.emuEmot.nwbLFPchProc_ITIRandomTimes(itiDataFiltEmotion, 'fs', fs,...
        'chNum', chInterest, 'itiCount', 250, 'itiEpochMinus', randomTimeItiTotal/2, 'itiEpochPlus', randomTimeItiTotal/2 );
end
...
%as desired
   [identityTaskLFP, itiDataReal.IdentityTask.trueIti] = Analysis.emuEmot.nwbLFPchProc(itiDataFiltIdentity, PresentedEmotionIdxId,...
       ...
       multiTaperWindow, 'CorrectTrials', CorrectTrialsId, 'ResponseTimesAdj', ResponseTimesDiffIdentity, 'chLocationName', chLocationName);
