...
matNameEm = 'NBack_EMOTION_2022_10_20.13_20_21'; %place this in an "Emotion" folder
matNameId = 'NBack_IDENTITY_2022_10_20.13_12_18'; %place this in an "Identity" folder
if oneFile == 0
...
%For MW16 and MW18 with the hex code shenaningans
if strcmp(sessionName, 'MW_16') || strcmp(sessionName, 'MW_18')
...
if height(TableChannel) ~= TableChannel.channID(end)
    error('channel numbers do not match row numbers, check that some channels are removed for some reason AND REMEMBER TO ADJUST THE CHANNEL NAMES')
    ...
    channIDadj(62:end) = channIDadj(62:end)-3; %here i am manually removing the missing channel numbers
    channIDadj(113:end) = channIDadj(113:end)-13;
...
%chInterest = [1,2,3,4,9,10,11,12, 17:20, 29, 30, 31, 45, 46, 47, 48, 59, 60, 67, 68, 69, 70, 81:84, 93:96 ];
...
if strcmp(sessionName, ('MW_16')) || strcmp(sessionName, ('MW_18'))%unique situation with MW_16 and hex codes are just 255 and 256
    ...
    combinedHexKey(:,1) = EvKeyEm.Names;
    for jj = 1:length(combinedHexKey)
        combinedHexKey{jj,2} = EvKeyEm.Codes(jj);
        combinedHexKey{jj,3} = EvKeyEm.GoodTTLs(jj);
    end
...
%% This part is just to cut the long file if it's all one file

%   if it's all on one file, break it up. NOTE,THIS GETS VERY SPECIFIC FOR
...
if oneFile == false
    ...
    % For dealing with MW_16 and MW_18
    if strcmp(sessionName, ('MW_16')) || strcmp(sessionName, ('MW_18'))%unique situation with MW_16 and hex codes are just 255 and 256
        %will need to load the file Rex created
        %convert the hex codes to the appropriate codes
        %THERE IS A TON OF VARIATION HERE AND IT'S WORTH DOING EACH ONE BY
        %HAND.
