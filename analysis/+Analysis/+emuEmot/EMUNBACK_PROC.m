...
%run first so this just checks that the start time for the NWB is before
%the EM.

trialStartTimeEm = testfileEm.session_start_time;
trialStartTimeId = testfileId.session_start_time;
if trialStartTimeId < trialStartTimeEm
    NWBsCorrectOrder = true;
else
    NWBsCorrectOrder = false;
    warning('NWB presumed ID starts after NWB presumed EM. The NWB numbers are supposed to be in order and ID task is always first, but need to verify')
end
