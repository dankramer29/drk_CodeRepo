...
%this allows you to plot the emotion task vs identity task for a channel.
plotForManuscript = 0;
chNumLocationName = [];
if plotForManuscript
    ...
    for jj = 1:length(chNumName)
        for ii = 1:length(channelNameFinal)
            if strcmp(chNumName{jj}, channelNameFinal{ii})
                chNumLocationName{jj,1} = chLocationName{ii};
            end
        end
    end
    plt.nbackPlotSpectrogramManuscriptCompare(nbackCompareImageOn, 'timePlot', ttImage, 'frequencyRange', ff, ...
        'chName', chNumName, 'chLocationName', chNumLocationName,  'figTitleName', comparisonName)
...
if savePlot
    ...
    plt.save_plots([1:nh], 'sessionName', sessionName, 'subjName', subjName, ...
        'versionNum', 'v1RandIti_p05_NODOUBLEMEAN');
end
...
for ii = 1:length(channelName) %this records the actual times that were pulled
