...
%identity if by emotion and vice versa). Make a temp table and do rowsort
%and it will sort it in order of the other flagged variants so that it
%works like above.
TremoveTemp = Tremove;
TremoveTemp.flaggedVariant = [];
TremoveTemp.flaggedTrials = [];
TrempveTemp = sortrows(TremoveTemp);
ch = TrempveTemp.flaggedChannel;
variant = TrempveTemp.flaggedVariantOtherway;
trial = TrempveTemp.flaggedTrialOtherway;
trialAdj = 1;
for ii = 1:size(Tremove,1)
    if ii > 1
        %need to adjust the trial count back one every time you remove one
