function [ mnd1, mnd2, sd1, sd2, sigclust, centroid, tstatSum, boundingBox, allClusterNumbers, thresholds ] = cluster_permutation_Ttest_gpu3d( data1, data1iti, data2, data2iti, varargin )
%USE THIS ONE
% 
...
if splitPosNeg
    ...
    for cii = 1:size(clRPos)
        if clRPos(cii).Centroid(1,2)<HzThresholdPos %remove any that are below the 20hz threshold
            cl_aRPos(cii) = 1;
            xx(cii) = cii;
        end
    end
    ...
    for cii = 1:size(clRNeg)
        if clRNeg(cii).Centroid(1,2)<HzThresholdNeg %remove any that are below the a hz threshold
            cl_aRNeg(cii) = 1;
            xx(cii) = cii;
        end
    end
    cl_keepNeg=find(cl_aRNeg>50); %get the ones with an area >100 pixels
    idxc=1;
    ...
    for cii = 1:size(clR)
        if clR(cii).Centroid(1,2)<HzThresholdPos %remove any that are below the 20hz threshold
            cl_aR(cii) = 1;
            xx(cii) = cii;
        end
    end
    cl_keep=find(cl_aR>50); %get the ones with an area >100 pixels
    idxc=1;
