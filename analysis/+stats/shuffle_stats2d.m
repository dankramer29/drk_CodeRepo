...
[varargin, itiData1T]=util.argkeyval('itiData1', varargin, []); %if doing difference, you can include iti.
[varargin, itiData2T]=util.argkeyval('itiData2', varargin, []); %if doing difference, you can include iti.
...
if strcmp(testChoice, 'subtractedDiff') 
    if isempty(itiData1T)
        warning('iti data is empty for data 1')
        itiData1T = data2; %make the difference a comparison of differences with data 2
    end
     if isempty(itiData2T)
        warning('iti data is empty for data 2')
        itiData2T = data1; %make the difference a comparison of differences with data 1
     end 
end
...
%if data2 (iti) is not as long as the real data, add mirrored ends. 
...
for ii=1:xshuffles
    ...
    switch testChoice %do a ttest as the comparison 
        ...
        case 'subtractedDiff'
            ...
            %take the most extreme tstat (tmax) and build the distribution off of
            %that.
            thresh(ii,1) = max(abs(tstat_res(ii,tR(1):tR(2))));
    end
end
toc(ticT)
if gpuOn
...
sp=sqrt(((L1-1)*std(data1T,[],1).^2+(L2-1)*std(data2T,[],1).^2)./(L1+L2-2));
tstat_resR=(mn1-mn2)./(sp*sqrt(1/L1+1/L2));
