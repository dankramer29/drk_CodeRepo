...
for ii=1:xshuffles
    switch testChoice %do a ttest as the comparison 
        case 'ttst'
            ...
            maxTiti1(ii,1) = max(abs(tstat_res(ii,tR(1):tR(2))));
            ...
            %take the most extreme tstat (tmax) between the time threshold
            maxTiti2(ii,1) = max(abs(tstat_res(ii,tR(1):tR(2))));
           
            %% take the difference and build the histogram
...
tstat_resR=(mn1-mn1iti)./(sp*sqrt(1/L1+1/L1iti));
maxTdata1 = abs(tstat_resR(1, tR(1):tR(2))); %do this in the time range.

%again for data2
mn2=mean(data2T);
mn2iti=mean(itidata2T);
sd1=std(data2T);
sd1iti=std(itidata2T);
SEM2 = std(data2T, [], 1) / sqrt(size(data2T,1));
SEM2iti = std(itidata2T, [], 1) / sqrt(size(itidata2T,1));

sp=sqrt(((L1-1)*std(data2T,[],1).^2+(L2iti-1)*std(itidata2T,[],1).^2)./(L2+L2iti-2));
tstat_resR=(mn2-mn2iti)./(sp*sqrt(1/L2+1/L2iti));
maxTdata2 = abs(tstat_resR(1, tR(1):tR(2)));
...
thresh_binary=realDiff>thresh1tail; %this is where it counts how many shuffled points, this would be essentially 1 tailed.
...
if plt
    ...
    hold on;    
    H1 = shadedErrorBar(tt,mn1,SEM1*2,'lineprops', {'-b'});
    ax = axis;
    if sum(thresh_binary) > 0
        ...
        for ii = 1:length(startIdx)
            x_seg = tt(trueIndices(startIdx(ii)):trueIndices(endIdx(ii))); % Get x values for segment
