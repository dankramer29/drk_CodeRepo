classdef EventFeatureList < handle & Framework.FeatureList.Interface & util.Structable & util.StructableHierarchy
    ...
    methods
        function this = EventFeatureList(parent,varargin)
            this = this@Framework.FeatureList.Interface(parent);
            
            % parse user inputs
            props = properties(this);
            for kk=1:length(props)
                idx = find(strcmpi(varargin,props{kk}));
                if ~isempty(idx)
                    this.(props{kk}) = varargin{idx+1};
                end
            end
