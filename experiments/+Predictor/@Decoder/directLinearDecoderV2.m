function [Bcf,R2,cv]=directLinearDecoderV2(obj,decoderTMP,varargin)
%%
cv=[];
PopVec=[];
...
cond2 = size(x,2)<(obj.decoderParams.cvOptions.minTestSize+obj.decoderParams.cvOptions.minTrainSize) ;
% if cond1 && cond2
...
if strcmp(obj.decoderParams.filterType, 'direct')
    ...
    if exist('R2cv')
        obj.msgName(sprintf('R2cv = [%s] , R2 = [%s] : %d samples / %d Features ; FitTime (per dof) = %0.1f (secs)',R2cvstr(1:end-1),R2str(1:end-1),length(sX(1,:)),size(sZ,1), mean(fitTime)))
    else
        obj.msgName(sprintf('R2 = [%s] : %d samples / %d Features ; FitTime (per dof) = %0.1f (secs)',R2str(1:end-1),length(sX(1,:)),size(sZ,1), mean(fitTime)))
    end
elseif strcmp(obj.decoderParams.filterType, 'directInversion')
    
    % load PV
    Q=obj.trainingProps.PopVec.Q;
    Q=Q(decodeFeatures,decodeFeatures);
    
    H=obj.trainingProps.PopVec.H;
    H=H(decodeFeatures,:);
    
    [Bcf,CC]=InvertPopVec(obj,H,Q,sX,sZ);
    R2=(CC.^2)';
    
    Hcv=obj.trainingProps.PopVec.Hcv(decodeFeatures);
    for i=1:10
        for j=1:length(Hcv)
            Hcvf(j,:)=Hcv{j}(i,:);
        end
        [Bcvf{i},CC(i,:)]=InvertPopVec(obj,Hcvf,Q,sX,sZ);
    ...
    R2cv=mean(CC).^2;
    R2cvstr=[R2cvstr, sprintf('%0.2f ',R2cv)];
    R2str=[R2str, sprintf('%0.2f ',R2)];
    if exist('R2cv')
        obj.msgName(sprintf('R2cv = [%s] , R2 = [%s] : %d samples / %d Features',R2cvstr(1:end-1),R2str(1:end-1),length(sX(1,:)),size(sZ,1)))
    else
        obj.msgName(sprintf('R2 = [%s] : %d samples / %d Features',R2str(1:end-1),length(sX(1,:)),size(sZ,1)))
    end
end
