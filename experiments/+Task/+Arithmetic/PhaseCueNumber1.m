classdef PhaseCueNumber1 < handle & Experiment2.PhaseInterface & util.Structable & util.StructableHierarchy
    ...
    methods
        function this = PhaseCueNumber1(varargin)
            this = this@Experiment2.PhaseInterface(varargin{:});
        end % END function PhaseCueNumber1
        ...
        function PostDrawFcn(this,evt,hTask,varargin)
            ...
            if strcmpi(hTask.cTrialParams.nummodality,'char')
                if iscell(hTask.cTrialParams.num1Info.position)
                    pos1 = hTask.cTrialParams.num1Info.position{1};
                    pos2 = hTask.cTrialParams.num1Info.position{2};
                else
                    pos1 = hTask.cTrialParams.num1Info.position(1);
                    pos2 = hTask.cTrialParams.num1Info.position(2);
                end
            ...
            elseif strcmpi(hTask.cTrialParams.nummodality,'audio')
                if this.count<1
                    hTask.hSound.play(hTask.cTrialParams.num1Info.soundname);
...
end % END classdef PhaseCueNumber1