classdef Task < handle & Experiment2.TaskInterface & Framework.Task.Interface & util.StructableHierarchy & util.Structable
    ...
    %
    % This task is a basic delayed reach task. A rectangle will be
    % displayed on a screen, removed from the screen, and then the subject
    % will press on the screen where the rectangle was displayed. This
    % experiment will be run with a simultaneous intracranial EEG recording
    % of the subject. The intent is to analyze the neural activity occuring
    % while planning and executing a basic movement. The delay is to remove
    % noise from the visual event.
    %
    ...
    %*********************%
    properties(Constant)
        description = 'Delayed Reach Task';
    ...
    methods
        ...
        function PrefaceStartFcn(this,evt,varargin)
            ...
            % function call to Task.DelayedReach.createTrailParameters.
            % this.params.user contains the obj.user variables set in the
            % DelayedReach parameters file. And the last argument of feval
        ...
        function TrialStartFcn(this,evt,varargin)
            ...
            comment(this,sprintf('trial %d/%d (%s minutes); current time: %s; loc: %d, color: %s',...
