classdef PhaseRespondGNG < handle & Experiment2.PhaseInterface & util.Structable & util.StructableHierarchy
    ...
    methods
        function this = PhaseRespondGNG(varargin)
            this = this@Experiment2.PhaseInterface(varargin{:});
        end % END function PhaseRespondGNG
        ...
        function PreDrawFcn(this,evt,hTask,varargin)
            %Red or Green from cTrialParams gng
        ...
        function PhaseFcn(this,evt,hTask,varargin)
            ...
            if ~isempty(hTask.hTrial.TrialData.tr_responseLoc) && hTask.cTrialParams.gng
            ...
            elseif ~isempty(hTask.hTrial.TrialData.tr_responseLoc) && ~hTask.cTrialParams.gng
                hTask.hTrial.TrialData.calculateSuccessGNG(hTask);
                exstr = 'FAILURE';
                comment(hTask,sprintf('Response provided on No-Go trial. Recorded (%s)',exstr));
                hTask.hTrial.advance;
            end
        ...
        function TimeoutFcn(this,evt,hTask,varargin)
            ...
            if hTask.cTrialParams.gng
                hTask.hTrial.TrialData.calculateSuccessGNG(hTask);
                exstr = 'FAILURE';
                comment(hTask,sprintf('No response provided on Go trial. Recorded (%s)',exstr));
            else
                hTask.hTrial.TrialData.calculateSuccessImagined(hTask);
                exstr = 'SUCCESS';
                comment(hTask,sprintf('No response provided on No-Go trial. Recorded (%s)',exstr));
            end
            hTask.hTrial.advance;
...
end % END classdef PhaseRespondGNG