function DefaultSettings_GNG(obj)
...
% trial data
obj.trialDataConstructor = @Task.DirectReachGNG.TrialData;

% trial params
obj.trialParamsFcn = @Task.DirectReachGNG.createTrialParams;
...
obj.prefaceDefinitions{pidx} = {@Task.Common.PrefaceTitle,...
    'Name','Title',...
    'titleString','Direct Reach Go-No-Go Task',...
...
obj.phaseDefinitions{1} = {@Task.Common.PhaseITI,...
    ...
    'durationTimeout',@(x)1};
obj.phaseDefinitions{2} = {@Task.DirectReachGNG.PhaseFixate,...
    'Name', 'Fixate',...
    'drawFixationPoint',true,...
    'durationTimeout',@(x)2+2*rand};
 obj.phaseDefinitions{3} = {@Task.DirectReachGNG.PhaseCueRespondGNG,...
     'Name', 'CueRespondGNG',...
     'durationTimeout', 4.0};
