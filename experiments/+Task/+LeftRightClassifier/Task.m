classdef Task < handle & Experiment2.TaskInterface & Framework.Task.Interface & util.StructableHierarchy & util.Structable
    ...
    %
    % This task is intended to classify neural activity associated with
    % left/right movement intention.
    
    %*********************%
    % CONSTANT PROPERTIES %
    %*********************%
    properties(Constant)
        description = 'Lelft/Rright Classifier task';
    ...
    methods
        ...
        function TrialStartFcn(this,evt,varargin)
            ...
            cue_block = this.cTrialParams.cue_block;
            switch lower(this.cTrialParams.response_modality)
                case 'text'
                    cue_word = sprintf('*%s',cue_word);
                case 'block'
                    cue_block = sprintf('*%s',cue_block);
                otherwise
                    error('Unknown response modality "%s"',this.cTrialParams.response_modality);
            end
            
            % print out to the screen
            comment(this,sprintf('trial %d/%d (%s minutes); current time: %s; block name: %s, block position: %s, response: %s',...
        ...
        function drawFixationPoint(this)
            ...
            fix_block = [1 1 1];
            this.hDisplayClient.drawOval(pos,diam,fix_block*user.fixationBrightness)
