function Parameters_StimParamSweep_Duration(obj)
% This parameter set sweeps across all durations
...
obj.user.stimDurations = [100/1000 500/1000]; % duration of pulse train in sec
...
obj.user.numTrialsPerBalanceCondition = obj.user.numBlockRep*numel(obj.user.electrodeLabels)*numel(obj.user.stimDurations); % number of trials for each balance condition
obj.user.conditionsToBalance = {'stimFrequencies','stimAmplitudes','stimPolarities','stimPulses','stimPulseWidths',...
    'stimInterphases'}; % fields that contain values to be balanced across trials
obj.user.conditionsToDistribute = {'stimFrequencies','stimAmplitudes','stimPolarities','stimPulses','stimPulseWidths',...
    'stimInterphases'}; % fields that contain values to be distributed
...
[obj.user.freqID,obj.user.amplID,obj.user.polID,obj.user.pwID,obj.user.iphID] = Task.Common.balanceTrials(obj.user.whetherToBalance,numValuesPerCondition,obj.user.numTrialsPerBalanceCondition,obj.user.allowedEqualIDs);
obj.user.numParam = numel(obj.user.stimDurations);
durInd = 1:obj.user.numParam;
obj.user.durID = repmat(reshape(repmat(durInd, numel(obj.user.electrodeLabels),1),[],1),obj.user.numBlockRep,1);
