function Parameters_StimParamSweep_Pulsewidth(obj)
% This parameter set sweeps across all pulsewidths
...
obj.user.stimPulseWidths = [160 260 360 460 560 660]; % width of the first and second phase of stimulation in usec
...
obj.user.numTrialsPerBalanceCondition = obj.user.numBlockRep*numel(obj.user.electrodeLabels)*numel(obj.user.stimPulseWidths); % number of trials for each balance condition
obj.user.conditionsToBalance = {'stimFrequencies','stimAmplitudes','stimPolarities','stimPulses',...
    'stimInterphases','stimDurations'}; % fields that contain values to be balanced across trials
obj.user.conditionsToDistribute = {'stimFrequencies','stimAmplitudes','stimPolarities','stimPulses',...
...
[obj.user.freqID,obj.user.amplID,obj.user.polID,obj.user.iphID,obj.user.durID] = Task.Common.balanceTrials(obj.user.whetherToBalance,numValuesPerCondition,obj.user.numTrialsPerBalanceCondition,obj.user.allowedEqualIDs);
obj.user.numParam = numel(obj.user.stimPulseWidths);
durInd = 1:obj.user.numParam;
obj.user.pwID = repmat(reshape(repmat(durInd, numel(obj.user.electrodeLabels),1),[],1),obj.user.numBlockRep,1);
