function Parameters_AttendColor_Mixed_FastResponse(obj)
...
Task.Stroop.DefaultSettings_FastResponse(obj);
...
obj.user.cue_congruency = {'congruent','incongruent'};

% control number of trials (one entry for each balance-condition option)
obj.user.conditionsToBalance = {'cue_colors','cue_congruency'}';
obj.user.numTrialsPerBalanceCondition = 4; % number of trials for each balance condition

% set up pairwise checks on equal condition IDs (prevent same words/colors)
% process is to generate trials with unique combinations, then overwrite
% based on requested congruency or not
obj.user.allowedEqualIDs(strcmpi(obj.user.conditionsToDistribute,'cue_words'),strcmpi(obj.user.conditionsToDistribute,'cue_colors')) = false;
obj.user.allowedEqualIDs(strcmpi(obj.user.conditionsToDistribute,'cue_colors'),strcmpi(obj.user.conditionsToDistribute,'cue_words')) = false;
