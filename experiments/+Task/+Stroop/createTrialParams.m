...
warning('below code assumes cue_words and cue_colors are the same list!');
for kk=1:numTrialsTotal
    
    % identify the correct response
    % force congruent if necessary
    % (this means that it would be difficult to balance colors and words
    ...
    switch lower(user.response_modality{rmodID(kk)})
        ...
        case 'color'
            ...
            ansID(kk) = clrID(kk);
            answer{kk} = user.cue_colors{ansID(kk)};
        otherwise
            error('Unknown response modality "%s"',user.response_modality{rmodID(kk)});
    end
    
    % identify the rgb values to use for the color
    idx = strcmpi(user.color_names,user.cue_colors{clrID(kk)});
    assert(nnz(idx)==1,'Could not identify single match for color "%s"',user.cue_colors{clrID(kk)});
    rgb{kk} = user.color_rgb{idx};
end

% create array of structs (cell arrays args dealt across array)
params = struct(...
    'cue_word',user.cue_words(wrdID(:)'),...
    'cue_color',user.cue_colors(clrID(:)'),...
    'cue_rgb',rgb(:)',...
