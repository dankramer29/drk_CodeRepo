classdef EDFData < handle
    ...
    methods
        function obj = EDFData(fileName)
            ...
            unknownbaseScale = false(1,length(obj.units));
            for ii = 1: length(obj.units)
                switch lower(deblank(obj.units{ii}))
                    ...
                    case {'s' 'seconds' 'second'}
                        obj.baseScale(ii) = 1;
                    otherwise
                        unknownbaseScale(ii) = true;
                        obj.baseScale(ii) = 1;
                end
            end
            if any(unknownbaseScale)
                ch_str = arrayfun(@(x)sprintf('%d',x),find(unknownbaseScale),'UniformOutput',false);
                ch_str = sprintf('[%s]',strjoin(ch_str,' '));
                fprintf('Method could not determine the units of the data for channels %s, setting unknown conversion factors to 1.\n',ch_str);
            end
