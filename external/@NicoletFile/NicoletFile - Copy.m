classdef NicoletFile < handle
    ...
    methods
        function obj = NicoletFile(filename)
            ...
            while curIdx < obj.Qi.nrEntries
                ...
                for i = 1: nrIdx
                    ...
                    Index(curIdx + i).blockL = mod(var(3*(i-1)+3),2^32); % block length is 3rd 64-bit value, mod 2^32
                    Index(curIdx + i).sectionL = round(var(3*(i-1)+3)/2^32); % secton length is 3rd 64-bit value, divided by 2^32 and rounded
                    % offset is the byte position in the file
                    % sectionL indicates number of bytes in the section
                    % what is blockL?
                end
            ...
            SIG_struct.name = deblank(fread(h, obj.ITEMNAMESIZE, '*char')');
            unkown = deblank(fread(h, 152, '*char'));         %#ok<NASGU>
            fseek(h, 512, 'cof');
            ...
            CH_struct.name = deblank(fread(h, obj.ITEMNAMESIZE, '*char')');
            fseek(h, 152, 'cof');
            ...
            for iSeg = 1:length(obj.segments)
                ...
                for i = 1:elems
                    ...
                    offset = offset + 552;
                    %disp([num2str(i) ' : ' obj.tsInfo(i).label ' : ' obj.tsInfo(i).activeSensor ' : ' obj.tsInfo(i).refSensor ' : ' num2str(obj.tsInfo(i).dSamplingRate)]);
            ...
            while (pktGUID == evtPktGUID)
                ...
                obj.eventMarkers(i).label         = evtLabel;
        ...
        function out = getdata(obj, segment, range, chIdx)
            ...
            %   and CHIDX is a vector of channel indices.
