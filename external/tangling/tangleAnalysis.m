...
%     in out.allCombos.
%     - out.v: the fractional variance of the pc
%  
%
% USAGE EXAMPLES
...
A_masked = A(myMask,:); %seems as though this does want to take all of the data and the above lines are just to make sure times and conditions that aren't being used are excluded

% reduce to numPCs or however many PCs capture fracVar of the variance
[PCs, score, v, ~, expVar] = pca(A_masked); %the third variable is the eigen values of the variance, and the 5th is the explained percentage of the variance.
V = cumsum(v)./sum(v);
if ~isempty(fracVar)
...
out.q_allPairs = q;
%drk adding this
out.v = v; %add the variance
out.expVar = expVar; %actual percent variance per unit
out.score = score; %this is the projection in actual PCA subspace
