...
class XLTEK_Converter:
    def __init__(self, subj, xltek_dir, subs_dir=os.getcwd(),  reader=None):
        ## Attributes ##
        self.subj = subj
        ...
        if reader is None:
            self.reader = xltek_reader.XLTEK_Reader(subj, xltek_dir)
        else:
            self.reader = reader
        self.day_dirs = []
        self.h5_fobj = None
    ...
    def start_convert(self, multiprocess=True):
...
if __name__ == "__main__":
    # Build Parser
    parser = argparse.ArgumentParser(description='A Converter that converts XLTEK studies to H5 files.')
    parser.add_argument('subj', help='The subject ID that the patient will have.')
    parser.add_argument('xltek_ID', help="The XLTEK ID of the patient's first study.")
    parser.add_argument('xltek_dir', help='The directory of the XLTEK patient folders.')
    parser.add_argument('subs_dir', help='The directory to place the converted patient files.')

    # Parse Commandline Arguments
    args = parser.parse_args(sys.argv[1:])

    # Run Conversion
    patient = xltek_reader.XLTEK_Reader(args.subj, args.xltek_ID, args.xltek_dir)
    converter = XLTEK_Converter(args.subj, args.xltek_dir, args.subs_dir, patient)

    converter.start_convert(multiprocess=False)
    converter.reader.shutdown()
