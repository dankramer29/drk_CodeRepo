...
from pyxlrd_win64 import libpyxlrd
...
class XLTEK_Reader:
    ...
    def __init__(self, subj, study_ID, xltek_dir=os.getcwd(), manager=None):
        ## Proxy Manager Setup ##
        ...
        self.xltek_dir = pathlib.Path(xltek_dir)
    ...
    def get_neuro_dirs(self):
        ...
        for location in self.network_dir.iterdir():
            dir_name = str(os.path.split(location)[-1])
            str_start = dir_name.find('(')
            if str_start > 0:
                server_name = dir_name[str_start + 1:-1]
                folder_name = dir_name[:str_start - 1]
                neuroworks_dir = '//' + server_name + '/' + folder_name
                try:
                    sub_dir = pathlib.Path(neuroworks_dir, 'DbData')
                    if sub_dir.is_dir():
                        os.path.getatime(str(sub_dir))
                        self.neuroworks_dirs.append(sub_dir)
                except:
                    print('Cannot access %s skipping.' % (neuroworks_dir))
    ...
    def get_subj_name(self, study_ID):
        ...
        study_dirs = self.xltek_dir.glob(('*'+study_ID))
    ...
    def get_studies(self):
        ...
        study_glob = self.xltek_dir.glob((self._name_key + '*'))
        self._all_study_dirs |= {(os.path.split(x)[-1], x) for x in study_glob}
    ...
    def create_studies(self, study_dirs, current):
        ...
        if len(valid_studies) > 1:
            processors = len(valid_studies)-1 if len(valid_studies)<5 else 4
    ...
    def update_study_dirs(self):
        ...
        study_dirs |= {(os.path.split(x)[-1],x) for x in study_glob}
    ...
    def stop_updater(self):
        ...
        '''
        if self.studies_process is not None and self.studies_process.is_alive():
    ...
    def get_last_data(self, entries=1, t2d=True, buffer=None, interrupt=multiprocessing.Event()):
        ...
        if entries <= self.last_etc_size:
            ...
            if buffer is not None:
                ...
                with NormalPool(processes=2) as pool:
                    results = []
                    first = ranges.pop(-1)
                    for i in range(index, self.etc_total):
...
class ErdFile(object):
    ...
    def read_entries(self, entries, t2d=True, buffer=None, interrupt=multiprocessing.Event(), limit=600):
            ...
            with multiprocessing.Pool(processes=pro) as pool:
                ...
                for res in results:
                    ...
                    if interrupt.is_set():
                        ...
                        break
                    if t2d:
                        buffer.append(self.toc2dict(res.get(timeout=10)))
                    else:
                        buffer.append(res.get(timeout=10))
        else:
            for num in entries:
                te = self.etc.etc_data[num]
                while (len(buffer) >= limit) and not interrupt.is_set():
