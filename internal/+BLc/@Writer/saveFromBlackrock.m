function files = saveFromBlackrock(this,varargin)
% SAVEFROMBLACKROCK Create a BLc file from a BLACKROCK source
%
%   FILES = SAVEFROMBLACKROCK(THIS)
%   Use the source BLACKROCK object to provide data and properties to
%   create one or more binary BLC files. The BLC file(s) will be created in
%   the same directory, and with the same basename (but with a zero-based
...
assert(~isempty(regexpi(class(this.hSource),'Blackrock.NSx')),'Must provide a valid object of class "Blackrock.NSx", not "%s"',class(this.hSource));

% compute number of output files and time in each
numPointsInNSx = sum(this.hSource.PointsPerDataPacket);
...
currFile = 1;
numFramesLeftInFile = numPointsInNSx;
...
try
    data = this.hSource.read('packets',1,'points',[1 1],'ref','packet');
catch ME
    util.errorMessage(ME);
    keyboard
end
idxUserToSource = ismember([this.hSource.ChannelInfo.ChannelID],this.indexChannelToWrite{1});
...
%globalNumFramesToRead = numPointsInNSx;
currPacket = 1;
currDataPointInPacket = 1;
if flagSave
    %hWaitbar = waitbar(0/numPointsInNSx,sprintf('Writing data to %s (%d/%d)',outbase,1,numOutputFiles));
...
for ff=1:numOutputFiles
    ...
    packets(100) = struct('nsxPacket',nan,'startPoint',nan,'endPoint',nan);
    packets(1).nsxPacket = currPacket;
    ...
    while numPointsRead < numPointsToRead
        ...
        if numPointsToReadFromPacket==0
            ...
            % update packet data
            packets(numDataSections).nsxPacket = currPacket;
    ...
    for kk=1:numDataSections
        ...
        if packets(kk).nsxPacket>1
            pointsBeforePacket = sum(this.hSource.PointsPerDataPacket(1:packets(kk).nsxPacket-1));
        end
        ...
        secondsBeforeThisPacket = this.hSource.Timestamps(packets(kk).nsxPacket)/this.hSource.Fs;
        ...
        try
            while numToRead>0
                ...
                data = this.hSource.read('packet',packets(kk).nsxPacket,'points',[currStart currStart+numToRead-1],'ref','packet','microvolts','channels',this.indexChannelToWrite{1});
