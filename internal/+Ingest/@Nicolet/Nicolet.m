classdef Nicolet < handle
    properties
        hDebug % handle to Debug.Debugger object
        hGridMap % handle to grid map object
        sourceFiles % full path to existing *.e file to be ingested
    ...
    methods
        function this = Nicolet(varargin)
            ...
            if ~found_debug,this.hDebug=Debug.Debugger('Ingest.Nicolet');end
            util.argempty(varargin);
            
            % data dir is almost always the same
            setNeuralDirectory(this,'\\STRIATUM\Data\neural');
        end % END function Nicolet
        
        function addFile(this,file,varargin)
            % ADDFILE Add a file to be converted from Nicolet to BLc
            %
            %  inputs
            %  MODE:    'framework-experiment' or 'patient-date'
            %           use framework-style filenames (i.e., don't rename)
            %           or rename to patient/date string
            [varargin,mode] = util.argkeyval('mode',varargin,'patient-date'); % 'framework-experiment' or 'patient-date'
            util.argempty(varargin);
            
            % validate input - full path to valid Nicolet data file
            if isa(file,'Natus.NicoletEFile')
            ...
            assert(strcmpi(srcext,'.e'),'Must provide valid Nicolet data file (extension *.e, not %s)',srcext);
        ...
        function autoIdentify(this)
            ...
            try
                ...
                [numHits,idxMatch] = max(hits);
                assert(nnz(hits==numHits)==1,'Could not match a unique token');
            ...
            assert(nnz(idx)==1,'Could not identify a token with YYYYMMDD_HHMMSS format');
        ...
        function exitCode = convertToBLc(this)
            % CONVERTTOBLC Convert source files from Nicolet to BLc
            exitCode = 0; % success
            ...
            for kk=1:size(this.sourceFiles,1)
                ...
                [~,outputFiles] = BLc.convert.e2blc('srcfile',fullfile(srcdir,srcfile),...
        ...
        function archiveSourceData(this,varargin)
            ...
            for ss=1:size(this.sourceFiles,1)
                for kk=1:length(this.sourceFiles.output_file{ss})
                    ...
                    task_archive = fullfile(archive_dir,'task');
