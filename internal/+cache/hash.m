function sh = hash(Data, Opt)
% HASH - Checksum for Matlab array of any type
...
if isFile
   ...
   % Calculate the hash:
   if usetypecastx
      sh = typecastx(Engine.digest, 'uint8');
   else
      sh = typecast(Engine.digest, 'uint8');
   end
...
switch OutFormat
   case 'hex'
      sh = sprintf('%.2x', double(sh));
   case 'HEX'
      sh = sprintf('%.2X', double(sh));
   case 'double'
      sh = double(reshape(sh, 1, []));
   case 'uint8'
      sh = reshape(sh, 1, []);
   case 'base64'
      sh = fBase64_enc(double(sh));
   otherwise
