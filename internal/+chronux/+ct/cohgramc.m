...
f=chronux.hlp.getfgrid(Fs,nfft,fpass); 
Nf=length(f);
...
for n=1:nw;
   ...
   if nargout==10;
     [c,ph,s12,s1,s2,f,confc,phie,cerr]=chronux.ct.coherencyc(datawin1,datawin2,params);
...
%      phierr(2,n,:,:)=squeeze(phie(2,:,:));
   ...
   elseif nargout==9;
     [c,ph,s12,s1,s2,f,confc,phie]=chronux.ct.coherencyc(datawin1,datawin2,params);
%      phierr(1,n,:,:)=squeeze(phie(1,:,:));
%      phierr(2,n,:,:)=squeeze(phie(2,:,:));
      phistd(n,:,:)=phie;
   else
     [c,ph,s12,s1,s2,f]=chronux.ct.coherencyc(datawin1,datawin2,params);
   end;
   C(n,:,:)=c;
   S12(n,:,:)=s12;
   S1(n,:,:)=s1;
