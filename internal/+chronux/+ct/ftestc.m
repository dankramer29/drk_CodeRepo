...
tapers=chronux.hlp.dpsschk(tapers,N,Fs); % calculate the tapers
[N,K]=size(tapers);
nfft=max(2^(nextpow2(N)+pad),N);% number of points in fft
[f,findx]=chronux.hlp.getfgrid(Fs,nfft,fpass);% frequency grid to be returned
...
Kodd=1:2:K;
Keven=2:2:K;
J=chronux.ct.mtfftc(data,tapers,nfft,Fs);% tapered fft of data - f x K x C
...
if nargout==0 || strcmp(plt,'y');
   [S,f]=chronux.ct.mtspectrumc(detrend(data),params);subplot(211); plot(f,10*log10(S));xlabel('frequency Hz'); ylabel('Spectrum dB');
