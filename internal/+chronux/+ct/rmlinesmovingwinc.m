...
[tapers,pad,Fs,fpass,err,trialave,params]=chronux.hlp.getparams(params); % set defaults for params
clear err trialave
...
for n=1:nw;
   ...
   datawin=data(indx,:);
   [datafitwin,as,fs]=chronux.ct.fitlinesc(datawin,params,p,'n',f0);
   Amps{n}=as;
   freqs{n}=fs;
...
datac=[datac; data((size(datac,1)+1):end,:)]; % ADDED skellis 20170131 to preserve size
if strcmp(plt,'y');
    [S,f]=chronux.ct.mtspectrumsegc(data,movingwin(1),params);
    [Sc,fc]=chronux.ct.mtspectrumsegc(datac,movingwin(1),params);
