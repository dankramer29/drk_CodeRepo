...
%      end;
     ...
     for ch2=1:ch1-1;
          ...
          if errtype==2 
             if nargin<3;
                 [conf,phie,Ce]=chronux.hlp.coherr(abs(C12),J1,J2,err,trialave);
             else
                 [conf,phie,Ce]=chronux.hlp.coherr(abs(C12),J1,J2,err,trialave,Nsp(ch1),Nsp(ch2));
             end
             confC(ch1,ch2)=conf; 
          ...
          elseif errtype==1
             if nargin<3;
                 [conf,phie]=chronux.hlp.coherr(abs(C12),J1,J2,err,trialave);
             else
                 [conf,phie]=chronux.hlp.coherr(abs(C12),J1,J2,err,trialave,Nsp(ch1),Nsp(ch2));
             end
             confC(ch1,ch2)=conf; 
             phierr(1,:,ch1,ch2)=phie;phierr(2,:,ch1,ch2)=phie;
             confC(ch2,ch1)=conf; 
