...
[N,Ch]=chronux.hlp.check_consistency(data1,data2,1);

Nwin=round(Fs*movingwin(1)); % number of samples in window
Nstep=round(movingwin(2)*Fs); % number of samples to step through
nfft=max(2^(nextpow2(Nwin)+pad),Nwin);
f=chronux.hlp.getfgrid(Fs,nfft,fpass);  Nf=length(f);
params.tapers=chronux.hlp.dpsschk(tapers,Nwin,Fs); % check tapers
...
for n=1:nw;
   ...
   t=indx/Fs;
   datawin1=data1(indx,:);datawin2=chronux.pt.extractdatapt(data2,[indx(1)/Fs indx(end)/Fs]);
   if nargout==11;
     [c,ph,s12,s1,s2,f,zsp,confc,phie,cerr]=chronux.hyb.coherencycpt(datawin1,datawin2,params,fscorr,t);
...
%      phierr(2,n,:,:)=squeeze(phie(2,:,:));
   ...
   elseif nargout==10;
     [c,ph,s12,s1,s2,f,zsp,confc,phie]=chronux.hyb.coherencycpt(datawin1,datawin2,params,fscorr,t); 
%      phierr(1,n,:,:)=squeeze(phie(1,:,:));
%      phierr(2,n,:,:)=squeeze(phie(2,:,:));
     phistd(n,:,:)=phie;
   else
     [c,ph,s12,s1,s2,f,zsp]=chronux.hyb.coherencycpt(datawin1,datawin2,params,fscorr,t);
   end;
   C(n,:,:)=c;
   phi(n,:,:)=ph;
   S12(n,:,:)=s12;
