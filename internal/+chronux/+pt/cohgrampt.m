...
[N,Ch]=chronux.hlp.check_consistency(data1,data2);
[mintime1,maxtime1]=chronux.pt.minmaxsptimes(data1);
[mintime2,maxtime2]=chronux.pt.minmaxsptimes(data2);
...
f=chronux.hlp.getfgrid(Fs,nfft,fpass); Nf=length(f);
...
for n=1:nw;
   ...
   datawin1=chronux.pt.extractdatapt(data1,[t(1) t(end)]);datawin2=chronux.pt.extractdatapt(data2,[t(1) t(end)]);
   if nargout==11;
     [c,ph,s12,s1,s2,f,zsp,confc,phie,cerr]=chronux.pt.coherencypt(datawin1,datawin2,params,fscorr,t);
...
%      phierr(2,n,:,:)=squeeze(phie(2,:,:));
   ...
   elseif nargout==10;
     [c,ph,s12,s1,s2,f,zsp,confc,phie]=chronux.pt.coherencypt(datawin1,datawin2,params,fscorr,t);
%      phierr(1,n,:,:)=squeeze(phie(1,:,:));
%      phierr(2,n,:,:)=squeeze(phie(2,:,:));
     phistd(n,:,:)=phie;
   else
     [c,ph,s12,s1,s2,f,zsp]=chronux.pt.coherencypt(datawin1,datawin2,params,fscorr,t);
   end;
   C(n,:,:)=c;
   phi(n,:,:)=ph;
   S12(n,:,:)=s12;
