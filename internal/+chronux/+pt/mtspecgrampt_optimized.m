...
for n=1:nw;
    ...
    datawin=chronux.pt.extractdatapt(data,[t(1) t(end)]);
    datawin=chronux.hlp.change_row_to_column(datawin);
    ...
    if nargout==5;
        if fscorr==1;
            serr=chronux.hlp.specerr(S,J,err,trialave,Nsp);
        else
            serr=chronux.hlp.specerr(S,J,err,trialave);
        end
