...
dtmp=chronux.hlp.change_row_to_column(data);
...
[mintime,maxtime]=chronux.pt.minmaxsptimes(dtmp);
...
%     varS=var(lS,1,2); % variance of log spectrum
    ...
    if nargout > 4
       ...
       if nargout > 5
          ...
          if nargout==7; 
             if fscorr==1;
                Serr=chronux.hlp.specerr(SS,J,err,segave,Nsp);
             else
                Serr=chronux.hlp.specerr(SS,J,err,segave);
             end;
          end;
       end;
    end;
