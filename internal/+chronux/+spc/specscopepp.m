...
if not(exist('chronux.ct.mtspecgramc','file'));
...
while 1;
    ...
    if acq.deriv
      [s, t, f] = chronux.ct.mtspecgramc(abs(diff(data)), acq.moving_window, acq.params );
    else
      [s, t, f] = chronux.ct.mtspecgramc(diff(data), acq.moving_window, acq.params );
    end
...
function defaults()
  ...
  acq.params.tapers=chronux.hlp.dpsschk(acq.params.raw_tapers,round(acq.params.Fs*acq.moving_window(1)),acq.params.Fs);
  acq.offset = 0;
  acq.scale = 500;
  acq.display_size = 3;
