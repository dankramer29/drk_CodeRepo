function Serr=specerr(S,J,err,trialave,numsp,gpuflag)
...
if nargin<5 || (nargin>=5 && isempty(numsp)); numspflag=false; else numspflag=true; end;
if nargin < 6; gpuflag=false; end
...
if trialave
    ...
    C=1;
    dof=2*dim;
    if numspflag; dof = fix(1/(1/dof + 1/(2*sum(numsp)))); end
    J=reshape(J,nf,dim);
else
    dim=K;
    dof=2*dim*ones(1,C);
    for ch=1:C;
        if numspflag; dof(ch) = fix(1/(1/dof + 1/(2*numsp(ch)))); end
    end;
end;
Serr=zeros([2,nf,C],'gpuArray');
if errchk==1;
    Qp=chi2inv(pp,dof);
    Qq=chi2inv(qq,dof);
    
    Serr(1,:,:)=dof(ones([nf,1]),:).*gather(S)./Qp(ones([nf,1]),:);
    Serr(2,:,:)=dof(ones([nf,1]),:).*gather(S)./Qq(ones([nf,1]),:);
elseif errchk==2;
    tcrit=gpuArray(tinv(pp,dim-1));
    for k=1:dim;
...
Serr=squeeze(Serr);
if ~gpuflag; Serr=gather(Serr); end
