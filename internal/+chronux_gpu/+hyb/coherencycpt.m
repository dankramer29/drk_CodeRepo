...
[N,Ch]=chronux_gpu.hlp.check_consistency(data1,data2,1);
...
data1 = cast(data1, 'single');  % change the type of data1 into single, as using single is more efficient with GPU
J1=chronux_gpu.ct.mtfftc(data1,tapers,nfft,Fs, true); % fourier transform of continuous data with GPU
...
J2 = cast(J2, 'single');
err = cast(err, 'single');
Nsp2 = cast(Nsp2, 'single');
if nargout==10;
    if fscorr==1;
        [confC,phistd,Cerr]=chronux_gpu.hlp.coherr(C,J1,J2,err,trialave,[],Nsp2);
    else
        [confC,phistd,Cerr]=chronux_gpu.hlp.coherr(C,J1,J2,err,trialave);
    end;
elseif nargout==9;
    if fscorr==1;
        [confC,phistd]=chronux_gpu.hlp.coherr(C,J1,J2,err,trialave,[],Nsp2);
    else
