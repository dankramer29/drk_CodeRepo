classdef Structable < handle
    ...
    %   customize some aspects of the struct conversion process by
    %   overloading the TOSTRUCT method.  Manually overloading TOSTRUCT is
    %   unnecessary unless there are properties to manually process.
    %
    %     classdef MyClass < Structable
    %       properties
    ...
    %     >> st = obj.toStruct;
    %
    %   See also STRUCTABLE/TOSTRUCT and STRUCTABLEHIERARCHY.
    ...
    methods
        function st = toStruct(this,varargin)
            ...
            if isa(this,'util.StructableHierarchy')
    ...
    methods(Access=private)
                    ...
                    for kk=1:length(propNames)
                        ...
                        if iscell(this.(propNames{kk}))
                            ...
                            if isempty(this.(propNames{kk}))
                                ...
                                st.(propNames{kk}) = cellfun(@(x,y)convertSingleProperty(this,x,propNames{kk},className),this.(propNames{kk}),'UniformOutput',false);
                            end
        ...
        function val = convertSingleProperty(this,prop,varargin)
            ...
            if isobject(prop)
                ...
                % run the object
                val = subprop(this,prop,info,varargin{:});
            else
        ...
        function val = subprop(this,prop,info,varargin)
            ...
            elseif any(strcmpi(methodList,'toStruct'))
                ...
                % look for x2struct method
                builtinToStructMethod = sprintf('%s2struct',class(prop));
                if exist(builtinToStructMethod,'file')==2
                    try
                        val = feval(builtinToStructMethod,prop);
                    catch ME
                        msg = util.errorMessage(ME,'noscreen','nolink');
                        warning('Structuable cannot process ''%s'' (an instance of ''%s'') for class ''%s'': %s\n',name,class(prop),className,msg);
                        val = className;
                    end
                else
