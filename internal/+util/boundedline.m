...
[found, linewidth, varargin] = parseparam(varargin, 'linewidth');

if ~found
    linewidth = 1;
end

if ~isscalar(linewidth) || linewidth < 0
    error('Line width must be positive scalar');
end
...
% Axis -- 
% SKELLIS 9/21/2015 MOVED
% Ran into a strange corner case where the handle was a numerical value
% which happened to be the same value as the transparency (0.5) - the code
% below pulled out both the transparence and the axis handle thinking they
% were both handles, and caused an error when the 'transparency' arg
% didn't have a corresponding value associated.
...
[xl, yl, xp, yp, marker, lnsty, lncol, ptchcol, alpha, lnwdth] = deal(cell(nline,1));
...
for iln = 1:nline
    hl(iln) = line(xl{iln}, yl{iln}, 'marker', marker{iln}, 'linestyle', lnsty{iln}, 'color', lncol{iln}, 'linewidth', lnwdth{iln});
end
