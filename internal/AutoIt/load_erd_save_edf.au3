...
If Not $myErdFile Then
   $myErdFile = "Z:\neural\incoming\unsorted\keck\Jung Francesca_7115_Ph2D7\JUNG~ FRANCESC_c7b1a5b4-afb4-45a4-939f-52e3a37ab633.erd"
EndIf

;Create hard link to file on serv

If Not $myOutputDirectory Then
   $myOutputDirectory = "Z:\neural\working\persyst_export_test\"
EndIf
...
;=============================================================================
; _CreateEdfFile
; save out an .edf file from an .erd file
;=============================================================================
Func _CreateEdfFile($basename)
	$fcnstr = "(_CreateEdfsFile) "
...
;~ 	_LogMessage($fcnstr & "Event file is " & $myEventFile & " in directory " & $myEventDir)
	...
	ControlClick($ErdWindowTitle, "", "[CLASS:XTPToolBar; INSTANCE:2]", "left", 1, 40,590 )

	;open clip/export window
	MouseClick($MOUSE_CLICK_LEFT, 175, 680, 1)
	Sleep(1000)
	...
	;Activate clip exportwindow and save out .edf
	_ActivateWindow('Clip/Export: Choose the Output File')

    ;change output directory
	ControlClick('Clip/Export: Choose the Output File', "", "[CLASS:ToolbarWindow32; INSTANCE:4]", "left", 1, 550, 10)
	Send($myOutputDirectory)
	Sleep(3000)

	;change output filename
	ControlClick('Clip/Export: Choose the Output File', "", "[CLASS:Edit; INSTANCE:1]", "left", 1, 775, 10)
	Sleep(100)

	;send output filename for
	$identifier='autoit_'
	Send($identifier & $myErdBasename & '.edf')
	Sleep(1000)
	...
	If  StringLen($myOutputDirectory & $myErdBasename) >= 64 Then
	   ...
	   Sleep(5000)
    EndIf
	If  FileExists($myOutputDirectory & '\' & $identifier & $myErdBasename & '.edf') Then
	   Sleep(1000)
	   ;_ActivateWindow('Archive')
    ...
    Else
	   Sleep(3000)
		...
		If WinExists($clipWindow) Then
		   WinClose($clipWindow)
	    EndIf
    EndIf
EndFunc ; _CreateEdfFile