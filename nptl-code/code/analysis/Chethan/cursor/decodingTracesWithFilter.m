function [hbias, vbias] = testBlockWithFilter(filter, R, Rprev)

%% make the T struct from the filter options

T = makeTWithFilterOptions(R, filter);

%% if Rprev is passed in, adapt filter baselines using Rprev
if exist('Rprev','var')
    Tprev = makeTWithFilterOptions(Rprev, filter);
    %[Tprev] = onlineTfromR(Rprev, Toptions);
    newBase = mean((bsxfun(@times,[Tprev.Z],filter.model.invSoftNormVals))');
    filter.model.C(:,5) = newBase;
end

Nkeep = ceil(600 / filter.options.binSize);
NkeepTrace = ceil(900 / filter.options.binSize);
...
for nd = 1:length(Rsm)
    ...
    for it = 1:length(Rsm(nd).R)
        ...
        io = find([out.trialNum]==nt);
        if ~isempty(io) && numel(out(io).xk(:));
            keepLen = min(NkeepTrace, size(out(io).xk,2));
            x = out(io).xk(1,1:keepLen);
            y = out(io).xk(2,1:keepLen);
