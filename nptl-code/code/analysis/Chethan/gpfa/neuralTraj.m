...
%        John Cunningham  jcunnin@stanford.edu
  ...
  outputDir     = '/net/home/chethan/localdata/gpfa/mat_results/';
  for nn = 1:2:length(extraOpts)
      if strcmp(extraOpts{nn},'outputDir')
          outputDir = extraOpts{nn+1};
      end
  end

  fprintf('\n---------------------------------------\n');
  if ~isdir(outputDir)
      disp(sprintf('creating directory %s', outputDir));
      mkdir(outputDir);
  end
  % Make a directory for this runIdx if it doesn't already exist
  runDir = sprintf('%s/run%03d', outputDir, runIdx);
  if isdir(runDir)
