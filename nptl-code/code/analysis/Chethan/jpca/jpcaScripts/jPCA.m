...
% %% added by CP, 2014-11-25
%   .removePCs     Default is 0.  How many of the top PCs to remove from the initial dim red step
%   .usePCA        Default is true.  Set tp false if you've done dimensionality reduction some other way
%
...
%%CP - the number of top PCs to remove
removePCs = 0;
if exist('params', 'var') && isfield(params,'removePCs')
    removePCs = params.removePCs;
end
if removePCs<0
    disp('you MUST remove a non-negative number of PCs.'); return;
end
...
usePCA = true;
if exist('params', 'var') && isfield(params,'usePCA')
    usePCA = params.usePCA;
end
...
if usePCA
    ...
    %% CP - modifications to allow removal of first few PCs (alternate way of removing CCM)
    % these are the directions in the high-D space (the PCs themselves)
    if numPCs > size(PCvectors,2) - removePCs
        disp('You asked for more PCs than there are dimensions of data');
        disp('Giving up');
    end
    if removePCs, fprintf('Removing %i PCs\n', removePCs); end
    %PCvectors = PCvectors(:,1:numPCs);  % cut down to the right number of PCs
    PCvectors = PCvectors(:,removePCs+(1:numPCs));  % cut down to the right number of PCs

    % CRITICAL STEP
    % This is what we are really after: the projection of the data onto the PCs
    Ared = rawScores(:,removePCs+(1:numPCs));  % cut down to the right number of PCs
else
    Ared = smallA;
    PCvectors = eye(numPCs);
end
...
if ~exist('params', 'var') || ~isfield(params,'suppressText') || ~params.suppressText
    fprintf('%% Var Captured (all %d dims):   %1.2f  <<---------------\n', numPCs, sum(varCaptEachJPC));
    fprintf('%% Var Captured (primary 2D plane):   %1.2f  <<---------------\n', sum(varCaptEachJPC(1:2)));
end
...
%% CP- 20150126
% code to estimate fraction of (total, high-D) variance captured by the rotational model
% original dimensionality reduction as follows:
%   smallA = bigA(analyzeMask,:);
%   [PCvectors,rawScores] = princomp(smallA,'econ');  % apply PCA to the analyzed times
%   meanFReachNeuron = mean(smallA);  % this will be kept for use by future attempts to project onto the PCs
...
% (code that manually projects all timepoints into LD)
%   bigAred = bsxfun(@minus, bigA, mean(smallA)) * PCvectors(:,1:numPCs); % projection of all the data (not just teh analyzed chunk) into the low-D space.

% projection matrix
projMatx = PCvectors(:,1:numPCs);
% mean subtracted data
meanSmallA = bsxfun(@minus, smallA, mean(smallA));

% underlying math is: Ared = projMatx * meanSmallA;
%  ->  (projMatx' * projMatx)^-1 * projMatx' * Ared = meanSmallA
% need to calculate the pseudoinverse of the projector matrix to project data back into high-D
projInv=inv(projMatx' * projMatx) * projMatx';

% project lowD -> highD
reproj = Ared * projInv;
reproj2 = (meanSmallA*projMatx)*projInv;
% there's a small difference here. not sure what the source is. small enough to ignore.


% manually check fraction of variance captured 
%  (i.e., what is left over when we subtract the reprojected data?)
totalVar = sum(meanSmallA(:).^2);
remainingVar = sum((meanSmallA(:)-reproj(:)).^2);

% disp check to user
% fprintf('CP - check: inv correct if %.3f == %.3f\n',sum(varCaptEachJPC), 1-remainingVar/totalVar);

% now try to do one-step prediction
%preState = Ared(maskT1,:);  % just for convenience, keep the earlier time in its own variable
nextState = Ared(maskT2,:);
nextStateEst = (preState' + Mskew * preState')';


%% use the initial state and predict all time points from there 
%% (edit: this is a horrible idea)
% for t = 1:size(preState,1)
%    if t>1
%        iNextStateEst(t,:) = (iNextStateEst(t-1,:)'+ Mskew*iNextStateEst(t-1,:)')';
%    else
%        iNextStateEst(t,:) = (preState(1,:)'+ Mskew*preState(1,:)')';
%    end
%end
%
%nextStateEst = iNextStateEst;

nextHighD = meanSmallA(maskT2,:);
nextHighDEst = nextStateEst*projInv;

%R2_Mskew_kD = (1-sum((nextState(:)-nextStateEst(:)).^2)) / sum(nextState(:).^2);
residHighDEst = nextHighD-nextHighDEst;
varCapt_Mskew_highD = 1 - sum(residHighDEst(:).^2) /sum(nextHighD(:).^2);

%fprintf('Estimating variance captured by High-D model:\n     %.3f\n',varCapt_Mskew_highD);
%keyboard
