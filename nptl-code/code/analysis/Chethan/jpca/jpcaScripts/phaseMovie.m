...
for ti = 2:length(times2plot)
    ...
    phaseSpace(Projection, Summary, frameParams);
    % put text label in absolute coordinates
    %t=text(-1,-1,sprintf('%03i ms',times2plot(ti)));
    textx = get(gca,'xlim') * 0.9;
    texty = get(gca,'ylim') * 0.9;
    t=text(textx(1),texty(1),sprintf('%03i ms',times2plot(ti)));
