...
%       .labelAxes        whether axes should be labeled with "projection onto jPC_1" etc.  Default is 'true'
%       .axisLabelFontSize font size for axis labels. default is 10.5
%       .centralCrossSize size of center cross. Default is 7.5
...
axLimScale = 1.35;
axisSeparation = 0.30;  % separated by 20% of the maximum excursion (may need more if plotting future times, which aren't used to compute farthestLeft or farthestDown)
...
%% added by CP
labelAxes = true;
if exist('params', 'var') && isfield(params,'labelAxes')
    labelAxes = params.labelAxes;
end
%% added by CP
...
if exist('params', 'var') && isfield(params,'axisLabelFontSize')
...
for pindex = 1:length(planes2plot)
    ...
    if ~exist('params','var') | ~isfield(params,'colorsToUse')
    ...
    if useAxes
        ...
        axisParams.fontSize = axisLabelFontSize;

        % horizontal axis
        axisParams.axisOffset = farthestDown + extraSeparation;

        if labelAxes, axisParams.axisLabel = 'projection onto jPC_1 (a.u.)'; end
        ...
        if labelAxes, axisParams.axisLabel = 'projection onto jPC_2 (a.u.)'; end
