        ...
        for alignIdx = 2
            ...
            for arraySetIdx = 1:length(arraySets)
                ...
                for c=1:length(codeSets)
                    ...
                    oneFactor_dPCA_plot( dPCA_out{c}, timeAxis, lineArgs, margNames, 'sameAxes', avgSpeed );
                ...
                timeAvg = squeeze(mean(tmp,1));
                
                for t=1:size(tmp,1)
                    smoothNeural = gaussSmooth_fast(squeeze(tmp(t,:,:)),2.5);
                    concatNeural = [concatNeural; smoothNeural];
                end
                concatNeural = bsxfun(@plus, concatNeural, -predata.featMean(chanIdx));
                concatNeural = bsxfun(@times, concatNeural, 1./predata.featStd(chanIdx));
                ...
                for c=1:length(codeSets)
                    ...
                    oneFactor_dPCA_plot( new_dpca, timeAxis, lineArgs, margNames, 'sameAxes', avgSpeed );
                ...
                concat_na = bsxfun(@plus, concat_na, -predata.featMean);
                concat_na = bsxfun(@times, concat_na, 1./predata.featStd);
                ...
                for t=1:size(reconVel,1)
                    ...
                    reconVel(t,:,:) = neuralData(timeIdx,:) * normFilts;
                end
