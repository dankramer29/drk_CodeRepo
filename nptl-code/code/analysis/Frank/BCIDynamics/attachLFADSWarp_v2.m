...
for d=1:length(datasets)
    ...
    for codeIdx = 1:length(codeList)
        ...
        nIter = 1; %register to the average ONCE, no iteration or regularization
        ...
        for k=1:nIter
            ...
            for t=1:length(trlIdx)
                ...
                objFun = @(coef)warpObj( squeeze(lfadsTop6(trlIdx(t),:,:)), cAvg, knots, coef );
                newCoef = fmincon(objFun, coefStart, [], [], [], [], zeros(1,length(coefStart)), inf(1,length(coefStart)), [], opts);

                %save result
                [~, warpedCurves(t,:,:), warpFunctions(trlIdx(t),:)] = warpObj( squeeze(lfadsTop6(trlIdx(t),:,:)), cAvg, knots, newCoef );
            end
...
%         for k=1:nIter
%             for dimIdx=1:6
%                 plot(squeeze(allAvg(k,:,dimIdx)),'LineWidth',1.5,'Color',colors(dimIdx,:));
%             end
%         end
