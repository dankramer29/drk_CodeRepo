...
for d=1:size(datasets,1)
    ...
    for blockSetIdx=1:length(datasets{d,2})
        ...
        dirIdx = zeros(size(targList,1),1);
        distIdx = zeros(size(targList,1),1);
        for t=1:size(targList,1)
            [~,dirIdx(t)] = min(matVecMag(dir-targList(t,1:2)/norm(targList(t,1:2)),2));
            [~,distIdx(t)] = min(abs(dist - norm(targList(t,1:2))));
        end
        ...
        for x=1:8
            ...
            codeIdx = find(dirIdx==x & distIdx==2);
            trlIdx = find(codeIdx==targCodesUse);
            for t=1:length(trlIdx)
                loopIdx = alignDat.eventIdx(trlIdx(t)):(alignDat.eventIdx(trlIdx(t))+100);
                plot(alignDat.windowsMousePosition_speed(loopIdx,:),'Color',[0.8 0 0]);
            end
            
            codeIdx = find(dirIdx==x & distIdx==1);
            trlIdx = find(codeIdx==targCodesUse);
            for t=1:length(trlIdx)
                loopIdx = alignDat.eventIdx(trlIdx(t)):(alignDat.eventIdx(trlIdx(t))+100);
                plot(alignDat.windowsMousePosition_speed(loopIdx,:),'Color',[0 0 0.8]);
            end
        end
        ...
        for x=1:8
            ...
            codeIdx = find(dirIdx==x & distIdx==2);
            trlIdx = find(codeIdx==targCodesUse);
            cDat = triggeredAvg(alignDat.windowsMousePosition_speed, alignDat.eventIdx(trlIdx), [0 ,100]);
            plot(mean(cDat),'r');
        ...
        for t=1:length(tcList)
            concatDat = triggeredAvg( smoothSpikes, alignDat.eventIdx(targCodesUse==tcList(t)), [-49,149] );
            datSmooth.(movLabels{t}) = concatDat;
            
            concatDat = triggeredAvg( alignDat.zScoreSpikes, alignDat.eventIdx(targCodesUse==tcList(t)), [-49,149] );
        ...
        if exist([outDir filesep 'warpedCubes_' setNames{blockSetIdx} '.mat'],'file') && useWarpedCubes
            %time-warped cubes
            datSmooth = load([outDir filesep 'warpedCubes_' setNames{blockSetIdx} '.mat']);
            warpSuffix = 'warped';
        else
            warpSuffix = 'raw';
        end
        ...
        for conIdx=1:length(movLabels)
            ...
            for trlIdx=1:length(conTrl)
                ...
                loopIdx = (globalIdx-49):(globalIdx+149);
