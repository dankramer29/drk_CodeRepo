...
for i in range(nSteps):   
    ...
    cvMag = tf.sqrt(tf.reduce_sum(tf.square(controlVec),axis=0,keep_dims=True)) + 1e-8
...
for i in range(nBatches):
    ...
    if i%100 == 0:
        ...
        cs = sess.run([cursorStates], feed_dict={new_lr: lr, startDecState: decStartStates, batchTargets: targetSeq,
                                                       startFMState: fmStartStates, startCursorState: cursorStartStates, tuningCoef: neuralTuning,
                                                       neuralNoise: newNoise, batchCIS: newCIS})
