...
args = parser.parse_args(['--datasets','t5.2016.09.16','t5.2016.09.19','t5.2016.09.26','t5.2016.09.28','t5.2016.10.03','t5.2017.01.30',
                          't5.2017.02.15','t5.2017.03.30','t5.2017.04.26','t5.2017.05.24','t5.2017.07.31','t5.2017.09.20',
                          ...
                          '--nLayers','1','--nDecInputFactors','2','--L2Reg','10','--learnRateStart','0.01','--nEpochs','2000',
                          '--initWeightScale','1.0','--nDecUnits','50','--useInputProj','0','--device','/cpu:0'])
...
with tf.device(args.device[0]):
    ...
    for i in range(args.nEpochs[0]):
            ...
            for batchIdx in range(nBatches):
                ...
                ao, to, te = sess.run([lr_update, train_op, totalErr], 
                                                    feed_dict={new_lr: lr, startDecState: prevStartStates, batchInputs: inputSeq, 
                                                               batchTargets: targSeq, datasetIdx: currentDatasetIdx, errorMask: eMask})
        ...
        for currentDatasetIdx in range(nDatasets):
            ...
            te = sess.run([totalErr], feed_dict={new_lr: lr, startDecState: prevStartStates, batchInputs: inputSeq, batchTargets: targSeq, 
                                       datasetIdx: currentDatasetIdx, errorMask: eMask})
            errListVal.append(te)
        
        #log progress
        mnErrTrain = np.mean(errListTrain)
        mnErrVal = np.mean(errListVal)
        testSummary, trainSummary = sess.run([testErrSummary, trainErrSummary], feed_dict={trainErr_ph: mnErrTrain, testErr_ph: mnErrVal})
    ...
    for currentDatasetIdx in range(nDatasets):
        ...
        for batchIdx in range(nBatches):
            ...
            outputs[currentDatasetIdx][selIdx,:,:] = do[0]
            inFac[currentDatasetIdx][selIdx,:,:] = infa[0]
