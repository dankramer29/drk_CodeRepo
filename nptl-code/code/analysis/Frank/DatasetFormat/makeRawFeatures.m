...
saveDir = '/net/home/fwillett/Data/Derived/2dDatasets/features';
datasetDir = '/net/home/fwillett/Data/Derived/2dDatasets/';
mkdir(saveDir);

t5sess = getT5_2D_CL_Datasets();
t6sess = getT6_2D_CL_Datasets();
sessSets = {t5sess, t6sess};
subjectFolders = {{'t5','t5east'},{'t6','Q'}};
subjectCodes = {'t5','t6'};
...
for setIdx=1:length(sessSets)
    ...
    for s=1:size(currSet,1)
        ...
        if strcmp(currSet{s,3},'east')
            fileNames = getEastNS5FileNames( [readDir subjectFolders{setIdx}{2} filesep currSet{s,1}], currSet{s,2}, subjectCodes{setIdx} );
        elseif strcmp(currSet{s,3},'west')
            fileNames = getWestNS5FileNames( [readDir subjectFolders{setIdx}{1} filesep currSet{s,1}], currSet{s,2}, subjectCodes{setIdx} );
        end
        ...
        opts.doTX = true;
        opts.txThresh = [-2.5,-3.5,-4.5,-5.5,-6.5];
        opts.nCarChans = 60;
        opts.blockList = currSet{s,2};
        opts.resultDir = [saveDir filesep currSet{s,1}];
        opts.syncType = currSet{s,3};
...
datasetDir = '/net/home/fwillett/Data/Derived/2dDatasets/';
nSec = 0;
for s=1:size(t5sess,1)
    disp(s);
    load([datasetDir t5sess{s,1} '.mat']);
    nSec = nSec + length(dataset.cursorPos)/50;
end
