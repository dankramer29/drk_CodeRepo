...
for c=1:length(codeList)
    ...
    traj = [cumsum(mn(51:end,2)), -cumsum(mn(51:end,1))];
...
for t=1:length(letterCodes)
    concatDat = triggeredAvg( alignDat.zScoreSpikes, alignDat.eventIdx(trlCodes==letterCodes(t)), [-50,150] );
    dat.(movLabels{t}) = concatDat;
end

save('alphabetCube.mat','-struct','dat');