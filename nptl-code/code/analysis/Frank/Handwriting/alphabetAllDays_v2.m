...
for sessionIdx=1:size(sessionList,1)
    ...
    for setIdx = 1:length(codeSets)
        if ~isempty(mPCA_out{setIdx})
            simMatrix = plotDistMat_cv( mPCA_out{setIdx}.featureVals  , [1,50], movLabelSets{setIdx} );
    ...
    %%
%     cGroups = {[1 12 26], [2 11 27],[3 10 28]};
%     figure; 
...
%             plot(cumsum(curve(:,1)), cumsum(curve(:,2)), 'Color', [0.8 0 0], 'LineWidth', 2);
%            % plot(curve(:,2), 'Color', [0 0 0.8], 'LineWidth', 2);
%         end
%     end
    ...
    for groupIdx=1:length(cGroups)
        ...
        for cIdx=1:3
            ...
            plot(curveRot(:,1), curveRot(:,2),'-o','LineWidth',2);
        end
        axis equal;
    end
...
%         
    ...
    if strcmp(sessionName,'t5.2019.05.06')
        ...
        [COEFF, SCORE, LATENT, TSQUARED, EXPLAINED, MU] = pca(prepVec(1:24,:) - SCORE(1:24,1:3)*COEFF(:,1:3)');
        
        %%
        %subtract off radial manifold
        rdCon = prepVec(25:end,:);
        nPoints = 16*20;
        radInterp = zeros(nPoints,size(rdCon,2));
        ...
        for segIdx=1:16
            ...
            for neuronIdx = 1:size(rdCon,2)
                radInterp(pointIdx,neuronIdx) = interp1([0,1],[rdCon(segIdx,neuronIdx), rdCon(nextPoint,neuronIdx)],linspace(0,1,20));
            end
            pointIdx = pointIdx + length(pointIdx);
        end
        
        curveVec = prepVec(1:24,:);
        for c=1:size(curveVec,1)
            distVec = matVecMag(curveVec(c,:)-radInterp,2);
            [~,minIdx] = min(distVec);
            curveVec(c,:) = curveVec(c,:) - radInterp(minIdx,:);
        end
        
        [COEFF, SCORE, LATENT, TSQUARED, EXPLAINED, MU] = pca(curveVec);
        ...
        %organize by hypothetical linear second-order dynamics
        colors =[0.8 0 0; 0 0 0; 0 0 0.8];
        curveRotDir = [1,1,1,3,3,3,...
            2,2,2,2,2,2,...
            3,3,3,1,1,1,...
            2,2,2,2,2,2];
        ...
        for c=1:length(rdIdx)
            ...
            plot3(SCORE(rdIdx(c),1), SCORE(rdIdx(c),2), SCORE(rdIdx(c),3),mType,...
                'MarkerFaceColor',colors(colorIdx,:),'Color',colors(colorIdx,:),'MarkerSize',mSizes(curveSize(c)));
        end
    end
        ...
        for plotConIdx=1:length(currIdx)
            ...
            for dimIdx = 1:3
                ...
                if dimIdx==1 || dimIdx==2
                    ...
                    plot(timeAxis, [ones(size(avgNeural,1),1), avgNeural]*filts_curve*5,'-','LineWidth',2,'Color',[0 0.8 0]);
