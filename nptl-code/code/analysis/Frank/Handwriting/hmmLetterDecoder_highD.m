...
for sessionIdx=1:size(sessionList,1)
    ...
    for valFoldIdx=1:10
        ...
        vMult = [1.0];
        cStats = zeros(length(vMult),2);
        
        for vIdx=1:length(vMult)
            for t=1:length(mappedText)
                word = mappedText{t};
                ...
                pStates = hmmdecode_frw_gaussian(binDat,A_hmm,B_hmm,pStateStart,vMult(vIdx));

                [currentState, logP] = hmmviterbi_frw_gaussian(binDat,A_hmm,B_hmm,pStateStart,vMult(vIdx));
            ...
            disp(mean(flCorrect(valTrials)));

            useIdx = allDecLabels~=27 & allLabels~=27;
            disp(mean(allDecLabels(useIdx)==allLabels(useIdx)));
            
            cStats(vIdx,:) = [mean(flCorrect(valTrials)), (mean(allDecLabels(useIdx)==allLabels(useIdx)))];
        end
