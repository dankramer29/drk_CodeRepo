function [pStates, pSeq] = hmmdecode_frw_gaussian(seq,tr,e,startProb,varFactor)
...
%tr = tr';
for count = 2:L
    gaussianEmissionProb = exp(-sum((e-seq(count-1,:)).^2,2)/varFactor);
...
bs = ones(numStates,L);
for count = L-1:-1:1
    gaussianEmissionProb = exp(-sum((e-seq(count,:)).^2,2)/varFactor);
    for state = 1:numStates
