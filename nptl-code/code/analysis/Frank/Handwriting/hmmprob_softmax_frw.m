function [pSeq] = hmmprob_softmax_frw(eProb,tr,eProbStateIdx,startProb)
...
numStates = size(tr,1);
numSteps = size(eProb,1);

fs = zeros(numStates,numSteps+1);
fs(:,1) = startProb;

s = zeros(1,numSteps+1);
s(1) = 1;

tr = tr';
epReorder = eProb(:,eProbStateIdx)';

for count = 2:(numSteps+1)
    fs(:,count) = epReorder(:,count-1) .* (tr*fs(:,count-1));
