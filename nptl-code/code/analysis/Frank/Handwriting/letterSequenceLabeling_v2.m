...
for sessionIdx=1:size(sessionList,1)
    ...
    for t=1:length(mappedText)
        ...
        %initial guess
        figure('Position',[ 217         745        1704         299]);
        subplotDim = ceil(sqrt(length(word)));
        ...
        for c=1:length(word)
            ...
            subplot(subplotDim,subplotDim,c);
            %subplot(1,length(word),c);
            hold on;
            imagesc(possibleStart/100, possibleStretch, allCorr);
            axis tight;
            plot(possibleStart(validStartIdx(maxStartIdx))/100,possibleStretch(maxStretch),'kx','MarkerSize',18,'LineWidth',5); 
            %plot([currIdx, currIdx]/100,get(gca,'YLim'),'--w','LineWidth',2);
            title([word(c) ' (' num2str(letterStarts(c)/100) ')'],'FontSize',14);
            xlabel('Time (s)');
        ...
        saveas(gcf,[outDir filesep 'charLabel_' num2str(c) '.png'],'png');
        close(gcf);
            ...
            for c=1:length(word)
                ...
                for stretchIdx=1:length(possibleStretch)
                    for startIdx=1:length(pStart)
                        ...
                        tmpCost(stretchIdx, startIdx) = reconCost( dat, templates(allTempIdx), currStart, currStretch );
                    end
                end
