...
for sessionIdx=1:size(sessionList,1)
    ...
    for t=1:length(mappedText)
        idx = find(strcmp(mappedText{t},mappedText));
        if length(idx)>1
            disp(mappedText{t});
        end
    end
    ...
    %shuffle for better training/validation split
    shuffIdx = randperm(size(fullData,1));
    fullData = fullData(shuffIdx,:,:);
    fullDataLabels = fullDataLabels(shuffIdx,:,:);
    fullDataWeight = fullDataWeight(shuffIdx,:);
    mappedText = mappedText(shuffIdx);
    numBinsPerTrial = numBinsPerTrial(shuffIdx);
