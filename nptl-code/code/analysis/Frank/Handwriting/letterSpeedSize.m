...
blockList = [16 17 18 19 20 21 22];
...
movLabels = [movLabels1, movLabels2];

dat = struct();
for t=1:length(codeList)
    concatDat = triggeredAvg( alignDat.zScoreSpikes_blockMean, alignDat.eventIdx(trlCodes==codeList(t)), [-50,250] );
    dat.(movLabels{t}) = concatDat;
end

save('letterSpeedSize_cube.mat','-struct','dat');
...
codeList = unique(trlCodes);
codeSets = {codeList(1:16), codeList(17:end), codeList(1:4), codeList(5:8), codeList(9:12), codeList(13:16)};
