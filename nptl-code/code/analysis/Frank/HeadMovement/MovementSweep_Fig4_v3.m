...
for d=1:size(datasets,1)
    ...
    for blockSetIdx=1:length(datasets{d,2})
        ...
        smoothSpikes_3 = gaussSmooth_fast(alignDat.zScoreSpikes, 3);
        for setIdx=1:length(idxSets)
            trlIdx = find(ismember(targCodes(useTrl),idxSets{setIdx}));

            dPC_window = [-500,1500];
            dPCA_all_xval{setIdx} = apply_dPCA_simple( smoothSpikes_3, alignDat.eventIdx(trlIdx), ...
                targCodes(useTrl(trlIdx)), dPC_window/binMS, binMS/1000, {'CD','CI'}, 20, 'xval', 'marg');
            close(gcf);

            %jPCA
            nCon = size(dPCA_all_xval{setIdx}.featureAverages,2);
            tw_all = [-49, 149];
            timeStep = binMS/1000;
            ...
            for n=1:nCon
                Data(n).A = squeeze(dPCA_all_xval{setIdx}.featureAverages(:,n,:))';
                Data(n).times = timeMS;
            end
            ...
            for wIdx=1:length(winStart)
                ...
                saveas(gcf, [outDir filesep setNames{blockSetIdx} '_' num2str(windowIdx(1)) '_to_' num2str(windowIdx(2)) '_jPCA.png'],'png');
            end
            close all;
        end
