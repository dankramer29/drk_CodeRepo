...
for d=1:size(datasets,1)
    ...
    %matchSets = {[2 4]};
    matchSets = {[5 8]};
    for matchIdx=1:length(matchSets)
        set1 = matchSets{matchIdx}(1);
        set2 = matchSets{matchIdx}(2);
        
        set1Blocks = datasets{d,2}{set1};
        set2Blocks = datasets{d,2}{set2};
        
        set1Trl = find(ismember(alignDat.bNumPerTrial, set1Blocks));
        set2Trl = find(ismember(alignDat.bNumPerTrial, set2Blocks));
        
        movCues = alignDat.currentMovement(alignDat.eventIdx);
        set1Cues = movCues(set1Trl);
        set2Cues = movCues(set2Trl)+100;
        
        allTrl = [set1Trl; set2Trl];
        allCues = [set1Cues; set2Cues];
        [~,~,cueRemap] = unique(allCues);
        
        dPCA_out_dual = apply_dPCA_simple( alignDat.zScoreSpikes, alignDat.eventIdx(allTrl)+1, ...
            cueRemap, timeWindow/binMS, binMS/1000, {'Dir L', 'Dir R', 'CI', 'L x R'}, 10 );
        
        effSets = {1:4, 5:8};
        cWindow = (10:50)+75;
        dualOnly = dPCA_out_dual.featureVals(:,[5:8, 13:16],:,1:18);
        
        simMatrix = plotCorrMat_cv( dualOnly, cWindow, [], effSets, [] );
    end
...
%         plot([1.5 1.5],get(gca,'YLim'),'--k','LineWidth',2);
        ...
        [cvScore, COEFF] = cvPCA_class( tmp_unroll, classLabels, 'rotation' );
