...
for d=1:size(datasets,1)
    ...
    R = getStanfordRAndStream( sessionPath, horzcat(datasets{d,2}{:}), 4.5, 1, filtOpts );
    ...
    for prepIdx=1:2
        ...
        pca_Z = cell(length(datasets{d,2}),1);
        for blockSetIdx=1:length(datasets{d,2})
            ...
            dPCA_all_xval{blockSetIdx} = apply_dPCA_simple( smoothSpikes, alignDat.eventIdx(trlIdx), ...
                fullCodes(trlIdx), dPC_window/binMS, binMS/1000, {'CD','CI'}, 20, 'xval');
            close(gcf);
            
            trlAvg = squeeze(mean(dPCA_all_xval{blockSetIdx}.featureAverages(:,:,cWindow),3));
            winAvg = squeeze(mean(dPCA_all_xval{blockSetIdx}.featureVals(:,:,cWindow,:),3));
            
            [COEFF, SCORE, LATENT, TSQUARED, EXPLAINED, MU] = pca(trlAvg');
            pca_Z{blockSetIdx} = zeros(size(winAvg,3),size(winAvg,2),2);
            winAvg = permute(winAvg,[3 2 1]);
            
            for tIdx=1:size(winAvg,1)
                pca_Z{blockSetIdx}(tIdx,:,1) = (squeeze(winAvg(tIdx,:,:))-MU)*COEFF(:,1);
                pca_Z{blockSetIdx}(tIdx,:,2) = (squeeze(winAvg(tIdx,:,:))-MU)*COEFF(:,2);
            end
        end
        ...
        for plotIdx=1:length(toPlot)
            ...
            if usePCA
                ...
                Z = squeeze(dPCA_all_xval{blockSetIdx}.cval.Z_singleTrial(:,cdIdx,:,cWindow));
            end
            nanRows = find(any(isnan(Z(:,:)),2));
            Z(nanRows,:,:) = [];
            ...
            signPatterns = [1, 1; -1, 1; 1, -1; -1, -1];
            xcost = zeros(size(signPatterns,1),1);
            transMat = cell(size(signPatterns,1),1);
            for signIdx=1:size(signPatterns,1)
                X_signed = X;
                X_signed(:,1) = X_signed(:,1)*signPatterns(signIdx,1);
                X_signed(:,2) = X_signed(:,2)*signPatterns(signIdx,2);
                
                target_unroll = [xAxis_target; yAxis_target];
                data_unroll = [[X_signed, zeros(size(X_signed))]; [zeros(size(X_signed)), X_signed]];

                manifold = stiefelfactory(2,2);
                problem.M = manifold;

                problem.cost  = @(x) ((target_unroll - data_unroll*x(:))'*(target_unroll - data_unroll*x(:)));
                problem.egrad = @(x) rotationCostGrad(x, target_unroll, data_unroll);

                options.verbosity = 0;
                warning('off', 'manopt:getHessian:approx');

                [transMat{signIdx}, xcost(signIdx), info] = trustregions(problem,[],options);
            end
            [~, minIdx] = min(xcost);
            
            for t=1:nTargs
                zs = squeeze(Z(:,t,:));
                zs = zs .* signPatterns(minIdx,:);
                zs = zs * transMat{minIdx};
