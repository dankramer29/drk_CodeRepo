...
%reorder the codes to put sets of movements together
reorderIdx = [34, ...
...
for setIdx=1:length(codeSets)
    setTrlWithBaseline = find(ismember(trlCodesRemap, [1,codeSets{setIdx}]));
    setGrandMean = mean(meanRate(setTrlWithBaseline,:));
    
    setTrl = find(ismember(trlCodesRemap, codeSets{setIdx}));
    setConMean = mean(meanRate(setTrl,:));
    
    for t=1:length(codeSets{setIdx})
        trlIdx = find(trlCodesRemap==codeSets{setIdx}(t));
        baseIdx = find(trlCodesRemap==1);
        
        meanCon = mean(meanRate(trlIdx,:))-setConMean;
        meanBase = mean(meanRate(baseIdx,:));
        
        averageModulation(codeSets{setIdx}(t)) = norm(meanCon-meanBase);
    end
end

%%
%compute modulation vectors for each movement
modVectors = zeros(length(trlCodeList),size(meanRate,2));
for t=1:length(trlCodeList)
    trlIdx = find(trlCodesReorder==t);
    baselineIdx = find(trlCodesReorder==1);
    modVectors(t,:) = mean(meanRate(trlIdx,:)) - mean(meanRate(baselineIdx,:));
end

%subtract avg mod vector from each set
% {'Face','Head','Arm','Leg'};
codeSets = {2:12, 13:20, 21:28, 29:34};
for setIdx=1:length(codeSets)
    modVectors(codeSets{setIdx},:) = modVectors(codeSets{setIdx},:) - mean(modVectors(codeSets{setIdx},:));
end

%%
%for each trial type, compute modulation magnitude relative to baseline
averageModulation = zeros(length(trlCodeList),2);
figure; 
for t=1:length(trlCodeList)
    trlIdx = find(trlCodesReorder==t);
...
%%
%plot an overview as a sanity check
...
plot(averageModulation(:,1),'-o','LineWidth',2);
