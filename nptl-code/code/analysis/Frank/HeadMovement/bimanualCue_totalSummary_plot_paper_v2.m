...
figure
hold on;
for t=1:size(effDim,1)
    for x=1:8
        plot(linSpacing(x)+t,contextDim(t,x),'o','Color',colors(x,:));
        plot(linSpacing(x)+[t,t],[contextDim_ci(t,2*(x-1)+1), contextDim_ci(t,2*x)],'-','Color',colors(x,:));
    end
end
...
%%
allDat.varSingle_1 = dat(3:5:end,:);
allDat.varSingle_2 = dat(4:5:end,:);
allDat.varDual_1 = dat(1:5:end,:);
allDat.varDual_2 = dat(2:5:end,:);
allDat.varInteraction = dat(5:5:end,:);

allDat.attenFactor1 = dat(3:5:end,1)./dat(1:5:end,1);
allDat.attenFactor2 = dat(4:5:end,1)./dat(2:5:end,1);
allDat.relativeAttenuation = allDat.attenFactor2 - allDat.attenFactor1;
allDat.relativeVariance_singleContext = dat(4:5:end,1)./dat(3:5:end,1);
...
af1 = dat(1:5:end,1)./dat(3:5:end,1);
af2 = dat(2:5:end,1)./dat(4:5:end,1);
...
figure('Position',[332   780   498   310]);
hold on;
...
bothSingleVar = [allDat.varSingle_1, allDat.varSingle_2];
bothDualVar = [allDat.varDual_1, allDat.varDual_2];
...
figure('Position',[332   780   498   310]);
for effIdx=1:length(effOrder)
    subplot(length(effOrder),1,effIdx);
	hold on;
    ...
    [~,sortIdx] = sort(taskCodes(tableIdx,1),'descend');
    tableIdx = tableIdx(sortIdx);
    expLabels = cell(length(tableIdx),1);
    
    for x=1:length(tableIdx)
        colIdx1 = find(taskCodes(tableIdx(x),:)==currentEff);
        colIdx2 = find(taskCodes(tableIdx(x),:)~=currentEff);
        if colIdx1==1
            colIdx1_mn = 1;
            colIdx1_ci = [2,3];
        else
        ...
        newMN = [bothSingleVar(tableIdx(x),colIdx1_mn), bothDualVar(tableIdx(x),colIdx1_mn), allDat.varInteraction(tableIdx(x),1), ...
            decAcc_dual(tableIdx(x),colIdx1), decAcc_singleToDual(tableIdx(x),colIdx1)];
        newCI = [bothSingleVar(tableIdx(x),colIdx1_ci);
            bothDualVar(tableIdx(x),colIdx1_ci);
            allDat.varInteraction(tableIdx(x),2:3);
            [decAcc_dual(tableIdx(x),colIdx1), decAcc_dual(tableIdx(x),colIdx1)];
            [decAcc_singleToDual(tableIdx(x),colIdx1), decAcc_singleToDual(tableIdx(x),colIdx1)]];
        
        newMN(1:3) = newMN(1:3) / bothSingleVar(tableIdx(x),colIdx1_mn);
        newCI(1:3,:) = newCI(1:3,:) / bothSingleVar(tableIdx(x),colIdx1_mn);
        
        newMN = newMN(1:2);
        newCI = newCI(1:2,:);
        
        eff1Code = taskCodes(tableIdx(x),colIdx1);
        xSpace = linspace(-0.15,0.15,length(newMN));
        
        for barIdx=1:length(newMN)
            bar(currentPlotIdx+xSpace(barIdx), newMN(barIdx), 0.3, 'FaceColor', effColors(eff1Code,:),'LineWidth',1);
            errorbar(currentPlotIdx+xSpace(barIdx), newMN(barIdx), newMN(barIdx)-newCI(barIdx,1), newCI(barIdx,2)-newMN(barIdx), 'k','LineWidth',1);
        end
        
        expLabels{x} = [effNames{taskCodes(tableIdx(x),colIdx1)} ' vs ' effNames{taskCodes(tableIdx(x),colIdx2)}];
        
        currentPlotIdx = currentPlotIdx + 1;
    end
    xlim([0,8]);
    set(gca,'XTick',1:length(tableIdx),'XTickLabel',expLabels,'XTickLabelRotation',45);
end
