...
for d=1:length(datasets)
    ...
    if strcmp(datasets{d,1}(1:2),'t5')
        afSet = {'goCue','goCue','goCue'};
        twSet = {[-1500,3000],[-1500,0],[-3000,3000]};
        pfSet = {'goCue','delay','longDelay'};
    else
        afSet = {'goCue'};
        twSet = {[-1500,6500]};
        pfSet = {'goCue'};
    ...
    %           BI_DOWN_DOWN(210)
                ...
                %classifier
                goCueIdx = alignDat.eventIdx(allIdx(noSingleMovements));
                lossFunction = zeros(length(timeAxis),2);
                timeAxis = 1+(timeWindow(1)/binMS):(timeWindow(2)/binMS);
                
                for movSeqIdx=1:2
                    trlCodes = movFactors(noSingleMovements,movSeqIdx);

                    for timeStepIdx=1:length(timeAxis)
                        disp(timeStepIdx);
                        
                        loopIdx = goCueIdx + timeAxis(timeStepIdx);
                        obj = fitcdiscr(alignDat.zScoreSpikes(loopIdx,:),trlCodes,'DiscrimType','diaglinear');                   
                        cvmodel = crossval(obj);
                        lossFunction(timeStepIdx, movSeqIdx) = kfoldLoss(cvmodel,'LossFun','classiferror');
                    end
                end
                
                timeAxis_s = timeAxis*0.02;
                
                figure;
                hold on;
                plot(timeAxis_s, 1-lossFunction,'LineWidth',2);
                plot([0,0],get(gca,'YLim'),'--k','LineWidth',2);
                plot(get(gca,'XLim'),[0.25, 0.25],'--k','LineWidth',1);
                xlabel('Time');
                ylabel('Decoding Accuracy');
                legend({'Movement 1','Movement 2'});
                set(gca,'FontSize',16);
                xlim([-3.0, 1.5]);
                
                saveas(gcf,[outDir filesep datasets{d,3}{outerMovSetIdx} '_discreteDecode_' pfSet{alignSetIdx} '.png'],'png');
                saveas(gcf,[outDir filesep datasets{d,3}{outerMovSetIdx} '_discreteDecode_' pfSet{alignSetIdx} '.svg'],'svg');
                ...
                allDecCI = cell(length(movSets),2);
                ...
                for setIdx=1:2
                    ...
                    for movIdx=1:4
                        ...
                        handleSolid = plot(pos(:,1), pos(:,2), '-', 'LineWidth', 2, 'Color', colors(movIdx,:));
                        %plot(squeeze(allDecCI{setIdx,1}(movIdx,end,1,:)), [pos(end,2), pos(end,2)], '-', 'LineWidth', 2, 'Color', colors(movIdx,:));
                        %plot([pos(end,1), pos(end,1)], squeeze(allDecCI{setIdx,1}(movIdx,end,2,:)), '-', 'LineWidth', 2, 'Color', colors(movIdx,:));
                        ...
                        handleCrossed = plot(posCross(:,1), posCross(:,2), '-', 'LineWidth', 2, 'Color', colors(movIdx,:), 'LineStyle', ':');
                        %plot(squeeze(allDecCI{setIdx,2}(movIdx,end,1,:)), [posCross(end,2), posCross(end,2)], ':', 'LineWidth', 2, 'Color', colors(movIdx,:));
                        %plot([posCross(end,1), posCross(end,1)], squeeze(allDecCI{setIdx,2}(movIdx,end,2,:)), ':', 'LineWidth', 2, 'Color', colors(movIdx,:));
                        
                        rectangle('Position',[squeeze(allDecCI{setIdx,2}(movIdx,end,1,1)), squeeze(allDecCI{setIdx,2}(movIdx,end,2,1)), ...
                            squeeze(allDecCI{setIdx,2}(movIdx,end,1,2))-squeeze(allDecCI{setIdx,2}(movIdx,end,1,1)), ...
                            squeeze(allDecCI{setIdx,2}(movIdx,end,2,2))-squeeze(allDecCI{setIdx,2}(movIdx,end,2,1))], 'Curvature', [1 1], ...
                            'LineWidth', 2, 'EdgeColor', colors(movIdx,:), 'LineStyle',':');
                    end
                    axis equal;
                    xlim([-1.1, 1.1]);
                    ylim([-1.1, 1.1]);
                    axis off;
                    if setIdx==1
                        legend([handleSolid, handleCrossed], {'Single Movement','Dual Movement'}, 'Location', 'Best');
                    end
                    title(movTitles{setIdx});
                    set(gca,'FontSize',16);
                end
                
                saveas(gcf,[outDir filesep datasets{d,3}{outerMovSetIdx} '_decTraj_' pfSet{alignSetIdx} '.png'],'png');
                saveas(gcf,[outDir filesep datasets{d,3}{outerMovSetIdx} '_decTraj_' pfSet{alignSetIdx} '.svg'],'svg');
            end
        end
        ...
        if strcmp(datasets{d,1},'t5.2018.03.14')
            ...
            dPCA_out = apply_dPCA_simple( alignDat.zScoreSpikes, alignDat.eventIdx(allIdx), ...
                [movType, movCues], timeWindow/binMS, binMS/1000, {'Side', 'Dir', 'CI', 'Side x Dir'} );
            ...
            [yAxesFinal, allHandles] = twoFactor_dPCA_plot( dPCA_out,  (timeWindow(1)/binMS):(timeWindow(2)/binMS), ...
                lineArgs, {'Side', 'Dir', 'CI', 'Side x Dir'}, 'sameAxes');
