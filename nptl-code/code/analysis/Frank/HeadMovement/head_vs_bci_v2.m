...
orthoSNR = zeros(length(zeroWeights),2);
...
for x=1:length(alphaCoeff)
    ...
    [fTarg, weightVec] = fitFTarg(posErrMatrix(cursorIdx,:), decOut(cursorIdx,:), 500, 10, true);
    noise = (weightVec - decOut(cursorIdx,1:2));
    signal = mean(fTarg((end-2):end,2));
    orthoSNR(x) = signal / nanstd(noise(:));
