...
for d=1:size(datasets,1)
    ...
    %%
    outDir = [paths.dataPath filesep 'Derived' filesep 'MovementSweep' filesep 'SpatialQuadModSize' filesep datasets{d,1}];
    mkdir(outDir);
    ...
    for blockSetIdx=1:length(datasets{d,2})
        ...
        elseif strfind(datasets{d,3}{blockSetIdx}, 'QuadReachRadial6')
            ...
            idxSets = {[14 18 22 23 19 15],[1 5 9 10 6 2],[16 20 24 25 21 17],[3 7 11 12 8 4]};
            for setIdx=1:length(idxSets)
                targLayout(idxSets{setIdx},:) = targLayout(idxSets{setIdx},:) - mean(targLayout(idxSets{setIdx},:));
                targLayout(idxSets{setIdx},:) = targLayout(idxSets{setIdx},:)./matVecMag(targLayout(idxSets{setIdx},:),2);
            end
        ...
        for targSet=1:4
            trlIdx = find(ismember(targCodes(useTrl), idxSets{targSet}));
            [~,~,remapCodes] = unique(targCodes(useTrl(trlIdx)));
            mPCA_out = apply_mPCA_general( smoothSpikes, alignDat.eventIdx(trlIdx), ...
                remapCodes, [-1800, 1800]/binMS, binMS/1000, opts_m);
            
            movWindow = [20 60];
            baselineWindow = [-70 -50];
            codeSets = {1:6};

            [ cVar_marg, rawProjPoints_marg, scatterPoints ] = modulationMagnitude_internalBaseline( remapCodes, smoothSpikes, ...
            ...
            set(gcf,'Position',[164   751   341   338]);
            
            saveas(gcf,[outDir filesep 'bar_vsInternalBaseline_raw_set' num2str(targSet) '.png'],'png');
            saveas(gcf,[outDir filesep 'bar_vsInternalBaseline_raw_set' num2str(targSet) '.svg'],'svg');
            ...
            saveas(gcf,[outDir filesep 'bar_vsInternalBaseline_ms_set' num2str(targSet) '.png'],'png');
            saveas(gcf,[outDir filesep 'bar_vsInternalBaseline_ms_set' num2str(targSet) '.svg'],'svg');
        end
        ...
        %save numerical values for summarizing later
        save([outDir filesep 'CMat_' datasets{d,3}{blockSetIdx} '.mat'],'cMat_xVal');
...
%%
%summarize correlation values across both sessions
datasets = {
    't5.2018.12.05',{[14 15 16 17 19 20 21 22 23 24 25 26]},{'QuadReachRadial6'};
    't5.2019.02.13',{[14 15 16 17 19 20 21 22]},{'Quad_Radial8'};
};
...
cMat = cell(size(datasets,1),1);
for d=1:size(datasets,1)
    outDir = [paths.dataPath filesep 'Derived' filesep 'MovementSweep' filesep 'Fig4' filesep datasets{d,1}];
    cMat{d} = load([outDir filesep 'CMat_' datasets{d,3}{blockSetIdx} '.mat'],'cMat_xVal');
end

horz1 = squeeze(cMat{1}.cMat_xVal(:,:,1));
horz2 = squeeze(cMat{2}.cMat_xVal(:,:,1));
h_negIdx = [2 4 5 7 10 12 13 15];
h_posIdx = [3 8 9 14];

vert1 = squeeze(cMat{1}.cMat_xVal(:,:,2));
vert2 = squeeze(cMat{2}.cMat_xVal(:,:,2));
v_posIdx = [2 3 4 5 7 8 9 10 12 13 14 15];
    
negValues = [horz1(h_negIdx), horz2(h_negIdx)];
posValues = [horz1(h_posIdx), horz2(h_posIdx), vert1(v_posIdx), vert2(v_posIdx)];

[h,p] = ttest2(negValues, posValues);
