function [ cVar, rawProjPoints, example_plane ] = modulationMagnitude_dpca( trlCodes, smoothSnippetMatrix, snippetMatrix, eventIdx, trlBaselineMatrix, ...
    ...
    for pIdx = 1:length(codeSets)
        ...
        ortho_dpca = apply_dPCA_simple( smoothSnippetMatrix, eventIdx(setTrl), ...
            trlCodes(setTrl), [0, 1000]/binMS, binMS/1000, {'CD','CI'}, [11, 11], 'standard', 'ortho' );
        close(gcf);
        ...
        for codeIdx=1:length(codeSets{pIdx})
            ...
            %example points
            example_plane{codeSets{pIdx}(codeIdx),1} = ma * cdDim;
            example_plane{codeSets{pIdx}(codeIdx),2} = ba_r * cdDim;
        end
    end    
end
