...
save([outDir filesep 'barData_movSweepAll_dPCA'],'cVar_dpca','rawProjPoints_dpca', 'scatterPoints', 'mnMod');
...
%%
%for fun, run mPCA on all movements
trlIdx = find(ismember(trlCodesReorder, horzcat(codeSets{:})));
mc = trlCodesReorder(trlIdx)';
mc_oneStart = mc-min(mc)+1;

mPCA_out_all = apply_mPCA_general( smoothSnippetMatrix, eventIdx(trlIdx), ...
    mc_oneStart, tw, binMS/1000, opts_m );
