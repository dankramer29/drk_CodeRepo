...
[ cVar_marg, rawProjPoints_marg, scatterPoints ] = modulationMagnitude_marg( trlCodes', snippetMatrix, eventIdx, baselineTrls, movWindow, ...
...
nDimToShow = 3;
effSets = {1:8, 1:8, 1:6, 1:6};

for setIdx = 1:length(codeSets)
    figure('Position',[680   185   442   913]);
    for c=1:length(codeSets{setIdx})
        concatDat = triggeredAvg( mPCA_out{setIdx}.readoutZ_unroll(:,1:nDimToShow), eventIdx(trlCodes==codeSets{setIdx}(c)), tw );
        %concatDat = triggeredAvg( mPCA_out{setIdx}.readoutZ_unroll(:,6:8), eventIdx(trlCodes==codeSets{setIdx}(c)), tw );
        ...
        for dimIdx=1:nDimToShow
            ...
            if dimIdx==1
                ylabel(movLabels{effSets{setIdx}(c)},'FontSize',12,'FontWeight','bold');
            end
            if c==1
                title(['Dimension ' num2str(dimIdx)],'FontSize',16);
            end

            if c==length(codeSets{setIdx})
                set(gca,'YTick',[]);
                xlabel('Time (s)');
            else
                set(gca,'XTick',[],'YTick',[]);
...
allMeanRates = cell(length(plotNames),1);
...
for pIdx = 1:length(plotNames)
    ...
    %%
    %reconstruct activity without laterality component
    nonLatDim = find(dPCA_out{pIdx}.whichMarg~=2);
    reconActivity = zeros(size(dPCA_out{pIdx}.featureVals));
    for x1=1:size(reconActivity,2)
        for x2=1:size(reconActivity,3)
            for x3=1:size(reconActivity,5)
                tmp = squeeze(dPCA_out{pIdx}.featureVals(:,x1,x2,:,x3))';
                reconActivity(:,x1,x2,:,x3) = ((tmp*dPCA_out{pIdx}.W(:,nonLatDim))*dPCA_out{pIdx}.V(:,nonLatDim)')';
            end
        end
    end
    
    twMov = [200, 600];
    movLoopIdx = (twMov/binMS)-(timeWindowMov(1)/binMS);
    movLoopIdx = movLoopIdx(1):movLoopIdx(end);
    
    contraRate = squeeze(reconActivity(:,:,2,movLoopIdx,:));
    ipsiRate = squeeze(reconActivity(:,:,1,movLoopIdx,:));
    
    mn1 = nanmean(nanmean(nanmean(contraRate,1),3),4);
    mn2 = nanmean(nanmean(nanmean(ipsiRate,1),3),4);
    
    allMeanRates{pIdx} = [mn1', mn2'];
...
save([outDir filesep 'dPCA_laterality'],'dPCA_out','allCrossValues_cell','dPCA_out_xval','allMeanRates');

%%
%reconstructed average firing rates without laterality dimension
baseRate = mean(baselineTrls(:));
reconRates = {allMeanRates{3}(:,1)-baseRate, allMeanRates{3}(:,2)-baseRate, allMeanRates{2}(:,1)-baseRate, allMeanRates{2}(:,2)-baseRate};
...
plot([3 4], [reconRates{3}, reconRates{4}], 'k-')
plot([1 2], [reconRates{1}, reconRates{2}], 'k-')
...
saveas(gcf,[outDir filesep 'bar_signedFR_noLatRecon.png'],'png');
saveas(gcf,[outDir filesep 'bar_signedFR_noLatRecon.svg'],'svg');

[h,p]=ttest(reconRates{2}-reconRates{1})
[h,p]=ttest(reconRates{4}-reconRates{3})
...
%%
%mean firing rates when reconstructing activity without laterality & effector
...
dPCA_out = apply_dPCA_general( smoothSnippetMatrix, eventIdx(trlIdx), ...
    newFactors(trlIdx,:), [-100,300], 0.010, opts);

nonLatDim = find(dPCA_out.whichMarg~=2 & dPCA_out.whichMarg~=1);
reconActivity = zeros(size(dPCA_out.featureVals));
for x1=1:size(reconActivity,2)
    for x2=1:size(reconActivity,3)
        for x3=1:size(reconActivity,4)
            for x4=1:size(reconActivity,6)
                tmp = squeeze(dPCA_out.featureVals(:,x1,x2,x3,:,x4))';
                reconActivity(:,x1,x2,x3,:,x4) = ((tmp*dPCA_out.W(:,nonLatDim))*dPCA_out.V(:,nonLatDim)')';
            end
        end
    end
end
...
twMov = [200, 600];
movLoopIdx = (twMov/binMS)-(-1000/binMS);
movLoopIdx = movLoopIdx(1):movLoopIdx(end);

contraRate_arm = squeeze(reconActivity(:,1,1,:,movLoopIdx,:));
ipsiRate_arm = squeeze(reconActivity(:,2,1,:,movLoopIdx,:));

contraRate_leg = squeeze(reconActivity(:,1,2,:,movLoopIdx,:));
ipsiRate_leg = squeeze(reconActivity(:,2,2,:,movLoopIdx,:));

cArm = nanmean(nanmean(nanmean(contraRate_arm,1),3),4);
iArm = nanmean(nanmean(nanmean(ipsiRate_arm,1),3),4);
cLeg = nanmean(nanmean(nanmean(contraRate_leg,1),3),4);
iLeg = nanmean(nanmean(nanmean(ipsiRate_leg,1),3),4);

allMeanRates = {cArm(~isnan(cArm)), iArm(~isnan(iArm)), cLeg(~isnan(cLeg)), iLeg(~isnan(iLeg))};

%%
%reconstructed average firing rates without laterality or effector dimension
baseRate = mean(baselineTrls(:));
reconRates = {allMeanRates{1}'-baseRate, allMeanRates{2}'-baseRate, allMeanRates{3}'-baseRate, allMeanRates{4}'-baseRate};
...
saveas(gcf,[outDir filesep 'bar_signedFR_noLatEffRecon.png'],'png');
saveas(gcf,[outDir filesep 'bar_signedFR_noLatEffRecon.svg'],'svg');
