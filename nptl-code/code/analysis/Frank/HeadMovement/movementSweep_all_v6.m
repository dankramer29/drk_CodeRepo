...
%     end
    ...
    decoder_cross_plot_norm( decOut, bNumPerTrial, trlCodes, trlCodesRemap, eventIdx, ...
        timeAxis, movTypes, movLegends, movTypeText, timeWindow, binMS );
...
remapTable = [];
currentIdx = 0;
newCodeGroups = cell(length(codes),1);
...
%     newData = [];
%     dataIdx = 30:50;
%     for n=1:length(trlIdx)
%         loopIdx = dataIdx + eventIdx(trlIdx(n));
%         newData = [newData; mean(snippetMatrix(loopIdx,:))];
%     end
%     
%     codeGroups{pIdx} = unique(newCodes);
%     allFeatures = [allFeatures; newData];
% end    
...
ml = movLabels(unique(allCodesRemap));
Y = tsne(mnFeat,'Distance','correlation','Perplexity',10);
...
figure
hold on
for p=1:length(newCodeGroups)
    if newCodeGroups{p}(end)<=length(Y)
        for x=1:length(newCodeGroups{p})
            plot(Y(newCodeGroups{p}(x),1), Y(newCodeGroups{p}(x),2),'o','MarkerFaceColor',colors(p,:),'Color',colors(p,:),'MarkerSize',8);
            text(Y(newCodeGroups{p}(x),1), Y(newCodeGroups{p}(x),2), ml{newCodeGroups{p}(x)},'FontSize',12);
        end
    end
end
...
%         for x=1:length(newCodeGroups{p})
%             randColor = randi(6);
%             plot(Y(newCodeGroups{p}(x),1), Y(newCodeGroups{p}(x),2),'o','MarkerFaceColor',colors(randColor,:),'Color',colors(randColor,:),'MarkerSize',8);
%             text(Y(newCodeGroups{p}(x),1), Y(newCodeGroups{p}(x),2), ml{newCodeGroups{p}(x)},'FontSize',12);
%         end
%     end
% end
