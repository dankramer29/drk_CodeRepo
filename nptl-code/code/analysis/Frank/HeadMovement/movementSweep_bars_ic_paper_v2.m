...
[ cVar, rawProjPoints ] = modulationMagnitude_v3( trlCodes', smoothSnippetMatrix, eventIdx, nothingDat, movWindow, ...
...
movLabels = {'ShoShrug','ArmRaise','ElbowFlex','WristExt','HandClose','HandOpen','IndexRaise','ThumbRaise',...
    ...
    'AnkleExt','AnkleFlex','KneeExt','HipFlex','ToeCurl','ToeOpen'};
singleTrialBarPlot_v2( codeSets, rawProjPoints, cVar, movLabels );
...
for c=1:length(codeSets)
    mnMod(c) = mean(cVar(codeSets{c},1));
end
...
for pIdx = 1:2
    ...
    %close(gcf);
    timeWindowMov = [0 1500];
    cutTrl = find(ismember(newCodes(:,1), [2 3 4 5]));
    dPCA_out{pIdx} = apply_dPCA_simple( smoothSnippetMatrix, eventIdx(trlIdx(cutTrl)), ...
        newCodes(cutTrl,:), timeWindowMov/binMS, binMS/1000, {'Movement','Effector','CI','MxE Interaction'}, [5 5 5], 'none', 'ortho' );
    close(gcf);
    
    movIdx = find(dPCA_out{pIdx}.whichMarg==1);
    movIdx = movIdx(1:end);
    movMod = squeeze(mean(dPCA_out{pIdx}.Z(movIdx,:,:,1:80),4));
    %movMod = squeeze(mean(dPCA_out{pIdx}.Z(movIdx,:,:,150:200),4));
    ...
    for movCon=1:size(movMod,2)
        ...
        plot([0,movMod(1,movCon,1)], [0,movMod(2,movCon,1)], '-','Color',colors(movCon,:), 'LineWidth',1);
    ...
    figure
    for movAx=1:size(movMod,1)
        subplot(1,size(movMod,1),movAx);
        hold on;
        for movCon=1:size(movMod,2)
            plot(movMod(movAx,movCon,1), movMod(movAx,movCon,2), 'o','Color',colors(movCon,:),'MarkerFaceColor',colors(movCon,:));
        end
        xlim([-3,3]);
        ylim([-3,3]);
    end
