...
%shuffle null
nShuffles = 100;
cVar_shuff = zeros(length(codeList),nShuffles);
cVar_proj_shuff = zeros(length(codeList),nShuffles);
    
for shuffIdx = 1:nShuffles
    ...
    for pIdx = 1:length(codeSets)
        ...
        for codeIdx=1:length(codeSets{pIdx})
            ...
            if useNothing
                baselineWindowActivity = nothingDat_reduced(:,timeOffset+baselineWindow,:);
            ...
            cVar_shuff(codeSets{pIdx}(codeIdx),shuffIdx) = testStat;
          
            cVar_proj_shuff(codeSets{pIdx}(codeIdx),shuffIdx) = projStat_cv(dataMatrix(dataLabels==1,:), dataMatrix(dataLabels==2,:));
        end
    end    
end

save([outDir filesep 'shuffleControl'],'cVar_shuff','cVar_proj_shuff');
