...
text(0.15,1.5,'Go','FontSize',16);
text(1.65,1.5,'Return','FontSize',16);
...
for axType=1:4
    ...
    axIdx = find(dPCA_lat_eff.whichMarg==axType);
    axCoef = dPCA_lat_eff.W(:,axIdx(1));
    
    for c=1:size(fa,2)
        tmp = squeeze(fa(:,c,:))';
        projResult = tmp*axCoef;
        plot(projResult, lineArgs{c}{:});
    end
    axis tight;
    ylim([-2,2]);
end
