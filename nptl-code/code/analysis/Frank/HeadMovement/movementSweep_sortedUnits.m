...
%replace threshold crossing raster with sorted unit raster
packaged = load('/Users/frankwillett/Data/Derived/sortedUnits/t5.2017.10.16.2-21/packaged.mat');
R = rmfield(R,{'spikeRaster','spikeRaster2'});
R = addSortedRaster(R, packaged, blockList);
...
R_curs = rmfield(R_curs,{'spikeRaster','spikeRaster2'});
R_curs = addSortedRaster(R_curs, packaged, blockList);

allSpikes_curs = [R_curs.spikeRaster]';
...
usedUnits = find(~tooLow);
nUnits = length(usedUnits);
featLabels = cell(nUnits,1);
for f=1:length(chanIdx)
    globalUnitIdx = usedUnits(f);
    chanNum = (packaged.unitIdentity(globalUnitIdx,2)-1)*96 + packaged.unitIdentity(globalUnitIdx,3);
    unitNum = packaged.unitIdentity(globalUnitIdx,4);
    featLabels{f} = [num2str(chanNum) '-' num2str(unitNum)];
end
...
chanNum = (packaged.unitIdentity(:,2)-1)*96 + packaged.unitIdentity(:,3);
chanNum = chanNum(usedUnits);
cSNR_expand = nan(192,7);
for c=1:7
    for chanIdx = 1:192
        unitIdx = find(chanNum==chanIdx);
        if ~isempty(unitIdx)
            cSNR_expand(chanIdx,c) = max(cSNR(unitIdx,c));
        end
    end
end
...
saveas(gcf,[outDir filesep 'tuningHeatmap_sorted.png'],'png');
saveas(gcf,[outDir filesep 'tuningHeatmap_sorted.svg'],'svg');
