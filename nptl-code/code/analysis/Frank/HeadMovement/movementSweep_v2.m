...
datasets = {'t5.2017.10.16',[5 6],'head'
    ...
    't5.2017.10.16',[1],'cursor_OL'
    't5.2017.10.16',[2 3],'cursor_CL'};
...
%%
for d=12:length(datasets)
    saveableName = [strrep(datasets{d,1},'.','-')];
    outDir = [paths.dataPath filesep 'Derived' filesep 'MovementSweep' filesep datasets{d,3}];
    mkdir(outDir);
    ...
    if any(strcmp(datasets{d,3},{'cursor_OL','cursor_CL','cursor_all'}))
        R(1) = [];
        opts.filter = false;
        opts.useDecodeSpeed = true;
        data = unrollR_1ms(R, opts);
        
        trlCodes = data.targCodes;
        centerIdx = find(trlCodes==5);
        trlCodes(centerIdx) = trlCodes(centerIdx-1)+9;
    else
    ...
    if any(strcmp(datasets{d,3},{'cursor_OL','cursor_CL','cursor_all'}))
        ...
        if any(strcmp(datasets{d,3},{'arm','leg'}))
            timeWindow = [-1200 2500*2];
        else
            timeWindow = [-1200 1500*2];
        end
        alignField = 'goCue';
    end
    ...
    for t=1:length(R)
        ...
        blockRows = [blockRows; repmat(allBlocks(loopIdx(1)), size(newRow,1), 1)];
