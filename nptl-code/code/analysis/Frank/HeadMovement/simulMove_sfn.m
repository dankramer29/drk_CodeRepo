...
%           BI_DOWN_DOWN(210)
        ...
        effDim = dPCA_out_dual.W(:,axIdx(1:4));
        [ varianceSummary(1,1), varianceSummary(1,2:3) ] = getSimulVariance( alignDat.eventIdx(noSingleMovements), movCues(noSingleMovements), ...
        ...
        effDim = dPCA_out_dual.W(:,axIdx(1:4));
        movWindow = 10:50;
        [ varianceSummary(2,1), varianceSummary(2,2:3) ] = getSimulVariance( alignDat.eventIdx(noSingleMovements), movCues(noSingleMovements), ...
            alignDat.zScoreSpikes, effDim, 10:50, 100 );
        ...
        for setIdx=1:length(movSets)
            ...
            movWindow = 10:50;
            [ varianceSummary(setIdx+2,1), varianceSummary(setIdx+2,2:3) ] = getSimulVariance( alignDat.eventIdx(singleMovements), movCues(singleMovements), ...
                alignDat.zScoreSpikes, effDim, 10:50, 100 );
            disp(meanVar);
        end
        
        save([outDir filesep datasets{d,3}{outerMovSetIdx} '_varSummary'],varSummary);
        ...
        for decTypeIdx=1:3
            for setIdx=1:2
                ...
                elseif decTypeIdx==2
                    for movIdx=9:24
                        if setIdx==1
                            lsToUse = ls{dirFactorMap(movIdx,3)};
                            colorToUse = colors(dirFactorMap(movIdx,2),:);
                        else
