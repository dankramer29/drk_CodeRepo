...
for s=1:size(sessionList,1)
    ...
    tDir = in.targetPos(P.trl.reaches(:,1),:) - in.cursorPos(P.trl.reaches(:,1),:);
    tDir = bsxfun(@times, tDir, 1./matVecMag(tDir,2));
    theta = linspace(0,2*pi,9);
    theta = theta(1:8)';
    dirList = [cos(theta), sin(theta)];
    
    dirCode = zeros(size(tDir,1),1);
    for t=1:length(dirCode)
        err = matVecMag(bsxfun(@plus, dirList, -tDir(t,:)),2);
        [~,dirCode(t)] = min(err);
    end
    
    allCode = [dirCode, in.speedCode];
    %fastCode = find(in.speedCode==2);
    %dirCode(fastCode) = dirCode(fastCode)+8;
    
    margNames = {'Dir', 'Speed', 'CI', 'Dir x Speed'};
    out = apply_dPCA_simple( normFeatures, P.trl.reaches(:,1), allCode, [-50 100], 0.02, margNames );
     
    colors = hsv(8)*0.8;
    newLineArgs = cell(8,2);
    for c=1:8
        for f=1:2
            if f==2
                newLineArgs{c,f} = {'Color',colors(c,:),'LineWidth',2,'LineStyle',':'};
            else
                newLineArgs{c,f} = {'Color',colors(c,:),'LineWidth',2,'LineStyle','-'};
            end
        end
    end
    yAxes = twoFactor_dPCA_plot( out, (-50:100)*0.02, newLineArgs, margNames, 'sameAxes', [], [-2, 2] );
    
    %modScales = oneFactor_dPCA_plot_mag( out, (-50:100)*0.02, newLineArgs, {'CD','CI'} );
    datTest = randn(100,10);
    [COEFF, SCORE, LATENT, TSQUARED, EXPLAINED] = pca(datTest);
