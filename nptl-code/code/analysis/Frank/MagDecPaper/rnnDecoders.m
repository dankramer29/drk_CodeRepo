function funOut = rnnDecoders(in)
    ...
    for n=1:nFolds
        ...
        for p=1:length(popIdx)
            [inputs, targets, globalIdx] = formatInput(in.reachEpochs, innerTrainIdx, nBinsPerChunk, popModelOut{n}, in.kin.posErrForFit, popIdx{p});
            [inputsVal, targetsVal, globalIdxVal] = formatInput(in.reachEpochs, innerTestIdx, nBinsPerChunk, popModelOut{n}, in.kin.posErrForFit, popIdx{p});
            [inputsFinal, targetsFinal, globalIdxFinal] = formatInput(in.reachEpochs, testIdx, nBinsPerChunk, popModelOut{n}, in.kin.posErrForFit, popIdx{p});
