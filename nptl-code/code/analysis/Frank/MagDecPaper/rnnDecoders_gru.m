function funOut = rnnDecoders_gru(in)
    ...
    for p=1:length(popIdx)
        xValOut{p} = zeros(size(in.kin.posErrForFit,1),2);
    end
    
    paths = getFRWPaths();
    hfTmpPath = [paths.dataPath filesep 'Derived' filesep 'gruTest'];
    ...
    for n=1:nFolds
        ...
        [inputs, targets, globalIdx] = formatInput(in.reachEpochs, innerTrainIdx, nBinsPerChunk, in.features, in.kin.posErrForFit, 1:size(in.features,2));
        [inputsVal, targetsVal, globalIdxVal] = formatInput(in.reachEpochs, innerTestIdx, nBinsPerChunk, in.features, in.kin.posErrForFit, 1:size(in.features,2));
        [inputsFinal, targetsFinal, globalIdxFinal] = formatInput(in.reachEpochs, testIdx, nBinsPerChunk, in.features, in.kin.posErrForFit, 1:size(in.features,2));

        save([hfTmpPath filesep 'gruData.mat'],'inputs','targets','inputsVal','targetsVal','inputsFinal','targetsFinal');
        tic;
