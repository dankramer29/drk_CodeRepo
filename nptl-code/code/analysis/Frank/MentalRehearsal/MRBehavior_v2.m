...
for d=1:size(datasets,1)
    ...
    trlA = find(ismember(blockNums, [4 8]) & [allR.isSuccessful]' & goCue>1 & rtAdjusted<800 & rtAdjusted>200);
    trlI = find(ismember(blockNums, [6 9]) & [allR.isSuccessful]' & goCue>1);
    trlW = find(ismember(blockNums, [7 11]) & [allR.isSuccessful]' & goCue>1);
    trlMR = find(ismember(blockNums, [14 15]) & [allR.isSuccessful]' & outerIdx & isnan(rtSimple));
    allTrl = [trlA; trlI; trlMR; trlW];
    ...
    psthOpts.trialConditions = [trlCodes(trlA); trlCodes(trlI)+8; trlCodes(trlMR)+16; trlCodes(trlW)+24];
    ...
    %%
    %make movies
    movieBlocks = [4 5 6 7 8 9 11 14 15 16 17 19 22];
    %movieBlocks = 22;
    for x=1:length(movieBlocks)
        dataPath = [datDir 'RS_' strrep(datasets{d,1},'.','_') '_block' num2str(movieBlocks(x)) '.mat'];
        allDat = load(dataPath);
        smoothWidth = 0;
        datFields = {'windowsPC1GazePointValid','windowsPC1GazePoint','windowsPC1HeadRot',...
            'windowsMousePosition','cursorPosition','currentTarget','xk'};
        binMS = 20;
        for t=1:length(allDat.R)
            allDat.R(t).blockNum = movieBlocks(x);
        end
        ...
        if isempty(allDat.R)
            stream = allDat.stream.continuous;
            nBins = size(stream.cursorPosition,1);
            cursorXY = double(stream.cursorPosition(1:20:end,1:2));
            targXY = zeros(size(cursorXY,1),2);
            cursorColor = [255,255,255]/255;
            targColor = [108,108,108]/255;
            targRad = repmat(100,nBins,1);
            cursorRad = repmat(45/2,nBins,1);
            extraCursors = cell(2,3);
            extraCursors{1,1} = double(stream.windowsMousePosition(1:20:end,1:2))*1080;
            extraCursors{1,1}(:,2) = -extraCursors{1,1}(:,2);
            extraCursors{1,2} = 45/2;
            extraCursors{1,3} = [0 208 108]/255;
            extraCursors{2,1} = bsxfun(@times, double(stream.windowsPC1GazePoint(1:20:end,1:2)),[1920, 1080]);
            extraCursors{2,1} = bsxfun(@plus, extraCursors{2,1},-[1920, 1080]/2);
            [B,A] = butter(4,6/25);
            extraCursors{2,1} = filtfilt(B,A,extraCursors{2,1}); %weird high frequency noise today
            extraCursors{2,1}(~stream.windowsPC1GazePointValid(1:20:end),:) = -5000;
            extraCursors{2,2} = 45/2;
            extraCursors{2,3} = [208 108 108]/255;
        else
            unrollDat = binAndUnrollR( allDat.R, binMS, smoothWidth, datFields );

            nBins = size(unrollDat.cursorPosition,1);
            cursorXY = unrollDat.cursorPosition(:,1:2);
            targXY = unrollDat.currentTarget(:,1:2);
            cursorColor = [255,255,255]/255;
            targColor = [108,108,108]/255;
            targRad = repmat(allDat.R(end).startTrialParams.targetDiameter/2,nBins,1);
            cursorRad = repmat(45/2,nBins,1);
            extraCursors = cell(2,3);
            extraCursors{1,1} = unrollDat.windowsMousePosition(:,1:2)*1080;
            extraCursors{1,1}(:,2) = -extraCursors{1,1}(:,2);
            extraCursors{1,2} = 45/2;
            extraCursors{1,3} = [0 208 108]/255;
            extraCursors{2,1} = bsxfun(@times, unrollDat.windowsPC1GazePoint(:,1:2),[1920, 1080]);
            extraCursors{2,1} = bsxfun(@plus, extraCursors{2,1},-[1920, 1080]/2);
            [B,A] = butter(4,6/25);
            extraCursors{2,1} = filtfilt(B,A,extraCursors{2,1}); %weird high frequency noise today
            extraCursors{2,1}(~unrollDat.windowsPC1GazePointValid,:) = -5000;
            extraCursors{2,2} = 45/2;
            extraCursors{2,3} = [208 108 108]/255;
        end
        ...
        extraCursors{2,1}(:,2) = -extraCursors{2,1}(:,2);
         ...
         writeMpegMovie( M, [outDir filesep 'movie_' num2str(movieBlocks(x))], 50 );
