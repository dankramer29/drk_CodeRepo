datasets = {'t5.2018.01.22',{[16 18 20 22],[17 19 21 23],[16 18 20 22 17 19 21 23],[24 25 26 27 28 29]},{'vertP','vertG','vertAll','3ring'};
    't5.2018.01.24',{[11 12 13 14]},{'horzG'}};
...
for d=1:length(datasets)
    ...
    twSet = {[-500,500],[-740,740],[0 1000]};
    pfSet = {'goCue','moveOnset','goCuePost'};
    ...
    for alignSetIdx=1:length(afSet)
        ...
        if strcmp(pfSet{alignSetIdx},'goCue')
            ...
            %cartesian PCA
            for blockSetIdx = 1:3
                validIdx = find(prepTable(:,3)==1 & prepTable(:,7)==blockSetIdx & prepTable(:,8)==1);
                [COEFF, SCORE, LATENT, TSQUARED, EXPLAINED, MU] = pca(prepNeuralTable(validIdx,:));

                topN = 4;
                coef_pca = buildLinFilts(prepTable(validIdx,5:6), [ones(length(validIdx),1), SCORE(:,1:topN)], 'standard');
                predVals_avg = [ones(length(validIdx),1), SCORE(:,1:topN)]*coef_pca;
                ...
                saveas(gcf,[outDir filesep '2dprep_means_pca_' num2str(blockSetIdx) '.png'],'png');
                saveas(gcf,[outDir filesep '2dprep_means_pca_' num2str(blockSetIdx) '.svg'],'svg');
                ...
                saveas(gcf,[outDir filesep '2dprep_dist_pca_' num2str(blockSetIdx) '.png'],'png');
                saveas(gcf,[outDir filesep '2dprep_dist_pca_' num2str(blockSetIdx) '.svg'],'svg');
            end
