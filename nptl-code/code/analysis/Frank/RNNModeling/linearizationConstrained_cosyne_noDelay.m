...
controller = load('/Users/frankwillett/Data/Derived/linearization/controllerLin4.mat');
...
timeIdx = 100:199;
esNorm = (right_noDelay.envState - controller.armStateZM)./controller.armStateZS;
rnnStateConcat = [squeeze(right_noDelay.rnnState_0), squeeze(right_noDelay.rnnState_1)];
musOutput = (controller.C*cTraj' + controller.d)';
musOutput_original = right_noDelay.controllerOutputs(100:200,:);
musAct = aTraj(:,5:end);

figure
hold on
plot(aTraj((delayBuffer+1):end,1:4));
plot(right_noDelay.envState(timeIdx,stateVarIdx(1:4)),'--');

figure
hold on
plot(cTraj((delayBuffer+1):end,:));
plot((rnnStateConcat(timeIdx,:)-controller.PC_mean)*controller.PC_coeff','--');
...
plot(right_noDelay.envState(timeIdx,47:48),'--','LineWidth',2);
...
delayBuffer = 10;
aTraj = right_noDelay.envState(90:260,[1:4, musActIdx]);
taskInputs = (right_noDelay.controllerInputs(:,1:2) - controller.armStateZM(47:48))./controller.armStateZS(47:48);
taskInputs = [taskInputs(90:260,:), zeros(length(taskInputs(90:260,:)),1)];
...
for n=(delayBuffer+1):(nSteps+delayBuffer)
    ...
    controllerInput = [armState; taskInputs(n-4,:)'];
    cX = controller.A*cX + controller.B*controllerInput + controller.b;
end
...
for n=(delayBuffer+1):(nSteps+delayBuffer)
    ...
    controllerInput = [arm.C*aTraj(1,:)' + arm.d; taskInputs(n-4,:)'];
