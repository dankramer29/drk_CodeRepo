...
xlabel('Time (s)');
ylabel('Arm State');
legend(lHandles,{'ShoPos','ElbPos','ShoVel','ElbVel'},'FontSize',16);
...
%%
%inputs -> muscle outputs
stepInput = zeros(110,1);
stepInput(11:end) = 1;
in = {stepInput, armStateFull(:,velIdx)-armStateFull(1,velIdx), armStateFull(:,posIdx)-armStateFull(1,posIdx), zeros(size(stepInput)),  zeros(size(stepInput))};
out = {musAct_baseline-musAct_baseline(1,:), musAct_vel, musAct_pos, musAct, repmat(musAct_baseline(1,:),110,1)};

figure
for x=1:length(in)
    for colIdx=1:2
        subplot(length(in),2,(x-1)*2+colIdx);
        hold on;
        ...
        plot(timeAxis, toPlot, 'LineWidth', 4);
        axis tight;
...
exportPNGFigure(gcf, [saveDir filesep 'ControllerBreakout']);
...
%subplot(1,2,1);
hold on;
plot(timeAxis,musFlow_baseline(plotIdx,dimIdx)*100,'LineWidth',3);
plot(timeAxis,musFlow_vel(plotIdx,dimIdx)*100,'LineWidth',3);
plot(timeAxis,musFlow_pos(plotIdx,dimIdx)*100-musFlow_cursorPos(plotIdx,dimIdx)*100,'LineWidth',3);
plot(timeAxis,musFlow_cursorPos(plotIdx,dimIdx)*100,'LineWidth',3);
plot(timeAxis,musFlow_force(plotIdx,dimIdx)*100,'LineWidth',3);
axis tight;
ylabel('Joint Acceleration (Radians/s^2)');
xlabel('Time (s)');
legend({'Target','Vel','MusLen','CursorPos','Force'},'AutoUpdate','off','box','off');
plotBackgroundSignal(timeAxis, speedTraj(plotIdx));
set(gca,'FontSize',24,'LineWidth',2);
exportPNGFigure(gcf, [saveDir filesep 'ControlContributions']);

figure
timeAxis = (1:length(plotIdx))*0.01;

hold on;
plot(timeAxis,armVelAcc(plotIdx,dimIdx)*100,'LineWidth',3);
plot(timeAxis,armPosAcc(plotIdx,dimIdx)*100,'LineWidth',3);
plot(timeAxis,musFlow_total(plotIdx,dimIdx)*100,'LineWidth',3);
plot(timeAxis,musFlow_total(plotIdx,dimIdx)*100+armPosAcc(plotIdx,dimIdx)*100+armVelAcc(plotIdx,dimIdx)*100,'LineWidth',3);
axis tight;

set(gca,'FontSize',16);
legend({'Vel','Pos','Controller','Total'},'AutoUpdate','off','box','off');
plotBackgroundSignal(timeAxis, speedTraj(plotIdx));
plot(get(gca,'XLim'),[0 0],'--k','LineWidth',2);
set(gca,'FontSize',24,'LineWidth',2);
xlabel('Time (s)');
ylabel('Joint Acceleration (Radians/s^2)');
exportPNGFigure(gcf, [saveDir filesep 'ArmContributions']);
...
%segmented slides of arm flow
plotIdx = 25:10:65;
xAxis = linspace(-0.15,0.38,10);
yAxis = linspace(1.0,1.65,10);

figure('Color','w','Position',[187   644   717   424]);
for pIdx=1:length(plotIdx)
    subtightplot(2,3,pIdx,[0.04 0.04],[0.04 0.04],[0.04 0.04]);
    hold on;
    %axis off;
    ...
    lineColors = [0 0 0; lines(6)];
    
    plot(baseState(1), baseState(2), 'ro','LineWidth',2);
    jntTraj = plot(aTraj(1:80,1), aTraj(1:80,2),'LineWidth',4,'Color',lineColors(1,:));
%     plot([baseState(1), baseState(1)+velAcc(1)], [baseState(2), baseState(2)+velAcc(2)],'k','LineWidth',2);
%     plot([baseState(1), baseState(1)+musFlow(1)], [baseState(2), baseState(2)+musFlow(2)],'m','LineWidth',2);
%     plot([baseState(1), baseState(1)+currentPosFlow(1)], [baseState(2), baseState(2)+currentPosFlow(2)],'b','LineWidth',2);
%     plot([baseState(1), baseState(1)+totalFlow(1)], [baseState(2), baseState(2)+totalFlow(2)],'c','LineWidth',2);
...
%     plot([baseState(1), baseState(1)+musFlow_baseline(1)], [baseState(2), baseState(2)+musFlow_baseline(2)],'-om','LineWidth',2);
    ...
    armDamp = plot([baseState(1), baseState(1)+velAcc(1)], [baseState(2), baseState(2)+velAcc(2)],'LineWidth',4,'Color',lineColors(2,:));
    %totalMus = plot([baseState(1), baseState(1)+musFlow(1)], [baseState(2), baseState(2)+musFlow(2)],'m','LineWidth',2);
    armPos = plot([baseState(1), baseState(1)+currentPosFlow(1)], [baseState(2), baseState(2)+currentPosFlow(2)],'Color',lineColors(3,:),'LineWidth',4);

    musPos = plot([baseState(1), baseState(1)+musFlow_pos(1)], [baseState(2), baseState(2)+musFlow_pos(2)],'Color',lineColors(4,:),'LineWidth',4);
    musVel = plot([baseState(1), baseState(1)+musFlow_vel(1)], [baseState(2), baseState(2)+musFlow_vel(2)],'Color',lineColors(5,:),'LineWidth',4);
    %plot([baseState(1), baseState(1)+musFlow_force(1)], [baseState(2), baseState(2)+musFlow_force(2)],'-.m','LineWidth',2);
    musTarg = plot([baseState(1), baseState(1)+musFlow_baseline(1)], [baseState(2), baseState(2)+musFlow_baseline(2)],'Color',lineColors(6,:),'LineWidth',4);

    totalAcc = plot([baseState(1), baseState(1)+totalFlow(1)], [baseState(2), baseState(2)+totalFlow(2)],'Color',lineColors(7,:),'LineWidth',4);

    lHandles = [jntTraj, totalAcc, armPos, armDamp, musPos, musVel, musTarg];
    
    if pIdx==1
        legend(lHandles, {'Joint Traj','Total Acc','Arm Pos','Arm Vel','Controller Pos','Controller Vel','Controller Targ'}, ...
            'box','off','AutoUpdate','off','FontSize',14);
    end
    ...
    title(['Time = ' num2str(plotIdx(pIdx)/100) ' s'],'FontSize',18);
end
exportPNGFigure(gcf, [saveDir filesep 'ControlPolicySlides']);
...
for subsetIdx=1:length(subsets)
    ...
    for pIdx=1:length(plotIdx)
        ...
        if strcmp(subsets{subsetIdx},'ArmController')
            ...
            armAll = plot([baseState(1), baseState(1)+velAcc(1)+currentPosFlow(1)], [baseState(2), baseState(2)+velAcc(2)+currentPosFlow(2)],'k','LineWidth',4,'Color',lineColors(1,:));
            %armAll = quiver(baseState(1), baseState(2), velAcc(1)+currentPosFlow(1), velAcc(2)+currentPosFlow(2),0,'ko','LineWidth',2);
            %totalMus = plot([baseState(1), baseState(1)+musFlow(1)], [baseState(2), baseState(2)+musFlow(2)],'m','LineWidth',2);
            %armPos = plot([baseState(1), baseState(1)+currentPosFlow(1)], [baseState(2), baseState(2)+currentPosFlow(2)],'Color',[0 0.8 0],'LineWidth',2);

            musPos = plot([baseState(1), baseState(1)+musFlow_pos(1)], [baseState(2), baseState(2)+musFlow_pos(2)],'Color',lineColors(2,:),'LineWidth',4);
            musVel = plot([baseState(1), baseState(1)+musFlow_vel(1)], [baseState(2), baseState(2)+musFlow_vel(2)],'Color',lineColors(3,:),'LineWidth',4);
            %plot([baseState(1), baseState(1)+musFlow_force(1)], [baseState(2), baseState(2)+musFlow_force(2)],'-.m','LineWidth',2);
            musTarg = plot([baseState(1), baseState(1)+musFlow_baseline(1)], [baseState(2), baseState(2)+musFlow_baseline(2)],'Color',lineColors(4,:),'LineWidth',4);

            totalAcc = plot([baseState(1), baseState(1)+totalFlow(1)], [baseState(2), baseState(2)+totalFlow(2)],'Color',lineColors(5,:),'LineWidth',4);

            lHandles = [jntTraj, totalAcc, armAll, musPos, musVel, musTarg];
            legend(lHandles, {'Joint Traj','Total Acc','Arm','Controller Pos','Controller Vel','Controller Targ'}, ...
        ...
        axes(speedAx);
        hold on;
        plot(timeAxis, speedTraj, 'LineWidth', 2, 'Color', 'k');
