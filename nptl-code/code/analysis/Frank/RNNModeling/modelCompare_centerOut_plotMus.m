...
%         exportPNGFigure(gcf, [saveDir filesep signalNames{signalIdx} '_orthoSpace'])
    ...
    startIdx = plotStartIdx(1);
    loopIdx = startIdx + windowIdx + timeOffset;
    
    figure('Position',[46   909   473   196]);
    subplot(1,2,1);
    hold on;
    plot(timeAxis, envState(loopIdx,actIdx),'LineWidth',3);
    axis tight;
    legend(musNames);
    ylabel('Muscle Activation');
    xlabel('Time (s)');
    plot([0.25, 0.25],get(gca,'YLim'),'--k','LineWidth',2);
    ...
    hold on;
    handSpeed = [0; matVecMag(vel,2)];
    plot(timeAxis, handSpeed(loopIdx),'LineWidth',3);
    xlabel('Time (s)');
    ylabel('Speed (m/s)');
    axis tight;
    ...
    exportPNGFigure(gcf, [saveDir filesep 'TrajExample'])
    ...
    for x=1:length(plotStartIdx)
        ...
        plot(timeAxis, handSpeed(loopIdx),'LineWidth',2,'Color',colors(x,:));
        axis tight;
    end
    xlabel('Time (s)');
    ylabel('Speed (m/s)');
    set(gca,'LineWidth',2);
    set(gca,'FontSize',18);    
    exportPNGFigure(gcf, [saveDir filesep 'SpeedTraj'])
