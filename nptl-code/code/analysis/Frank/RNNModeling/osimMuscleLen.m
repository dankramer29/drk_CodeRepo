...
load('/Users/frankwillett/Documents/osimMuscleLenAndMoments.mat');
...
mn = mean(ln);
sd = std(ln);
inliers = ln<(mn+3.5*sd) & ln>(mn-3.5*sd);
ln = ln(inliers);
an = angles(inliers,:);
...
figure
hold on;
for t=(length(m1)-3000):length(m1)
    plot3(an(t,1), an(t,2), an(t,3), 'o', 'Color', colors(m1(t,:),:));
end
