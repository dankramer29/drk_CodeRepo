function [ negBitRate, succRate, acqTime ] = innerObjFunction_kb_2( simOpts, targetPos, startPos, targetIdx, keyCenters, dwellTime, coef )
    ...
    bitRate = log2(N-1)*max(sum(trialResults(:,1))-sum(~trialResults(:,1)),0)/totalTime;
    negBitRate = -bitRate;
    
    succRate = sum(trialResults(:,1))/nTargs;
    acqTime = mean(trialResults(:,2))/50;
end
