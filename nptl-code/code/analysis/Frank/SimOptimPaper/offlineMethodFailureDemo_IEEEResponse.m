...
for outerParamIdx = 3:size(params,1)
    %%
    figSuffix = ['_' num2str(params{outerParamIdx,1}) '_' params{outerParamIdx,2}];
    ...
    opts.trlCon.gen.tmOpts.targRad = params{outerParamIdx,1}; %1 and 4
    ...
    %%
    if strcmp(params{outerParamIdx,2},'refit')
        calVec = olVelXY;
    elseif ~isempty(strfind(params{outerParamIdx,2},'PLM'))
        calVec = movCommand_ol*plmGain;
    end
    ...
    decoder_ol = buildTopNDecoderKalman(simAct_ol, calVec, size(simAct_ol,2), 'FitA');
    ...
    elseif ~isempty(strfind(params{outerParamIdx,2},'PLM'))
        refitVel = res_ol.movCommand*plmGain;
    end
    ...
    for n=1:nRep
        ...
        elseif ~isempty(strfind(params{outerParamIdx,2},'PLM'))
            refitVel = resLast.movCommand*plmGain;
        end
    ...
    colorbar;
    set(gca,'FontSize',14);
    exportPNGFigure(gcf, [resultsDir filesep 'IEEE_offlineFailDemo' filesep 'onlineSurface' figSuffix]);
    ...
    plot(clBeta_i, clAlpha_i, '-ro', 'LineWidth', 2, 'MarkerSize', 6);
    plot(betaIdx, alphaIdx, 'wx', 'LineWidth', 2, 'MarkerSize', 10);
    exportPNGFigure(gcf, [resultsDir filesep 'IEEE_offlineFailDemo' filesep 'offlineSurface' figSuffix]);
    ...
    exportPNGFigure(gcf, [resultsDir filesep 'IEEE_offlineFailDemo' filesep 'traj' figSuffix]);

    %%
    save([resultsDir filesep 'IEEE_offlineFailDemo' filesep 'exampleData' figSuffix]);
end
