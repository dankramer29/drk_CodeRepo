function [ R, streams ] = getStanfordRAndStream( sessionPath, blockNums, rmsThresh, anchorRMSBlockNum, filtOpts )
    ...
    for b=1:length(blockNums)
        ...
        if ~isempty(filtOpts)
            for f=1:length(filtOpts.filtFields)
                ...
                streams{b}.continuous.([filtOpts.filtFields{f} '_speed']) = matVecMag(diff(streams{b}.continuous.(filtOpts.filtFields{f})(:,1:2)),2);
            end
        end
