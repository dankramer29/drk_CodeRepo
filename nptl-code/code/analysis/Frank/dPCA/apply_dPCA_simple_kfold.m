function out = apply_dPCA_simple_kfold( features, eventIdx, trialCodes, timeWindow, timeStep, margNames, maxDim, CIMode )
    %%
    ...
    elseif strcmp(CIMode,'xval') 
        ...
        for x=1:length(trialCodes)
            ...
            Cnoise = dpca_getNoiseCovariance(featureAverages_leaveOut, ...
                featureVals_leaveOut, trialNum_leaveOut, 'simultaneous', ifSimultaneousRecording);

            [W,V,whichMarg] = dpca(featureAverages_leaveOut, maxDim, ...
                'combinedParams', combinedParams, ...
                'lambda', optimalLambda, ...
                'Cnoise', Cnoise);
        ...
        Z_trialOrder = nan(nTrials,nDim,size(out.featureAverages,3));
        ...
        elseif nFactors==2
            ...
            Z_singleTrial = nan(maxRep,size(out.featureAverages,2),size(out.featureAverages,3),size(out.featureAverages,4));
