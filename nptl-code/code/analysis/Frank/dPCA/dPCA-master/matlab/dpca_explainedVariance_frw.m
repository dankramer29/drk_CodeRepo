function explVar = dpca_explainedVariance_frw(Xfull, W, V, varargin)
...
for i=1:length(XmargsValid)
    ZZ = V(:,1:min(8,size(V,2)))*(W(:,1:min(8,size(V,2)))'*XmargsValid{i});
    explVar.totalMarginalizedVar_firstEight(i) = nansum(ZZ(:).^2);
end
...
Z = W'*XValid;
for i=1:size(W,2)
    explVar.cumulativeDPCA(i) = 100 - sum(sum((XValid - V(:,1:i)*Z(1:i,:)).^2)) / explVar.totalVar * 100;    
    explVar.componentVar(i) = 100 - sum(sum((XValid - V(:,i)*Z(i,:)).^2)) / explVar.totalVar * 100;    
