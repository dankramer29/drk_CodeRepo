function [W, V, whichMarg, Xmargs] = dpca_frw(Xfull, numComps, varargin)
...
for i=1:length(Xmargs)
    ...
    try
        C = Xmargs{i}(:,validColumns)*XValid'/(XValid*XValid' + options.Cnoise + (totalVar*thisLambda)^2*eye(size(XValid,1)));
    catch exception
