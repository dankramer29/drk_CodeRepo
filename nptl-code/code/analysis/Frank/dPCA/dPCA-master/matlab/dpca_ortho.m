function [W, V, whichMarg, Xmargs] = dpca_ortho(Xfull, numComps, varargin)
    ...
    for i=1:length(Xmargs)
        ...
        %orthonormal vectors (within a marginalization). To do this in a way that orders the vectors by
        ...
        %and we also employ a reparameterization using the subspace matrix S so
        %that the next vector lies in a restricted subspace that is orthogonal
        %to all the previous vectors.

        allU = [];
        cXMarg = Xmargs{i}(:,validColumns);
        S = eye(size(X,1));

        for axIdx=1:nc
            disp(['dpca_ortho: marg ' num2str(i) ' ax ' num2str(axIdx)]);
            
            manifold = spherefactory(size(X,1)-axIdx+1);
            problem.M = manifold;

            problem.cost  = @(x) orthoCostFun(x, cXMarg, XValid, S);
            problem.egrad = @(x) orthoCostGrad(x, cXMarg, XValid, S);
            
            options.verbosity = 0;
            warning('off', 'manopt:getHessian:approx');
            
            [u, xcost, info] = trustregions(problem,[],options);

            u = S*u;
            allU = [allU, u];
            cXMarg = cXMarg - u*u'*XValid;
            S = orth(eye(size(XValid,1)) - allU*allU');
        end
