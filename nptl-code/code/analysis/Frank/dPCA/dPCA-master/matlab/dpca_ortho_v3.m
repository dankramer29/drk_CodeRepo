function [W, V, whichMarg] = dpca_ortho_v3(Xfull, numComps, varargin)
...
for i=1:length(Xmargs)
    ...
    for axIdx=1:nc
        ...
        problem.cost  = @(x) orthoCostFun(x, cXMarg, X, S);
        problem.egrad = @(x) orthoCostGrad(x, cXMarg, X, S);
