function Zfull_originalOrder = dpca_plot_frw(Xfull, W, V, plotFunction, varargin)
...
Zfull_originalOrder = reshape(Z', [size(Z,2) dataDim(2:end)]);
...
validRows = find(all(~isnan(Z),2));
a = corr(Z(validRows,1:numCompToShow));
%a = a*0;
