function dpca_plot_xval(Xfull, firingRates, numTrials, numComps, combinedParams, plotFunction, varargin)
...
%get cross-validated dimensions
nFolds = 10;
nt = numTrials(:,:);
nTrialTypes = size(nt,2);
trainSets = cell(length(nTrialTypes), 1);
testSets = cell(length(nTrialTypes), 1);
for t=1:nTrialTypes
    [ trainSets{t}, testSets{t} ] = makeXValSets( nt(1,t), nFolds );
end

%center the firing rates based on mean of trial-averages
X = Xfull(:,:)';
firingRates = bsxfun(@minus, firingRates, mean(X)');

%cross-validated dimensionality reduction
sf = size(firingRates);
Zpred = nan([numComps, sf(2:end)]);
Zdec = nan([numComps, sf(2:end)]);
if ndims(firingRates)==4
    for foldIdx = 1:nFolds
        fTmp = firingRates;
        for s1=1:size(firingRates,2)
            fTmp(:,s1,:,testSets{s1}{foldIdx}) = NaN;
        end
        fAvgInner = nanmean(fTmp,ndims(fTmp));

        [W,V,whichMarg] = dpca(fAvgInner, numComps, 'combinedParams', combinedParams, 'lambda', options.lambda, 'Cnoise', options.Cnoise);    
        for s1=1:size(firingRates,2)
            tmpRates = firingRates(:,s1,:,testSets{s1}{foldIdx});
            if ~isempty(tmpRates)
                for trlIdx=1:size(tmpRates,5)
                    Zpred(:,s1,:,testSets{s1}{foldIdx}(trlIdx)) = (squeeze(tmpRates(:,1,:,trlIdx))'*W)';
                    if ~isempty(options.decoder)
                        for compIdx = 1:size(W,2)
                            Zdec(compIdx,s1,:,testSets{s1}{foldIdx}(trlIdx)) = matVecMag(((squeeze(tmpRates(:,1,:,trlIdx))'*W(:,compIdx))*V(:,compIdx)')*options.decoder,2);
                        end
                    end
                end
            end
    ...
    for foldIdx = 1:nFolds
        fTmp = firingRates;
        for s1=1:size(firingRates,2)
            for s2=1:size(firingRates,3)
                tmpInd = sub2ind([size(firingRates,2), size(firingRates,3)], s1, s2);
                fTmp(:,s1,s2,:,testSets{tmpInd}{foldIdx}) = NaN;
            end
        end
        fAvgInner = nanmean(fTmp,5);

        [W,V,whichMarg] = dpca(fAvgInner, numComps, 'combinedParams', combinedParams, 'lambda', options.lambda, 'Cnoise', options.Cnoise);    
        for s1=1:size(firingRates,2)
            for s2=1:size(firingRates,3)
                tmpInd = sub2ind([size(firingRates,2), size(firingRates,3)], s1, s2);
                tmpRates = firingRates(:,s1,s2,:,testSets{tmpInd}{foldIdx});
                for trlIdx=1:size(tmpRates,5)
                    Zpred(:,s1,s2,:,testSets{tmpInd}{foldIdx}(trlIdx)) = (squeeze(tmpRates(:,1,1,:,trlIdx))'*W)';
                    if ~isempty(options.decoder)
                        for compIdx = 1:size(W,2)
                            Zdec(compIdx,s1,s2,:,testSets{tmpInd}{foldIdx}(trlIdx)) = ...
                                matVecMag(((squeeze(tmpRates(:,1,1,:,trlIdx))'*W(:,compIdx))*V(:,compIdx)')*options.decoder,2);
                        end
                    end
                end
            end
...
Z = nanmean(Zpred, ndims(Zpred));
Z = Z(:,:)';
X = Xfull(:,:)';
dataDim = size(Xfull);

% can't show more than there is
numCompToShow = min(options.numCompToShow, numComps);
