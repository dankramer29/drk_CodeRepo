function [yAxesFinal, allHandles, allYAxes] = nFactor_dPCA_plot( out, margOrder, timeAxis, lineArgs, margNamesShort, axisType, bgSignal, forceAxes )

    nMarg = length(margOrder);
    nPerMarg = 7;
        ...
        for p=1:nPerMarg
            ...
            for factor1 = 1:size(out.Z,2)
                for factor2 = 1:size(out.Z,3)
                    for factor3 = 1:size(out.Z,4)
                        plot(timeAxis, squeeze(out.Z(margIdx(p),factor1,factor2,factor3,:)),lineArgs{factor1,factor2,factor3}{:});
                        tmpIdx = tmpIdx + 1;
                    end
                end
            end
