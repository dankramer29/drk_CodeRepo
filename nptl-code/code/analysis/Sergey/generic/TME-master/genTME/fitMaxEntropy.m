...
if ~(sum((trSigma-mean(trSigma))>=-(sum(dim)*sqrt(eps)) & (trSigma-mean(trSigma))<=(sum(dim)*sqrt(eps)))==length(trSigma))
    if isfield( params, 'noTraceRequirement' ) && params.noTraceRequirement == true
        % SDS: For the LFP power, the traces were similar but slightly larger than epsilon. Maybe
        % has somethign to do with statistics/smoothness of LFP? For now I'd rather see the
        % results, so I'm allowing it to ignore this
        fprintf( 2, '[%s] Warning: marginal covariance traces are slightly unequal, which would error this code normally, but is being overwritten to ignore\n', mfilename ) 
    else
