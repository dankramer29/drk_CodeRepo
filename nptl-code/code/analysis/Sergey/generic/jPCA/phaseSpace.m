...
for pindex = 1:length(planes2plot)
    ...
    for c = 1:numConds  % cycle through conditions, and assign that condition's color
        ...
        % Sergey Stavisky: add capability to color base circle
        if isfield( params, 'circleColors' )
            circleColors{c} = params.circleColors{c};
        else
            circleColors{c} = [0 0 0]; % black
        end
    end
    ...
    for c = 1:numConds
        ...
        if ismember(c,conds2plot)
            ...
            if planMarkerSize>0
                plot(P1(1), P2(1), 'ko', 'markerSize', planMarkerSize, 'markerFaceColor', planMarkerColor{c}, 'markerEdgeColor', circleColors{c});
            end
