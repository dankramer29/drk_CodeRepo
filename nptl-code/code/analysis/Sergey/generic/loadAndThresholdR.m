function [R1 thresh]=loadAndThresholdR(participant,session,block, options)
% loadAndThresholdR.m
%
% Loads NPTL data and applies thresholding to get spikeRasters.
%
% Based off of Chethan's loadAndSmoothR.m
%
% USAGE: [R, thresh] = loadAndThresholdR(participant,session,block, thresh)
%
% EXAMPLE: [R, thresholds] = loadAndSmoothR( 't5', 't5.2016.09.14', 20, -3.5);
%
% INPUTS:
%   ...
%   options.thresh       if thresh is scalar, treats it as an rms multiplier. If vector,
%                        sets uV threshold values for each channel.
%           .useHLFP     whether to load high frequency LFP
%           .useLFPraw   whether to load raw LFP
% OUTPUTS: 
%   R1      Loaded R struct
%   thresh  Thresholds used for each channel
%
...
% Smoothing here applies only to HLFP. Spike trains are *not* smoothed, if
% you want that use loadAndSmoothR.m
...
streamfields={'minAcausSpikeBand','minAcausSpikeBand1','minAcausSpikeBand2','rasters','minSpikeBand'};
% if we need to remap the names
streamfieldsout={'minAcausSpikeBand','minAcausSpikeBand1','minAcausSpikeBand2','rasters', 'minAcausSpikeBand'};
...
if useLFPRaw
    ...
    %% apply thresholding to the streamed data    
    if length(thresh) == 1
        error('[%s]: threshold should be at least 96 channels', mfilename);
    end
%     spikeband = smoothStream(spikeband, thresh, gaussSD, useHalfGauss, normalizeKernelPeak);
    spikefield = 'minSpikeBand';   
    sf = spikeband.(spikefield);

    neuralChannels = 1:numel(thresh);
    rasters = false( size(sf) ); % preallocate
    for nc=neuralChannels
         rasters(:,nc)=sf(:,nc)<thresh(nc);
    end
    spikeband.rasters = rasters;
        
    if exist('lfpband','var')
        %% apply smoothing to the HLFP data
