...
% 
% Called by WORKUP_humanCursorPos.m
%
...
% 
% Created by Sergey Stavisky on 10 November 2016
function results = humanCursorPosOneDataset( dataset, params )
...
Rgrid(durationHeldBeforeSelected < params.minDurationHeldBeforeSelection)=[];
fprintf('Removed trials with duration held before selection < %gms. %i trials remain\n', ...
    params.minDurationHeldBeforeSelection, numel( Rgrid ) );

%% Grid Task hold firing rates
% get rasters from voltage values;
Rgrid = RastersFromMinAcausSpikeBand( Rgrid, thresholds );
Rgrid = AddFeature( Rgrid, 'spikesBinnedRate_1ms' ); % create the spiking feature
jenga = AlignedMultitrialDataMatrix( Rgrid, 'featureField', 'spikesBinnedRate_1ms' , ...
...
fighHists = figure;
figh.Name = sprintf('Workspace FR Diffs Histograms %s', dataset );
...
ExportFig( figh, [params.saveFiguresDir dataset '/Histograms/' titlestr]);
