...
saveResultsRoot = [ResultsRootNPTL '/speech/dPCA/']; 
if ~isdir( saveResultsRoot )
    mkdir( saveResultsRoot )
end

params.crossValidate = true; % as a santiy check; do dPCA on PSTHs from half the trials, then visualize the other half.
...
saveFilepath = sprintf('%scumvars-%s', saveResultsRoot, datasetName );
...
CIS1 = Vreordered(:,1);
CDdims = Vreordered(:,numel(CIdimsInds)+1:end);
...
titlestr = sprintf('dPC correlations %s', datasetName);
