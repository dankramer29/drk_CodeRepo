...
% identical to the one in the CIS figure (except if using different smoothing). The jPC dimensions will differ anyway, so I
...
params.neuralFeature = 'spikesBinnedRateGaussian_30ms'; % spike counts binned smoothed with 30 ms SD Gaussian as in Kaufman 2016
...
params.vidAlignEvent = 'handPreResponseBeep';
params.vidStartEvent = 'handPreResponseBeep - 2.500'; % 500 ms before audio cue
params.vidEndEvent= 'handPreResponseBeep + 2.000'; % ~ 1000 ms after VOT

params.vidEventString = 'Go'; % gets added to clock in corner
% ----------------------------
...
% align to GO when things start happening
% params.jPCAalignEvent = 'handPreResponseBeep';
% params.jPCAstartEvent = 'handPreResponseBeep + 0.899'; % 1 ms buffer to make sure exact ms is available
% params.jPCAendEvent = 'handPreResponseBeep + 1.401';
...
viz.cam.pauseAndRotate = [...
       1, 0.5,  5, 10; % brief pause 
       1, 2.0, 80, 15; % swing to show CIS1
       1, 0.5, 80, 15; % brief pause
       2, 2.0, 5, 10; % rotate to focus on CIS1
       2, 0.75, 5, 10; % brief pause
       2, 1.25, 45, 30; % rotate back to iso view
       ];
...
% operation above.
% CDproj = Wreordered(:,numCIdims+1:end);
...
% nspace = null( Wreordered(:,1)' ); % try null space of CIS1  % old way, on W...
nspace = null( Vreordered(:,1)' ); % try null space of CIS1 % should be in V 
...
% axTime = axes( figh, 'OuterPosition', [0, 0.9, 0.1, 0.1], 'Color', 'r');
axTime = axes( figh, 'OuterPosition', [0.05, 0.8, 0.1, 0.1], 'Color', 'r'); % allows for closer cropping after

axTime.Visible = 'off';
hTime = text( 0, 1, sprintf('%s%+.1fs', params.vidEventString, tVid(samplesEachFrame) ), 'FontSize', viz.FontSize, ...
...
% for iFrame = 200 : totFrames % note: these are neural trajectory frames, camera rotation pauses don't count here
...
for iFrame = 1 : totFrames % note: these are neural trajectory frames, camera rotation pauses don't count here
    ...
    for iLabel = 1 : numel( uniqueLabels )                    
        % Plot baseline
        endBaseline = min( sampleAudioCue-1, mySample );
        ...
        % Plot delay
        if mySample >= sampleAudioCue
            endDelay = min( sampleGoCue-1, mySample );
            x = allVidXYZ(sampleAudioCue:endDelay,1,iLabel);
            y = allVidXYZ(sampleAudioCue:endDelay,2,iLabel);
            z = allVidXYZ(sampleAudioCue:endDelay,3,iLabel);
            lh(end+1) = plot3( x, y, z, 'Color', viz.colorDelay, 'LineWidth', viz.LineWidthDelay );
        end
           
        % Plot speech
        if mySample >= sampleGoCue
            endSpeech = min( numel( tVid ), mySample );
            x = allVidXYZ(sampleGoCue:endSpeech,1,iLabel);
            y = allVidXYZ(sampleGoCue:endSpeech,2,iLabel);
            z = allVidXYZ(sampleGoCue:endSpeech,3,iLabel);            
