...
trialCounts = zeros(max(data.tgt), max(data.uCoh));
% nan buffer and fill in psthes: 
PSTH = nan(max(data.tgt), length(uCohIdx), );
for trial = 1:length(data.stimOnset)
   PSTH(data.tgt(trial), find(uCohIdx == data.uCoh(trial))) = 
end
for target = 1:size(trialCounts,1)
    for coh = 1:size(trialCounts, 2)
