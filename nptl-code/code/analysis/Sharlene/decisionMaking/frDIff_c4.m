function binnedR_All = frDiff_c4(binnedR_All)
% to be called after aggC4 has built the aggRc4 struct and binnedR_All is
% assigned one session's struct (ie aggRc4(session).HM or .BC)
% stimulus onset = target changes from red to blue
% movement onset = when the cursor or head mouse exceeds the speed threshold (5% of maximum)
%%
%count = count + 1;
...
%non-center targets: This is the end of the delay phase: 
tgtTrialStartIdx = find(abs([0;diff(binnedR_All.contTarg)]) & (binnedR_All.contTarg < 5)); 
testSpikes = binnedR_All.zSpikes;
%% nan-out all of the movement data for each trial. So between trialStartIdx and speedMO
% speedMO = reaches 5% of max movement 
% moveOnset = when the target switches from center to other, or other to
% center
for trial = 1:sum(~isnan(binnedR_All.moveOnset))
...
for trial = 1:sum(~isnan(binnedR_All.moveOnset))
   ...
   % psth(trial,:,:) = binnedR_All.zSpikes(binnedR_All.stimOnset(trial)+windowEarly:binnedR_All.stimOnset(trial)+windowLate, unitsIdx).*50; %in hz
    psth(trial,:,:) = testSpikes(binnedR_All.moveOnset(trial)+windowEarly:binnedR_All.moveOnset(trial)+windowLate, unitsIdx).*50; %in hz
...
%%
binnedR_All.rt = [binnedR_All.speedMO' - [0;trialStartIdx]]';
rt = binnedR_All.rt; 
rtBins = [3000:2000:15000]./20; %ms divided by 20 ms per bin
...
for rtIdx = rtBins
    rtCount = rtCount + 1;
    if rtIdx < min(rtBins)
        rIdx = rt < rtIdx & (rt < rtIdx+(5000/20)); 
    elseif rtIdx < max(rtBins)
        rIdx = (rt >= rtIdx) & (rt < rtIdx+(5000/20)); 
    else
        rIdx = rt >= rtIdx;
    end
    % psth is in size(trials, units?, bins on either side of the alignment window
    % we need to only fill in data from pre:rt . rt is also in bins, right? 
    LFR_rt(rtCount,:,:) = squeeze(nanmean(psth(rIdx' & (tgt == 3),:,:),1)); %average over trials of the same coh and targ
    RFR_rt(rtCount,:,:) = squeeze(nanmean(psth(rIdx' & (tgt == 4),:,:),1));
    UFR_rt(rtCount,:,:) = squeeze(nanmean(psth(rIdx' & (tgt == 1),:,:),1));
    DFR_rt(rtCount,:,:) = squeeze(nanmean(psth(rIdx' & (tgt == 2),:,:),1));
