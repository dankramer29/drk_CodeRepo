...
for sesh = 1 %:length(mcBlocks) 
    streams = [];
    [ ~, streams] = getStanfordRAndStream_SF( ['Users/sharlene/CachedData/t5.', mcDates{sesh}, '/'], mcBlocks{sesh}, -3.5, mcBlocks{sesh}(1), filtOpts);
    %binnedR(sesh).HM = [];
...
%tgt = binnedR_All.tgt; 
figure;
 numTrials = length(binnedR_All.clickType); 
...
for i = 1 :numReps
    ...
    Mdl_targ = fitcnb(table_All, classLabelTarg, 'Prior' , [0.5 0.125 0.125 0.125 0.125]);
% test decoder 
    tcount = 0;
