...
for cIdx = 1:length(usedCoh)
    numCorr(cIdx) = sum([R([R.coherence] == usedCoh(cIdx)).isSuccessful]); 
    outOf(cIdx) = sum([R.coherence] == usedCoh(cIdx)); 
...
[paramValues, LL, exitFlag, output] = PAL_PFML_Fit(usedCoh, numCorr, outOf,[0 0 .5 .01], [1 1 0 1], @PAL_Weibull)
x = 0:.5:max(usedCoh);
