
// Windows Instructions: 
//      For mex compilation on linux: mex mouseController.cpp -lXtst -lX11



#include "math.h"
#include "mex.h"   //--This one is required

#include<pthread.h>
#include <unistd.h>
#include<X11/Xlib.h>
#include<X11/extensions/XTest.h>
#include <stdio.h>

// 
// #include<string.h>
// #include <stdint.h>
// #include <stdlib.h>
#include <ctype.h>
// 
// 
// #include <arpa/inet.h>
#include <inttypes.h>
// #include <netdb.h>
// #include <sys/types.h>
// #include <sys/socket.h>
// #include <netinet/in.h>
// #include <arpa/inet.h>
// #include <fcntl.h>
// #define nonblockingsocket(s)  fcntl(s,F_SETFL,O_NONBLOCK)
// #define Sleep(a) usleep(a*1000)
// #endif


///////////// DATA TYPES DECLARATIONS /////////////
typedef uint8_t uint8;
typedef uint16_t uint16;
typedef uint32_t uint32;
typedef unsigned long long uint64;


struct jnk {
    uint32 x;
    uint32 y;
} packetSwitch;

///////////// GLOBALS /////////////

Display *dpy;

int mex_call_counter = 0; 

#define MOUSE_CLICK_BUTTON 1



int myoptstrcmp(const char *s1,const char *s2)
{
    int val;
    while( (val= toupper(*s1) - toupper(*s2))==0 ){
        if(*s1==0 || *s2==0) return 0;
        s1++;
        s2++;
        while(*s1=='_') s1++;
        while(*s2=='_') s2++;
    }
    return val;
}


void diep(const char *s)
{
  mexPrintf("Error with sockets: %s \n", s);
  perror(s);
  //exit(1);
}


// move cursor to position x and y of display dpy
int setCurrentPosition(uint32 x, uint32 y) {
    return XTestFakeMotionEvent(dpy,0,x,y,0);
}

// sends click state of mouse button
int doClickMain(uint32 down) {
    return XTestFakeButtonEvent(dpy, MOUSE_CLICK_BUTTON, (bool) down, 0);
}



void mexFunction(
        int           nlhs,           /* number of expected outputs */
        mxArray       *plhs[],        /* array of pointers to output arguments */
        int           nrhs,           /* number of inputs */
        const mxArray *prhs[]         /* array of pointers to input arguments */
        )
{
    char fun[80+1];
    uint32 *x, *y; 
    
    //mexPrintf("ENTER_MEX with %d arguments\n", nrhs);
    
    if(mex_call_counter==0)
    {
        mex_call_counter++;
    }
    
    if((nrhs == 3) && mxIsChar(prhs[0])){
        // GET FIRST ARGUMENT -- The "function" name 
        mxGetString(prhs[0],fun,80);
        
        
        if(myoptstrcmp(fun,"SET")==0){
            dpy = XOpenDisplay(NULL);
            x = (uint32 *) mxGetData(prhs[1]);
            y = (uint32 *) mxGetData(prhs[2]);
//            mexPrintf("x%d, y%d\n",*x,*y);
            if (!setCurrentPosition(*x,*y)) {
                mexPrintf("Problem Setting Position!\n");
            }
            XCloseDisplay(dpy);
        }
        else
            mexPrintf("Invalid Syntax!!\n");
    }
    else if((nrhs == 2) && mxIsChar(prhs[0])){
        // GET FIRST ARGUMENT -- The "function" name
        mxGetString(prhs[0],fun,80);
        
        if(myoptstrcmp(fun,"CLICK_MAIN")==0){
            dpy = XOpenDisplay(NULL);
            x = (uint32 *) mxGetData(prhs[1]);
            if (!doClickMain(*x)) {
                mexPrintf("Problem Clicking!\n");
            }
            XCloseDisplay(dpy);
        }
        else
            mexPrintf("Invalid Syntax!!\n");
    }
    else if((nrhs == 1) && (nlhs == 0) && (mxGetClassID(prhs[0]) == mxUINT32_CLASS))
    {
        
    }
    else
        mexPrintf("Invalid Syntax!!\n"); 
    
    return;
    }
