function outvar = createSimulinkStructure(s)

f = fields(s);
sout = struct();
for nn = 1:length(f)
    if isstruct([s.(f{nn})])
        newfield = createSimulinkStructure([s.(f{nn})]);
        sout.(f{nn}) = newfield;
    else
        sout.(f{nn}) = [s.(f{nn})];
    end
end
outvar = Simulink.Parameter;
outvar.Value = sout;
outvar.CoderInfo.StorageClass = 'ExportedGlobal';
...
%     set_param([sys '/' varargin{nn}],'Value',varargin{nn});
%     y = y+offset;
%     
...
%     evalin('base', [varargin{nn} ' = createTunableParameter(tmpvar, ''' ...
%         vclass ''');' ]);
% %     parameter(numElements).signal.name = varargin{nn};
% %     parameter(numElements).signal.value = varargin{nn+1};
% %     parameter(numElements).signal.class = vclass;
% %     portHandles(numElements) = get_param([sys '/' varargin{nn}], 'PortHandles');
% end
% 
% 
