...
cpDiff = cursorPosition(:, 1 : end - 1) - cursorPosition(:, 2 : end);
% updateTimes = find(cpDiff(1, 2 : end));
updateTimes=[];
updateRate=[];
for nx=1:size(cpDiff,1)
    updateTimes=find(cpDiff(nx, 2 : end)~=0);
    localRates = updateTimes(2:end) - updateTimes(1:end-1);

    if ~isempty(localRates)
        %updateRate = [updateRate(:); mean(localRates)];
        updateRate = [updateRate(:); median(localRates)];
    end
end
updateRate=mean(updateRate);%
 % if ~isfield(startTrialParams, 'decodeOn') || (~(startTrialParams.decodeOn) && abs(updateRate - 16) < 2)
