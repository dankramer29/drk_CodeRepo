%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Sergey Stavisky August 2016
% now will work in higher dimensions.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function  model = fitKalmanV( data, channels, A, W, opts)
  ...
  for n = 1 : length(data)
      ...
      if ~any(isnan(allx(:)))
          ...
          ZXt = ZXt + data(n).Z*data(n).X(idxUse, :)';
      end
  end
  tmpC = ZXt/XXt; % note matrix right division
  model.C = zeros(P, 2*numDims+1);
  model.C(:,oneDimReduced) = mean(data(n).Z, 2); %This is overwritten from tmpC, but they're idential
