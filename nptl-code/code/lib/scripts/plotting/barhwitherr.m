%**************************************************************************
%% CP: modification of "barwitherr" to do horizontal error bars
%
...
if nRows > 1
    ...
    for col = 1:nCols
        ...
        x = get(get(handles.bar(col),'children'),'ydata');
        ...
        hErrorbar(col,:) = herrorbar(values(xOrder,col), mean(x,1), lowerErrors(xOrder,col), upperErrors(xOrder, col), '.k');
    ...
    x = get(get(handles.bar,'children'),'ydata');
    hErrorbar = herrorbar(values, mean(x,1), lowerErrors, upperErrors, '.k');
