...
modelConstants.replayTrialsDir = 'Data/Structs/ReplayTrials/';
...
% SDS PN March 2017. Was 1400, now raising because we're going multipacket
modelConstants.network.maxUDPDataSize = [1 1400]; % for peripherals, etc
modelConstants.network.maxFileLoggerUDPDataSize = [1 1400];
% in EM code throughout the model, look for MAX_PACKET_LENGTH and change it
% to match the second element of modelConstants.network.maxUDPDataSize
...
% this sets the neural data buffer, especially inportant with multiple
% arrays / variable packets per ms
% sampleBufferSize - this is the total buffer size. any size is reasonable
%    here within memory constraints
modelConstants.cerebus.sampleBufferSize = 240;
% sampleBufferDelay - how much neural data do we spool up before pulling
%   from the buffer
modelConstants.cerebus.sampleBufferDelay = 90;
...
modelConstants.windowsPC1.ip = [192 168 30 1];
modelConstants.windowsPC1.DATA_DEST_PORT = 50141;

modelConstants.mouseData.ip = [192 168 30 1];
modelConstants.mouseData.DATA_DEST_PORT = 50142;