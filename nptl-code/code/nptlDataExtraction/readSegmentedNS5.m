function varargout = readSegmentedNS5(fname,segment,channels,dstart,dend,varargin)
%%
%% function varargout = readSegmentedNS5(fname,segment,channels,dstart,dend,varargin)
%
% Modified from openNSx v 5.1.6.0 to read a specific segment from NS5 files that have
%   "pause blocks" (which happens when multiple arrays are being
%   synced).
% Very limited functionality in comparison to openNSx (use only the
%   named arguments)
%
% Chethan Pandarinath, 2013,2014,2015
1;
if exist('varargin','var') & ~isempty(varargin)
    disp(['readSegmentedNS5: Warning- you are passing in arguments ' ...
          'via varargin that are being ignored.']);
end
...
if length(NSx.MetaTags.DataPoints) > 1
    ...
    %% added by Chethan to force this to be read as a pausedfile    
    NSx.RawData.PausedFile = 1;
    disp('warning - this doesnt appear to actually be a pausedfile, but treating it as such');
end
...
%     end
% end
userRequestedChannels = channels;
...
if strcmp(ReadData, 'read')
    ...
    if NSx.RawData.PausedFile
        for dataIDX = segment
%% go to the beginning of this data segment
            fseek(FID, f.BOData(dataIDX), 'bof');

%% added by chethan - skip over the first dstart time points
            % Skip the file to the beginning of the time requsted, if not 0
            fseek(FID, dstart * 2 * ChannelCount, 'cof');

%% go to the right spot for the channel(s) requested
            % Skip the file to the first channel to read
            fseek(FID, (find(NSx.MetaTags.ChannelID == min(userRequestedChannels))-1) * 2, 'cof');        

%% modified by chethan - only read the first (dend-dstart) packets
            NSx.Data{dataIDX} = fread(FID, [numChansToRead (dend-dstart)], [num2str(numChansToRead) precisionType], double((ChannelCount-numChansToRead)*2 + ChannelCount*(skipFactor-1)*2));
