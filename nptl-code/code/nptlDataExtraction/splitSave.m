...
%	maxVarSize = 2147483648; % 2GB
	...
	% quickly check that each individual slice fits under the limit
	chunksTooBig = false;
	slices = [ 0 : elementsPerSlice : numElements ];
	for i = 1 : numSlices
	  splitData = inArray(slices(i) + 1 : slices(i + 1));
	  whosSplitPiece = whos('splitData');
	  if whosSplitPiece.bytes >= maxVarSize
	    chunksTooBig = true;
	  end
	end
	if chunksTooBig
	  fprintf(['splitSave: WARNING - variability in individual chunks means some are too big. need to resize' ...
		   ' further.\n']);
	  elementsPerSlice = ceil(elementsPerSlice / 2);
	end
	...
	if elementsPerSlice < 1
		fprintf(['WARNING: array of structures too large, cannot slice small enough, saving as matlab v7.3\' ...
                         'n']);
                splitData = inArray;
		save( fullfile(saveName, sprintf('%04d.mat', 1)), 'splitData');
	else
