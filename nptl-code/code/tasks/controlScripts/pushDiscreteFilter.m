function pushDiscreteFilter()
    ...
    function pushChosenFilter(hOjb, event)
        loadedModel = load(fullfile(modelConstants.sessionRoot,modelConstants.discreteFilterDir, chosenFilter));
        loadedModel.discretemodel.filterName = zeros([100 1],'uint8');
        tmp = length([chosenFilter '.mat']);
        loadedModel.discretemodel.filterName(1:tmp) = uint8([chosenFilter '.mat']);
