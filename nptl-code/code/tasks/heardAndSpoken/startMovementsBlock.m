% Speaks and writes a log of MOVEMENT COMMANDS cues.
%
% Part of code for running minimal viable experiment to survey for neural tuning in motor
% cortex when a participant makes mouth movements.
%
...
% 
% Sergey D. Stavisky, October 2017, Stanford Neural Prosthetics Translational Laboratory.
...
repetitionsEachSound = 10; % how many repetitions of each sound

pauseAfterReadyMS = 400; % ms how long to pause after the ready cue before playing the sound
pauseAfterSoundPrompt = 100; % ms how long to pause after the sound prompt was played

pauseAfterMovePrompt = 1300; % ms how long to pause after the move prompt was played. 
                              % (this is basically from go cue to when return is commanded
pauseAfterReturnPrompt = 1200; % how long to pause after the return action is commanded                               

% Below is the list of sounds:
soundList = {...
    'beep'; % LISTEN CUE
    'metronomeShort'; % MOVE CUE
    'return'; % in second half of each trial
    'lipsBack';
    'lipsForward';
    'tongueUp';
    'tongueDown';
...
possibleSounds = soundList(4:end); % first 3 are the beep + chime + return
...
if fid < 1 
    ...
    fprintf( fid, 'pauseAfterMovePrompt, %i\n', pauseAfterMovePrompt );
    fprintf( fid, 'pauseAfterReturnPrompt, %i\n', pauseAfterReturnPrompt );
...
for iTrial = 1 : numel( promptList )    
   ...
   % Play the prompt
   if strcmp( mySound, '(silence)' )
       pause( silentPauseDuration/1000 )
   else
   ...
   sobjs.metronomeShort.playblocking
   pause( pauseAfterMovePrompt/1000 );   
   
   
   %% ----------------------
   %        Return
   %  ----------------------
   sobjs.return.playblocking
%    sobjs.metronomeShort.playblocking % no return tick
   pause( pauseAfterReturnPrompt/1000 );   
