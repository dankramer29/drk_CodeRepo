function [optAlpha, optBeta, simOut] = alphaBetaSweep(simOpts, alpha, beta, targDist, targRad, dwellTime)
    %Finds the optimal gain and smoothing value (alpha & beta) for the
    %specified simulation settings. 
    ...
    velSlopes = linspace(0,-2,10)*targDist;
    ...
    for a=1:length(alpha)
        ...
        for b=1:length(beta)
            ...
            for v=1:length(velSlopes)
                ...
                simOpts.control.fVelX = [0 1];
                simOpts.control.fVelY = [0 velSlopes(v)];
