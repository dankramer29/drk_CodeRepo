function [ model, predVals ] = fitPW( cVec, posHat, velHat, targPos, fitOpts )
    %Fits a piecewise model of the user's control strategy. 
    targDist = matVecMag(targPos - posHat,2);
    speed = matVecMag(velHat,2);
    nDim = size(cVec, 2);

    %find the knots based on percentiles of the data
    ...
    rowIdx = 1:size(cVec,1);
    for n=1:nDim
        biasCols(rowIdx, n) = 1;
        rowIdx = rowIdx + size(cVec,1);
    end
