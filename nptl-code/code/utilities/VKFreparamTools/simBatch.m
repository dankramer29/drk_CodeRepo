function [ out ] = simBatch( opts, targPos, startPos, targRads )
    %Simulates a set of movements and returns the trajectories.
    %If startPos is a single row, then each movement will begin at the end
    %of the previous one. If startPos has as many rows as targPos, the
    %cursor will be reset to startPos for every trial.
    
    if nargin<4
        targRads = repmat(opts.trial.targRad, size(targPos,1), 1);
    end
