...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    //defines expected fields for input struct
    char *funcString;
    ...
    char fieldsPlant[][20] = {"alpha","beta","nonlinType","n1","n2","fStaticX","fStaticY","nDim","bCoef"};
    ...
    else if (strcmp(funcString,"init")==0)
    {
        ...
        checkFields(plant,fieldsPlant,9,"plant");
        ...
        bCoef = mxGetField(plant,0,"bCoef");
        if( mxGetM(bCoef) > mxGetN(bCoef) )
            coefLen = (int)mxGetM(bCoef);
        else
            coefLen = (int)mxGetN(bCoef);
        sim.plant.nbCoef = coefLen;
        memcpy(sim.plant.bCoef, mxGetPr(bCoef), coefLen*sizeof(double));
