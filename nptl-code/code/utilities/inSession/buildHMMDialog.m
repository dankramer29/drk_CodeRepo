function output = buildHMMDialog(streams, options, prompt)
...
if ~exist('prompt','var')
    ...
    prompt.clickStateThreshold = '0.5';
    ...
    prompt.excludeAmbiguousData = 'true'; % BJ: new option added 2/2017 to 
...
if ~isempty(prompt)
    ...
    promptfieldsDisplay{strcmp( promptfieldsDisplay, 'clickSource' )} = 'clickSource (dwell, click, click+overtarget)'; 
    response=inputdlgcol(promptfieldsDisplay,'Filter options', 1, struct2cell(prompt),'on',2);
...
%     options.clickShift = 0;
    otherwise
        error('buildHMMDialog: Don''t understand this clickSource. Options are ''dwell'' (only works with OL data for now), ''click'', or ''click+overtarget''.');
end

%BJ: the following happened just above for all 3 cases; extracting down here (but if
%always hard-coded to 0, why does this option exist?)  
% options.clickShift = 0;
...
%     %% if we need to clickshift, just shift the state in the stream  %BJ: this appears to be obsolete (always set to 0)
%     if isfield(options, 'clickShift'),
