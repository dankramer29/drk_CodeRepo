function optimizeGain( )
    %%
    %get user input and load block
    prompt = {'Optimize for Game Type (Fitts or Radial):','Optimize for Dwell Time (s):',...
        'Radial Diameter: ', 'Fitts Diameters: ', 'Fit Model to Block Num (CL Only):'};
    name = 'Parameters for Optimization';
    numlines = 1;
    defaultanswer = {'Radial','2.0','0.02','[0.0290    0.0345    0.0410    0.0488]',''};
    answer = inputdlg(prompt,name,numlines,defaultanswer);

    fOpts.taskName = answer{1};
    fOpts.dwellTime = str2num(answer{2});
    fOpts.radialDiameter = str2num(answer{3});
    fOpts.fittsDiameters = str2num(answer{4});
    fOpts.blockToFit = str2num(answer{5});
    
    if isempty(fOpts.blockToFit)
        error('Enter block to fit');
    end
    
    disp('Loading and formatting data');
    ...
    R = onlineR(loadStream([flDir num2str(fOpts.blockToFit) '/'], fOpts.blockToFit));
