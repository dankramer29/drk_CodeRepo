function buildSplitVKFT6(streams)
...
%% specifically prompt for the X and Y axis blocks
r1 = inputdlg({'X axis block', 'Y axis block'}, 'Specify blocks', 1, {'', ''});
if isempty(r1) || isempty(r1{1}) || isempty(r1{2})
    disp('filter build canceled - empty responses')
    return
end

xblock = str2double(r1{1});
yblock = str2double(r1{2});
disp(sprintf('Using block %i for X, block %i for Y', xblock, yblock));

options.blocksToFit = [xblock yblock];
options.dimSpecificBlocks(1) = xblock;
options.dimSpecificBlocks(2) = yblock;
