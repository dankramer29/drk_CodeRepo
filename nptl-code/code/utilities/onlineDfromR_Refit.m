function [D] = onlineDfromR_Refit(R,taskConstants,discretemodel,optionsD)
% ONLINEDFROMR
%
...
for nR = 1:length(R) %nR = trials? 
    ...
    if isfield(R,'clickState') && ~any(R(nR).clickTarget > 1)  %SNF: backwards compatibility, 1-click only
...
%         D(nD).clickTarget = tempClickTarg; %might as well make this explicit
   ...
   % would be easier. BUT this function also already outputs the hoverState, will this be a problem? 
    elseif any(R(nR).clickTarget > 1) %% this should catch multiClick OpenLoop and closedLoop. We want to define clickState by the instructed target
        ...
        D(nD).clickState = CS;%this is in bin size not 1-ms bins!! 
        if isfield(R(nR), 'contClickTarg') %&& ~any(R(nR).state == 11) && ~any(R(nR).state == 6) %SNF: in this onlineD, we don't care if hte trial failed
