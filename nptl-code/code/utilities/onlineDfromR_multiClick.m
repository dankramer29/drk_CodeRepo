function [D] = onlineDfromR_multiClick(R,taskConstants,discretemodel,optionsD)
% ONLINEDFROMR
%
...
%% output the "hoverstate" - SF: why is this logic so different from clickState? 
...
%%  get click state for each trial 
    if isfield(R,'clickState') %clickState is always 0 in openLoop 
% SF: changed this block to make this more flexible, assigning "clickState" here is useless 
...
%         end
        %isClicked = R(nR).clickState == clickState; 
% end stuff SNF got rid of to replace with this line: 
% SNF: This change makes this MC-compatible without changing the previous logic:
        isClicked = (R(nR).clickState > DiscreteStates.CLICK_IDLE); %CLICK_IDLE is 0. 
% SNF update: we *do* want clickState to stay clickTarg agnostic. Added new field (D.clickTarget) for multiple click targets.
% This went binary -> targ # -> sum of targ #'s normalized back down so you can't tell what target it was. 
% Reverting to original logic by commenting my changes out with "%%%%". keep in mind R.clickState comes out of the decoder:  
        %%%IC0 = double(isClicked); % SNF needed this double here
        %%%IC0(isClicked) = R(nR).clickTarget; %aha. now it's 0-4. 
        %%%IC1 = IC0(inds2keep);
        IC1 = isClicked(inds2keep); %most recent update: this is 0 or 1. -SF
        IC1_reshaped = reshape(IC1, optionsD.binSize,[]); %smooshed to bin. not ideal but okay. -SF
        %%%%CS = sum(IC1_reshaped, 1)./R(nR).clickTarget; %if the clickTarget is 1, like in normal click, this divides by 1 = nothing happens
        ...
        %Is this intentional? SF says yes because a threshold is applied to that number later on. 
       % D(nD).clickState = CS; %this is in bin size not 1-ms bins, values range from 0 to binSize 
%         if isfield(R(nR), 'contClickTarg')% if this isn't a field in R we don't need it in D
%             % make this variable bin size -sized too: 
...
%             CT2 = max(CT,[],1); %label any column with a clickTarg as a clicking bin
%             D(nD).clickTarget = CT2; 
%         end
%% This OL-MC condition may not be needed- it'll probably be safer to use the original OL and CL with little changes        
...
% in the case where there's no clickState, it's openLoop,
% this conditional catches openLoop (bc it's an else to the above if) that is also multiClick (clickTarget > 1) 
        isClicked = R(nR).contClickTarg; %0 if dwell or not over the target, target # otherwise. 
        IC1 = isClicked(inds2keep);
        IC1_reshaped = reshape(IC1, optionsD.binSize,[]); 
        CS = sum(IC1_reshaped, 1)./R(nR).clickTarget; %SF: divide by click target for backward compatibility  
    else %otherwise, openLoop and not multiclick 
       % D(nD).clickState = false(1,total_bins);
        CS = false(1,total_bins);
        ...
        % we might need to use this OL logic and just modify for OL. 
    end
    % this should always happen, moved out of if statements: 
    D(nD).clickState = CS; %this is in bin size not 1-ms bins, values range from 0 to binSize 
    if isfield(R(nR), 'contClickTarg') && ((any(R(nR).state) == 13) || (any(R(nR).state) == 4))
...
%% in buildHMM code, taskConstants is usually "[]"
        if isfield(taskConstants,'STATE_ACQUIRE')%SNF: why check for ACQUIRE and not HOVER?
            dwellState = taskConstants.STATE_ACQUIRE; %SNF: why is this taskConstants and the later one is CursorStates? 
            isOverTarget = R(nR).state == dwellState;
            dwellState2 = CursorStates.STATE_HOVER;  
            isOverTarget = (R(nR).state == dwellState2) | isOverTarget;
        elseif any(R(nR).clickTarget > 1) %multiclick 
            isOverTarget = (R(nR).state == CursorStates.STATE_ACQUIRE) | ...
                            (R(nR).state == CursorStates.STATE_HOVER); 
        else %what in the actual what would cause this field to be useful? 
