...
%             rodWidth = 0.7; % fraction of XYZ acceptance window rod spans
            ...
            thetaXZ_xPC_limit_radian_limits = thetaXY_xPC_limit; %XZ plane angle/azimuth (spherical coordinates)
            thetaXZ_xPC_limit = thetaXY_radian_limits;
...
%                        
                        ...
                        % March 22 2017
                        theta_XY = ( sclTargPos(4) / thetaXY_xPC_limit)*thetaXY_radian_limits; % in radians
                        theta_YZ = ( sclTargPos(5) / thetaYZ_xPC_limit_radian_limits)*thetaYZ_xPC_limit; %  in radians
                        theta_XZ = ( sclTargPos(6) / thetaXZ_xPC_limit_radian_limits)*thetaXZ_xPC_limit; % azimuth, in radians
                        
                        Rorigin = [1 0 0; 0 1 0; 0 0 1];
                    
                        rZ = [0 0 1];
                        rY = [0 1 0];
                        rX = [1 0 0];
                        
                        asymDiagX = [0     -rX(3)  rX(2);
                            rX(3)     0  -rX(1);
                            -rX(2) rX(1)      0];
                        
                        asymDiagY = [0     -rY(3)  rY(2);
                            rY(3)     0  -rY(1);
                            -rY(2) rY(1)      0];
                        
                        asymDiagZ = [0     -rZ(3)  rZ(2);
                            rZ(3)     0  -rZ(1);
                            -rZ(2) rZ(1)      0];
                        
                        
                        RotX = cos(theta_YZ).*Rorigin + (1-cos(theta_YZ)).*(rX'*rX) + ...
                            sin(theta_YZ).*asymDiagX;
                        RotY = cos(theta_XZ).*Rorigin + (1-cos(theta_XZ)).*(rY'*rY) + ...
                            sin(theta_XZ).*asymDiagY;
                        
                        RotZ = cos(theta_XY).*Rorigin + (1-cos(theta_XY)).*(rZ'*rZ) + ...
                            sin(theta_XY).*asymDiagZ;
                        
                        
                        R_total = RotX*RotY*RotZ*Rorigin;
...
%                             theta_XY, theta_YZ, mat2str( R_total ) );
            ...
            if numDims > 5
                sclCursorPos(6) = double(data.cursorPosition(6))*armTargetPosScale*armFlip6;
            else
                sclCursorPos(6) = 0;
            end
            ...
            switch numDims
                ...
                case {4, 5, 6} % rectangular prism
                    ...
                    % theta_XY aka theta_Z
                    % theta_YZ aka theta_X
                    ...
                    theta_XZ =  ( sclCursorPos(6) / thetaXZ_xPC_limit_radian_limits)*thetaXZ_xPC_limit; %
                    
                    % Standard computer graphics 3D rotations. Second two
                    % rotations apply to where the object already is. First
                    % is in world-centered coordinates.
                    Rorigin = [1 0 0; 0 1 0; 0 0 1];
                    ...
                    R_total = RotX*RotY*RotZ*Rorigin;
%                     R_total = RotZ*RotY*RotX*Rorigin;
