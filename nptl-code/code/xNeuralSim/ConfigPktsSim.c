...
#define S_FUNCTION_NAME ConfigPktsSim
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
/* %%%-SFUNWIZ_defines_Changes_BEGIN --- EDIT HERE TO _END */
#define NUM_INPUTS          5
/* Input Port  0 */
#define IN_PORT_0_NAME      continue_in
...
/* Input Port  3 */
#define IN_PORT_3_NAME      cbhwverMajor
...
/* Input Port  4 */
#define IN_PORT_4_NAME      cbhwverMinor
...
/* Output Port  0 */
#define OUT_PORT_0_NAME      continue_out
...
/* Output Port  1 */
#define OUT_PORT_1_NAME      pktData
...
/* Output Port  2 */
#define OUT_PORT_2_NAME      pktSize
...
extern void ConfigPktsSim_Outputs_wrapper(const int32_T *continue_in,
                          const uint32_T *clk,
                          const uint8_T *start,
...
{
    const int32_T   *continue_in  = (const int32_T*) ssGetInputPortSignal(S,0);
    const uint32_T   *clk  = (const uint32_T*) ssGetInputPortSignal(S,1);
    const uint8_T   *start  = (const uint8_T*) ssGetInputPortSignal(S,2);
    const uint16_T   *cbhwverMajor  = (const uint16_T*) ssGetInputPortSignal(S,3);
    const uint16_T   *cbhwverMinor  = (const uint16_T*) ssGetInputPortSignal(S,4);
    int32_T        *continue_out  = (int32_T *)ssGetOutputPortRealSignal(S,0);
    uint8_T        *pktData  = (uint8_T *)ssGetOutputPortRealSignal(S,1);
    real_T        *pktSize  = (real_T *)ssGetOutputPortRealSignal(S,2);

    ConfigPktsSim_Outputs_wrapper(continue_in, clk, start, cbhwverMajor, cbhwverMinor, continue_out, pktData, pktSize);
}
