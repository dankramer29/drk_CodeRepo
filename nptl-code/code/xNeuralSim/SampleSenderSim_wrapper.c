 ...
 *
 */
void SampleSenderSim_Outputs_wrapper(const uint32_T *clk,
...
static int udpDataIdx = 0;
static int unsigned cbPktCount = 0;
int ch;

// inputs:
static unsigned int sysclk = 0; // = clk[0];
...
contDataPkt = (cbPKT_GROUP*)&udpData[udpDataIdx];
...
udpDataIdx += (contDataPkt->dlen * 4) + cbPKT_HEADER_SIZE;

// now decide if the udp packet is filled enough to send
if ((++cbPktCount == maxNumCbPkts[0]) || // max number of cerebus pkts to fit in udp pkt
    (sampleNum[0] == (numSamples[0] - 1))) // last sample in sampleData[]
{
    cbPkts[0] = cbPktCount;
    cbPktCount = 0;
    udpSize[0] = udpDataIdx; // pkt gets sent when non-zero udpSize
    udpDataIdx = 0;
}
