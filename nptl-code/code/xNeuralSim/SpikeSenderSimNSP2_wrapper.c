 ...
 *
 */
void SpikeSenderSimNSP2_Outputs_wrapper(const uint32_T *clk,
...
static int udpDataIdx_NSP2;
static unsigned int cbPktCount_NSP2;
int i;
cbPKT_SPK* spikePkt;
...
if (chanNum[0] == 0)
{
    cbPktCount_NSP2 = 0; // number spikes cumulative in for loop
    udpDataIdx_NSP2 = 0; // index into the udp data array
}
...
if (spikeCounts[chanNum[0]])
{
    ...
    {
        spikePkt = (cbPKT_SPK*)&udpData[udpDataIdx_NSP2];
        ...
        udpDataIdx_NSP2 += (spikePkt->dlen * 4) + cbPKT_HEADER_SIZE;

        cbPktCount_NSP2++; // update number of cerebus packets sent
    }
}
...
// then send it 
if ((udpDataIdx_NSP2 >= MAX_UDP_DATA_SIZE) || (chanNum[0] == (numChans[0]-1)))
{
    cbPkts[0] = cbPktCount_NSP2;
    cbPktCount_NSP2 = 0;
    udpSize[0] = udpDataIdx_NSP2; // udp pkt gets sent when non-zero udpSize
    udpDataIdx_NSP2 = 0; // start again with a fresh udp buffer
