...
localPort = 49454;            % local port, keep at 5554
...
disp(sprintf('Sending to: %s', localIP))
disp(sprintf('Sending to: %s', remoteIP))
...
%     nspClock = [nspClock(2:end) nspTime_current];
    ...
    if S.MODE >= 2
        ...
        if ~isempty(currentGameUDPdata)
            ...
            velCmd(1) = velCmd(1) + 0.3; %artificial bias
...
%         mouseData([1 2 4]) = robotCmd;
    ...
    udpData = zeros(1,64);
    actualSendPacket = [t_send velocity clickState]; 
    % disp(t_send);
